...
// A version mismatch is now a RANGE question, and its tests live in
// wire_version_test.go.
//
// TestAVersionMismatchIsRefused stood here and was removed rather than kept,
// because what it asserted became untrue in the case it called "the real case":
// a node OLDER than the control plane is exactly what a server-first rollout
// produces, and it is now ACCEPTED whenever the two ranges overlap. Keeping a
// green test whose comment says the opposite is worse than having none.
// TestAWireRangeWithNoOverlapIsRefused replaces it and asserts strictly more:
// both directions, the permanence, both ranges in the message, and which side
// the message tells the operator to upgrade.
...
func TestOneWedgedNodeDoesNotStallADestroy(t *testing.T) {
	...
	// Serial would be at least three timeouts; concurrent is about one, so the
	// ceiling sits between them.
	//
	// 2.5x RATHER THAN 2x, AND NOT 3x. Three is exactly what serial costs, so a
	// ceiling there would pass against the very implementation this test exists to
	// refuse; two leaves only one timeout of slack above the concurrent case,
	// which is a wall clock under t.Parallel() with the whole package running.
	// Two and a half keeps half a timeout clear of serial and gives one and a half
	// above concurrent.
	if elapsed := time.Since(start); elapsed > 5*timeout/2 {
...
func TestANodeCannotReleaseAnotherNodesLease(t *testing.T) {
	...
	if byOwner, err := p.AuthorizeLease("a", "inc-a", "l1"); err != nil || !byOwner {
		t.Fatalf("the recorded owner was admitted byOwner=%v err=%v, want by ownership", byOwner, err)
	}
	...
	// AND A LEASE NOBODY RECORDED IS ADMITTED ON MEMBERSHIP ALONE, WHICH THE
	// ANSWER SAYS. The wire must then ask the ledger where the lease is placed —
	// from THIS answer, not from a second look at the record, because an owner
	// recorded between the two looks would turn "ownerless" into "recorded" and
	// skip the ledger for a lease another host's compute is still on.
	if byOwner, err := p.AuthorizeLease("b", "inc-b", "l2"); err != nil || byOwner {
		t.Fatalf("an unrecorded lease: byOwner=%v err=%v, want admitted by membership only", byOwner, err)
	}
}
...
func (countingRegistrar) NodeWithdrawn(context.Context, string, int64, string) error { return nil }

func (countingRegistrar) ForgetEveryNode(context.Context) error { return nil }

func (countingRegistrar) BumpDispatch(context.Context, string) (int64, error) { return 0, nil }
...
func (countingRegistrar) SettleCompletionOnTerminalLease(
	context.Context, string, int64, alloc.Phase,
) (bool, error) {
	return true, nil
}
...
// AN OWNER RECORDED AFTER THE ADMISSION DOES NOT SKIP THE LEDGER, staged
// deterministically rather than raced.
//
// The wire admits a current process on membership when nothing is recorded
// against the lease, and must then ask the ledger where the lease is placed.
// Deciding that from a SECOND look at the plane's records let a replacement
// adopt the lease in between — after a control-plane restart, exactly when
// nothing is recorded — and turn "ownerless" into "recorded", skipping the
// ledger for a lease the other host's compute was still on. The hook stands in
...
func TestAnOwnerRecordedAfterAuthorizationDoesNotSkipTheLedger(t *testing.T) {
	...
	p := New(log, deployment, time.Minute, WithRegistrar(a), WithTierCatalog([]config.Tier{tier}))
	for _, name := range []string{"n1", "n2"} {
		if _, err := p.Register(t.Context(), nodeapi.RegisterRequest{
			Version: nodeapi.Version, Node: name, Provider: config.ProviderDocker,
			Deployment: deployment, Incarnation: name + "-1", VCPU: 8, Memory: 32 * config.GiB,
		}); err != nil {
			t.Fatalf("register %s: %v", name, err)
		}
	}

	// A lease placed on one host, open, with nothing recorded against it: the
	// state after a control-plane restart, before that host has re-adopted it.
	// Escrow chooses the host, so the OTHER one is the intruder.
	lease, err := a.Reserve(t.Context(), tier.Label)
	if err != nil {
		t.Fatalf("reserve: %v", err)
	}
	holder := lease.TargetNode
	intruder := "n2"
	if holder == "n2" {
		intruder = "n1"
	}
	...
	// THE WINDOW: the intruder has been admitted on membership, and the holder
	// adopts the lease before the wire looks at the ledger. The hook runs only
	// on that branch, so entering it proves the admission; the adoption is
	// checked to have RECORDED the holder, or the race was never staged.
	var adoptions, recorded int
	p.afterAuthorizeForTest = func(node, leaseID string) {
		if node != intruder || leaseID != lease.ID {
			return
		}

		adoptions++
		p.AdoptOwnership(holder, holder+"-1", []string{lease.ID})

		if byOwner, err := p.AuthorizeLease(holder, holder+"-1", lease.ID); err == nil && byOwner {
			recorded++
		}
	}

	srv := httptest.NewServer(Handler(log, p, a, nil))
	t.Cleanup(srv.Close)

	body, err := json.Marshal(nodeapi.ReleaseRequest{Epoch: lease.Epoch, Outcome: string(alloc.PhaseFailed)})
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
	req, err := http.NewRequestWithContext(t.Context(), http.MethodPost,
		srv.URL+"/v1/nodes/"+intruder+"/leases/"+lease.ID+"/release", bytes.NewReader(body))
	if err != nil {
		t.Fatalf("request: %v", err)
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set(nodeapi.HeaderIncarnation, intruder+"-1")
	...
	if adoptions != 1 || recorded != 1 {
		t.Fatalf("the window was entered %d times and the holder recorded %d times, want once each; "+
			"the race was not staged", adoptions, recorded)
	}

	// REFUSED BY THE LEDGER'S PLACEMENT, not by anything earlier: the refusal
	// names where the lease is placed.
	...
	if resp.StatusCode != http.StatusConflict || refusal.Code != nodeapi.CodeSuperseded ||
		!strings.Contains(refusal.Message, "the ledger places lease") {
		t.Fatalf("release answered %d %s %q, want 409 %s from the ledger's placement",
			resp.StatusCode, refusal.Code, refusal.Message, nodeapi.CodeSuperseded)
	}
	if _, err := a.Lease(t.Context(), lease.ID); err != nil {
		t.Fatalf("the lease another host's compute is on was released: %v", err)
	}
}
