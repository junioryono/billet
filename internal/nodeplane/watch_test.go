...
type ledger struct {
	...
	// withdrawn records each host's withdrawal with the fence it presented, and
	// withdrawFailures is how many to reject before accepting one.
	withdrawn        map[string]withdrawal
	withdrawFailures int
	// holdWithdraw, when set, parks the next withdrawal inside the ledger write
	// until it is closed, announcing on withdrawEntered that it is parked. That is
	// what lets a test stage a registration landing during the write.
	holdWithdraw    chan struct{}
	withdrawEntered chan struct{}
...
// packages cannot see each other's test types and each has a fake that records
...
// NodeWithdrawn records a withdrawal and the fence it presented.
func (l *ledger) NodeWithdrawn(
	_ context.Context, name string, epoch int64, incarnation string,
) error {
	l.mu.Lock()
	defer l.mu.Unlock()

	if l.withdrawFailures > 0 {
		l.withdrawFailures--

		return errors.New("ledger unavailable")
	}

	l.withdrawn[name] = withdrawal{name: name, epoch: epoch, incarnation: incarnation}

	hold, entered := l.holdWithdraw, l.withdrawEntered
	l.holdWithdraw = nil
...
// withdrawalOf reports what the ledger was told about one host's withdrawal.
func (l *ledger) withdrawalOf(name string) (withdrawal, bool) {
	l.mu.Lock()
	defer l.mu.Unlock()

	w, ok := l.withdrawn[name]
...
func (l *ledger) SettleCompletionOnTerminalLease(
...
func (l *ledger) BumpDispatch(context.Context, string) (int64, error) { return 0, nil }

// goneCount is how many hosts the ledger has been told are gone.
func (l *ledger) goneCount() int {
	l.mu.Lock()
	defer l.mu.Unlock()
