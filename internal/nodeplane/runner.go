...
func (p *Plane) dispatch(ctx context.Context, n *node, pend *pending) (nodeapi.CommandResult, error) {
	...
	// THE CAPTURE AND THE ENQUEUE ARE ONE ACT WITH RESPECT TO REGISTRATION.
	//
	// A compute barrier reads its fence from the LEDGER and then queues here, and
	// registration commits the ledger epoch before installing the incarnation
	// under this mutex. Checking both — no registration in flight, and the
	// installed epoch still the one captured — closes the window in which the old
	// process would answer under the new registration's epoch.
	if pend.expectedLedgerEpoch > 0 {
		current, registered := p.nodes[n.name]
		_, registering := p.registering[n.name]
		...
		case registering:
			...
			return nodeapi.CommandResult{}, fmt.Errorf(
				"%w: node %s has a registration in progress, so nothing it says now belongs to "+
					"a settled incarnation", ErrNoNode, n.name)
		case !registered || current != n || n.ledgerEpoch != pend.expectedLedgerEpoch:
			p.mu.Unlock()

			return nodeapi.CommandResult{}, fmt.Errorf(
				"%w: node %s re-registered between this question's fence being captured and "+
					"the question being asked", ErrNoNode, n.name)
		}
	}

	if err := p.queueLocked(ctx, n, pend); err != nil {
		p.mu.Unlock()

		return nodeapi.CommandResult{}, err
	}
...
func (p *Plane) dispatchBound(
	...
	// A COMPLETION THAT CANNOT REACH ITS HOLDER CAN ONLY CORRECT, NEVER SETTLE,
	// and there are three ways not to reach one. The record names the process
	// that was handed the launch and a REPLACEMENT is refused the destroy
	// because it truthfully knows nothing — right, and it left this completion
	// with nowhere to go: the record is never dropped by a re-registration, so
	// the bound destroy answered "holder unavailable" for as long as the control
	// plane ran, while the listener renewed the lease and it never expired. A
	// holder nobody REPLACED — the host is simply gone — is the same shape
	// without the replacement. And a lease this plane holds NO RECORD for,
	// because a control-plane restart forgot it and the host's inventory has not
	// re-adopted it, used to be settled from this plane's CACHED inventory
	// snapshot, which has no ordering relationship to the lease's quarantine: a
	// snapshot taken before a build became visible and never refreshed — every
	// later listing failed — would settle the lease under a running build. That
	// was the one-observation-plus-a-clock shape the compute barrier removed,
	// one seam over.
	//
	// So none of the three settles anything itself: the host's ordinary
	// inventory reconciliation, a fresh observation taken after the grace, or an
	// operator's forced release settles the lease, and this only corrects a
	// provisional verdict to GitHub's. An open lease — renewed by a superseded
	// process still draining, or by a replacement that adopted the build —
	// answers "holder unavailable" exactly as before.
	//
	// THE RECORD ENDS WITH THE LEASE. MayMutateLease reads it to keep a
	// superseded process entitled to renew and release what it holds, so it
	// stays while the lease is open; once the ledger says the lease is terminal
	// nothing can be entitled to anything about it, and the record would
	// otherwise live for the plane's lifetime, one per job whose holder died.
	// The wire answers a superseded process asking about a terminal lease with
	// the ledger's truth, so dropping the record cannot strand its drain.
	n := p.nodes[nodeName]
	unreachable := !owned || n == nil || p.holderReplacedLocked(nodeName, owner)
	if unreachable {
		if p.registrar == nil {
			p.mu.Unlock()
		...
		settled, err := p.registrar.SettleCompletionOnTerminalLease(
			ctx, leaseID, leaseEpoch, outcome)
		if err == nil && settled && owned {
		...
		return nodeapi.CommandResult{}, "", settled, nil
	}

	if owner.node != nodeName || n.incarnation != owner.incarnation {
		p.mu.Unlock()
...
// holderReplacedLocked reports whether a lease's recorded holder is a process
// other than the one this node's commands currently reach.
//
// WHAT IT SAYS IS NARROW: dead, or superseded and still draining what it holds.
// It cannot tell those apart and nothing here needs it to — the caller only
// asks the ledger whether something ELSE has already settled the lease, and a
// draining process keeps its lease renewed and open. An empty current
// incarnation is a host that presented none, and answers false; so does a
// record naming another host, which the caller refuses on its own.
func (p *Plane) holderReplacedLocked(nodeName string, owner leaseOwner) bool {
	n := p.nodes[nodeName]
	if n == nil || owner.node != nodeName {
		return false
	}

	return n.incarnation != "" && n.incarnation != owner.incarnation
}

// queueLocked makes a command reachable by a node, charging the launch fence.
//
// THE FENCE AND THE APPEND MUST BE ONE ACT, and that is why this writes to the
// ledger while holding p.mu — which nothing else on the hot path does lightly.
// What matters is that no other queueing can interleave between them; the order
// of the two statements BELOW does not, and saying otherwise would be a rule
// whose stated reason is untrue. Mutation testing established that: swapping
// them changes no behaviour, because a poller needs p.mu to take anything and
// cannot run until this critical section ends either way.
//
// WHAT WOULD BREAK IT IS MOVING THE BUMP OUT OF THIS FUNCTION. A compute barrier
// captures a host's dispatch generation WITHOUT the mutex and then queues its
// inventory command with it; the acknowledgement is accepted only if that number
// has not moved. Bump outside the lock and a launch can advance the generation,
// block before appending, let a barrier read the new value and queue itself
// AHEAD — and the barrier's answer, taken before the launch runs, still looks
// current. The host is reported clear with a launch about to start on it. Inside
// the lock there is no such window: a generation the barrier can observe belongs
// to a launch that is already in the queue ahead of it.
//
// So the protection is structural — the `Locked` suffix, and the fact that both
// call sites hold p.mu across it. A crash between the two statements
// over-invalidates, costing one barrier round, which is the direction to fail in.
//
// ONLY A LAUNCH TAKES IT. A destroy, sweep, tend or inventory cannot create
// compute, and charging them would void proofs for nothing.
func (p *Plane) queueLocked(ctx context.Context, n *node, pend *pending) error {
	if pend.cmd.Kind == nodeapi.CommandLaunch && p.registrar != nil {
		if _, err := p.registrar.BumpDispatch(ctx, n.name); err != nil {
			// THE LAUNCH FAILS RATHER THAN GOING OUT UNFENCED, the same answer a
			// failed Bind gives. Nothing has been queued, so nothing started, and
			// the caller may release the lease with a clear conscience.
			return fmt.Errorf(
				"%w: could not advance the launch fence of node %s, so this launch was not sent: %w",
				ErrNoNode, n.name, err)
		}
	}
