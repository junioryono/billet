...
// Presign puts the whole signature in the query string, so a URL alone is the
// credential.
//
// A DIFFERENT SIGNATURE FROM Sign, NOT A REARRANGEMENT OF ONE. The six X-Amz-*
// parameters are part of the canonical request rather than headers, the payload
// hash is the literal UNSIGNED-PAYLOAD rather than a digest, and only Host is
// signed -- a presigned URL is fetched by something billet does not control and
// cannot make send a particular header.
//
// EXPIRY IS THE ONLY THING BOUNDING IT. Anyone holding the URL can fetch the
// object until it lapses, so callers pass the shortest window that covers the
// fetch and stage nothing behind one that a longer secret would protect.
func Presign(
	req *http.Request,
	creds Credentials,
	region, service string,
	...
	if req == nil || req.URL == nil {
		return "", errors.New("aws: cannot presign a request with no url")
	}

	if strings.TrimSpace(region) == "" || strings.TrimSpace(service) == "" {
		return "", errors.New("aws: presigning needs a region and service")
	}

	if expires <= 0 || expires > 7*24*time.Hour {
		return "", fmt.Errorf("aws: %s is not a presign lifetime; it must be positive and "+
			"no more than the seven days SigV4 allows", expires)
	}
	...
	// THE TOKEN IS SIGNED, NOT APPENDED AFTERWARDS. A session credential whose
	// token is added after signing produces a URL AWS rejects, and the rejection
	// names the signature rather than the token.
	if creds.SessionToken != "" {
