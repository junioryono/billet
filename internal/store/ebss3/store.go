...
type objectAPI interface {
	...
	// Delete removes one object. It is idempotent — a missing object is not an
	// error — and is used only by decommission's Purge, never on the cache's hot
	// path, which advances the pointer by conditional PUT and never deletes.
	...
	err = s.mutate(ctx, key, func(state *keyState) error {
		...
		if state.Writer != nil && now.Before(state.Writer.Lease.Expires) {
			return &storecontract.WriterHeldError{
				Key: key, Holder: state.Writer.Holder, Expires: state.Writer.Lease.Expires,
			}
		}
...
func (s Store) ReleaseWriter(
	...
	if err := checkKey(lease.Key); err != nil {
		return err
	}
	if strings.TrimSpace(lease.ID) == "" || fence == 0 {
		return errors.New("ebs-s3: a writer release needs the lease and fence that were issued")
	}

	return s.mutate(ctx, lease.Key, func(state *keyState) error {
		// The fence is the same triple PublishCAS checks, so this can never clear
		// a newer writer's record; and a record that is not ours needs no write,
		// because the key is already free of the lease being released. A lost
		// PUT that landed comes back through mutate's reload as this branch.
...
func (s Store) Snapshot(
	...
	now := s.now()
	// THE TOKEN NAMES THE OPERATION, NOT THE ATTEMPT. A volume is snapshotted at
	// most once and then deleted, so key+volume identifies this publication; a
	// clock in the token would give a restarted attempt a token nothing carries,
	// and the at-most-once rule in ebsAPI.CreateSnapshot rests on the lookup.
	snapshot, err := s.blocks.CreateSnapshot(ctx, volume.Handle, now,
		s.requestToken("snapshot", volume.Key, volume.Handle))
	if err != nil {
		return storecontract.Candidate{}, err
	}
...
func (s Store) Evict(ctx context.Context, olderThan time.Duration) error {
	...
	for _, snapshot := range remove {
		if !referenced[snapshot] {
			// NOT evictDelete: these handles come from this store's own namespaced
			// state, and the state record was already removed above. A not-owned
			// refusal here is corrupted or altered ownership tags, not a foreign
			// resource — skipping it would strand a snapshot with nothing left
			// pointing at it, unrecoverable if its cache-owner tag is also gone. Any
			// error aborts.
	...
	for _, snapshot := range snapshots {
		...
		if err := s.evictDelete(ctx, "snapshot", snapshot.ID, s.blocks.DeleteSnapshot); err != nil {
			return err
		}
	}
	...
	for _, volume := range volumes {
		...
		if err := s.evictDelete(ctx, "volume", volume.ID, s.blocks.DeleteVolume); err != nil {
			return err
		}
	}
...
// evictDelete runs one ORPHAN-SWEEP delete, treating a not-owned refusal as a SKIP
// rather than a failure. The block listings can surface a resource this store may
// not delete — another deployment sharing this cache-owner tag but carrying a
// different deployment-owner tag — and one such resource must not abort the sweep
// of everything this store legitimately can evict. Any other error still aborts.
//
// This is ONLY for the orphan sweeps: a delete driven by this store's own state
// (the remove list above) must abort on a not-owned refusal, because there the
// mismatch means altered state rather than someone else's resource.
func (s Store) evictDelete(
	ctx context.Context, kind, id string, del func(context.Context, string) error,
) error {
	err := del(ctx, id)
	if err == nil {
		return nil
	...
	if errors.Is(err, errNotOwned) {
		slog.Default().Warn("ebs-s3: skipping eviction of a resource this store does not own",
			"kind", kind, "id", id)
...
// Purge removes every cache resource this store owns — snapshots, available
// volumes, and the S3 state index — for a full deployment teardown (billet
// decommission, before `terraform destroy`). It is NOT eviction: it ignores
// retention and age and takes everything.
//
// It REFUSES while any state object records a LIVE active-clone lease, because that
// means billet still believes a job holds a clone — stop the node so compute drains
// first. A resource another deployment owns (one that shares this cache-owner tag
// but not this deployment-owner tag) is skipped, never deleted, the same as the
// orphan sweep.
func (s Store) Purge(ctx context.Context) (PurgeReport, error) {
	...
	for _, key := range keys {
		body, _, found, err := s.objects.Get(ctx, key)
		if err != nil {
			return report, err
		}
		if !found {
		...
		// VALIDATED, not merely parsed — the same check Evict makes. A structurally
		// valid but incomplete state object ({} or a missing key) must not read as
		// "no active leases": Purge deletes, so an unverifiable state has to fail
		// CLOSED, or a corrupted index lets Purge nuke a cache a job still holds.
		var state keyState
		if err := json.Unmarshal(body, &state); err != nil || state.Key == "" {
			return report, fmt.Errorf("ebs-s3: state object %s is not a valid cache state", key)
		}
		if key != s.stateKey(state.Key) {
			return report, fmt.Errorf("ebs-s3: state object %s is outside this owner's cache "+
				"namespace", key)
		}
		for _, active := range state.Active {
			// A ZERO EXPIRY IS UNVERIFIABLE, not expired: treat it as live and refuse,
			// rather than fail open on a truncated active record.
			if active.Expires.IsZero() || now.Before(active.Expires) {
				live++
			}
		}
	}
	if live > 0 {
		return report, fmt.Errorf("ebs-s3: refusing to purge the cache while %d active clone "+
			"lease(s) may still be live — stop the node so compute drains, then retry", live)
	}
	...
	for _, snapshot := range snapshots {
		deleted, err := s.purgeDelete(ctx, "snapshot", snapshot.ID, s.blocks.DeleteSnapshot)
		if err != nil {
			return report, err
		}
		if deleted {
	...
	for _, volume := range volumes {
		deleted, err := s.purgeDelete(ctx, "volume", volume.ID, s.blocks.DeleteVolume)
		if err != nil {
			return report, err
		}
		if deleted {
	...
	// The state index LAST, so an interrupted purge leaves it pointing at blocks a
	// re-run can still find and remove — the block listings are owner-tag-filtered
	// on EC2 and do not depend on it, but leaving it is the safer crash order.
	for _, key := range keys {
...
// purgeDelete deletes one owned resource, skipping (rather than aborting on) a
// foreign one. It reports whether the resource was deleted; false means it was
// skipped as not-owned. Any non-ownership error aborts.
func (s Store) purgeDelete(
	ctx context.Context, kind, id string, del func(context.Context, string) error,
) (bool, error) {
	err := del(ctx, id)
	switch {
	...
	case errors.Is(err, errNotOwned):
		slog.Default().Warn("ebs-s3: skipping purge of a resource this store does not own",
			"kind", kind, "id", id)
...
// CheckAccess probes the cache bucket with a GET under the owner's own state
// prefix — the path the node's policy actually grants. The VERDICT IS NOT
// BINARY, and the caller must not treat every error as broken: S3 answers 404
// for a missing key only when the caller holds s3:ListBucket on the bucket,
// and billet's minimal policy conditions that grant on s3:prefix — a context
...
func (s Store) CheckAccess(ctx context.Context) error {
	...
	// minimal grant and read as a broken bucket.
	key := s.stateKey("billet-preflight-probe-does-not-exist")
	if _, _, _, err := s.objects.Get(ctx, key); err != nil {
		return fmt.Errorf("ebs-s3: the cache bucket did not answer a probe read: %w", err)
	}
