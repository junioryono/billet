...
// holder is dead because its cookie says six hours requires the observer and the
// holder to agree about the time, and they do not have to: a clock running ahead
// makes a lock taken seconds ago look ancient. A counter that MOVED is proof of
// life that needs no agreement about anything.
...
//     genuinely run longer than the bound -- the raw write is deliberately
//     unbounded and imports up to a terabyte -- and an observer's clock can simply
//     be ahead. Both produce an old-but-live holder, and breaking its lock is the
//     exact corruption the lock exists to prevent.
func breakable(before, after heartbeat, cookieAge time.Duration) (bool, string) {
	...
	return true, fmt.Sprintf("its heartbeat counter did not move while this was watching and "+
		...
		cookieAge.Round(time.Second), StaleLockAfter)
}
