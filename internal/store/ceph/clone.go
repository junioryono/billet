...
// convenience. Choosing a generation when a caller names only an image is the
// storage layer's job, so the clone path refuses to invent a rule that would then
// have to be unpicked. An operator naming `ubuntu-2404-x64` gets a message
// pointing at the snapshot they meant.
//
