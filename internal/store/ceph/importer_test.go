...
type importFake struct {
	...
	// deadlines is parallel to calls: deadlines[i] is the bound the context of
	// calls[i] carried, zero when it carried none. APPENDED IN THE ONE PLACE
	// calls IS, which is the only reason two slices are safe here.
	//
	// It exists because success cannot tell a FRESH bound from an INHERITED one.
	// context.WithTimeout on an already-bounded parent keeps the parent's
	// deadline, so cleanup derived from an exhausted context and cleanup derived
	// from a live one differ only in whether the runner happened to be quick --
	// which is a race in CI and a comparison here.
	deadlines []time.Time
	...
	// onSnapRemove fires once, when a withdrawal issues its `snap rm`. It is how a
	// test marks that instant from INSIDE the call, which is the only place the
	// ordering of what comes after it can be observed.
	onSnapRemove func()
...
func (f *importFake) run(ctx context.Context, _ string, args []string) ([]byte, error) {
	...
	if sub == "snap" && slices.Contains(args, "rm") {
		...
		if f.snapRemoveTimesOut {
			// WHAT execRunner RETURNS WHEN A COMMAND CONSUMES ITS WHOLE DEADLINE,
			// returned WITHOUT waiting that deadline out. Waiting was the earlier
			// shape and it is what made the test flaky: c.wait had to be small
			// enough that the test did not sit on it, and the confirmation and
			// metadata cleanup that follow are bounded by that same small value --
			// so one descheduling on a loaded runner expired a window the test
			// needed to survive. What the caller must get right is which BOUND the
			// cleanup runs on, and that is asserted directly.
...
// deadlineOf reports the bound the first invocation matching every fragment was
// given, matched exactly the way ranWith matches.
//
// The second result is false when nothing matched OR when what matched carried no
// deadline, because both answers mean the same thing to a caller asking which
// bound an operation ran under: there is no bound here to compare.
func (f *importFake) deadlineOf(fragments ...string) (time.Time, bool) {
	for i, call := range f.calls {
		if !matchesCall(call, fragments) {
			continue
		}

		if f.deadlines[i].IsZero() {
			return time.Time{}, false
		}

		return f.deadlines[i], true
	}

	return time.Time{}, false
}

// matchesCall is the predicate ranWith and deadlineOf share, so the two cannot
// disagree about which invocation a test is naming.
func matchesCall(call, fragments []string) bool {
...
func TestImportGenerationWritesTheImageAndPublishesIt(t *testing.T) {
	...
	gen, err := importClient(t, f).ImportGeneration(t.Context(), "ubuntu-2404-x64", raw, "2.336.0", "", "7", importAt)
	if err != nil {
	...
	if !f.ranWith("image-meta", "set", GuestContractKey+"."+gen, "7") {
		t.Errorf("the guest contract was not recorded per generation; billet ran %v", f.calls)
	}
}
...
func TestImportGenerationCreatesTheHeadWhenItIsAbsent(t *testing.T) {
	...
	if _, err := importClient(t, f).ImportGeneration(t.Context(), "ubuntu-2404-x64", raw, "2.336.0", "", "7", importAt); err != nil {
		t.Fatalf("import: %v", err)
	}
...
func TestImportGenerationSeparatesAnAbsentImageFromABrokenCluster(t *testing.T) {
	...
	_, err := importClient(t, f).ImportGeneration(t.Context(), "ubuntu-2404-x64", raw, "2.336.0", "", "7", importAt)
	if err == nil {
...
func TestImportGenerationRefusesAnUnaddressableImageName(t *testing.T) {
	...
	for _, name := range []string{"", "other-pool/image", "image@gen", "-rf", " leading"} {
		...
		if _, err := importClient(t, f).ImportGeneration(
			t.Context(), name, raw, "2.336.0", "", "7", importAt); err == nil {
...
func TestImportGenerationReleasesTheLockEvenWhenTheContextIsDone(t *testing.T) {
	...
	_, err := importClient(t, f).ImportGeneration(ctx, "ubuntu-2404-x64", raw, "2.336.0", "", "7", importAt)
	if err == nil {
...
func TestRecordVerificationTakesThePublishLock(t *testing.T) {
	...
	err := importClient(t, f).RecordVerification(t.Context(), "ubuntu-2404-x64",
		"g20260815033431", "vmlinux-6.1.155-ea1d42638d13", "7", false, false, importAt)
...
func TestRecordVerificationRefusesAGenerationThatIsGone(t *testing.T) {
	...
	err := importClient(t, f).RecordVerification(t.Context(), "ubuntu-2404-x64",
		"g20260815033431", "", "7", true, false, importAt)
	if err == nil {
...
func TestRecordVerificationRecordsTheContractBeforePublishingTheVerification(t *testing.T) {
	f := &importFake{snapshots: []string{"g20260815033431"}}

	if err := importClient(t, f).RecordVerification(t.Context(), "ubuntu-2404-x64",
		"g20260815033431", "", "7", true, false, importAt); err != nil {
		t.Fatalf("record verification: %v", err)
	}

	contractAt, verifiedAt := -1, -1
	for i, call := range f.calls {
		joined := strings.Join(call, " ")
		if strings.Contains(joined, GuestContractKey+".g20260815033431") {
			contractAt = i
		}
		if strings.Contains(joined, VerifiedKey+".g20260815033431") {
			verifiedAt = i
		}
	}

	if contractAt < 0 || verifiedAt < 0 {
		t.Fatalf("contract or verification metadata was not recorded; billet ran %v", f.calls)
	}
	if contractAt > verifiedAt {
		t.Errorf("the verification became visible before its compatibility contract: contract step %d, verification step %d",
			contractAt, verifiedAt)
	}
}
...
func TestImportGenerationWithdrawsASnapshotWhoseKernelCannotBeRecorded(t *testing.T) {
	...
	if _, err := importClient(t, f).ImportGeneration(t.Context(), "ubuntu-2404-x64", raw,
		"2.336.0", "vmlinux-6.1.155-ea1d42638d13", "7", importAt); err == nil {
...
// widenedWait is what the fake sets the client's command budget to at the instant
// a withdrawal issues its `snap rm`.
//
// NOTHING EVER WAITS ON IT. It exists only to be compared: a context the
// withdrawal constructs AFTER the removal returns carries a deadline about an hour
// further out than one constructed before it, so the two are told apart by
// arithmetic rather than by whether a loaded runner got there in time.
//
// A PLAIN "LATER DEADLINE" TEST IS NOT ENOUGH, which is why this is here rather
// than a bare After. Three contexts all created up front from a live base ALSO
// have strictly increasing deadlines -- microseconds apart -- so that assertion
// passes while every one of them would be nearly exhausted by the time a real
// `snap rm` had burned its own. An hour of separation cannot be produced that way,
// and it also puts the comparison far beyond any question of clock granularity.
const widenedWait = time.Hour

// A withdrawal runs after the caller's context has been cancelled and after its
// own `snap rm` has reported a timeout, and the confirmation and EVERY metadata
// removal must run on a bound taken from cleanupBase once the removal returned.
//
// Inheriting either dead context is not a slower cleanup but no cleanup: the
// command fails before rbd is started, leaving a snapshot whose absence was never
// confirmed or permanent metadata keyed to a generation that no longer exists.
//
// THE PROOF IS ARITHMETIC ON DEADLINES RATHER THAN A WALL-CLOCK WAIT, and that is
// the whole reason this test was rewritten: it was a CI flake. The fake used to
// WAIT the removal's deadline out, which forced c.wait small enough that the test
// did not sit on it -- and the confirmation and the metadata cleanup are bounded
// by that same c.wait, so one descheduling under -race -covermode=atomic expired
// a window they needed. One knob cannot be calibrated between a deadline that
// must expire and deadlines that must not.
//
// So the fixture SIMULATES the removal timeout rather than living through one: rbd
// reports the deadline it consumed, and what is asserted is the LINEAGE of the
// bounds that follow, which is the property the production code has to get right
// either way.
func TestImportGenerationConfirmsRemovalAfterCallerAndRemovalDeadlinesExpire(t *testing.T) {
	...
	// THE FIXTURE PROVES ITSELF FIRST. Everything below reads a difference the
	// widening is what creates, so a widening that never happened would report
	// every phase as badly ordered and name the wrong culprit.
	if !widened {
		t.Fatal("the fake never widened the command budget, so a bound taken before the " +
			"removal and one taken after it are indistinguishable and nothing below proves anything")
	}

	removeAt, ok := f.deadlineOf("snap", "rm")
	if !ok {
		t.Fatalf("the withdrawal ran no bounded `snap rm`, so there is no removal deadline for "+
			"the rest of it to be compared against; billet ran %v", f.calls)
	}

	// EVERY ATTEMPTED FIELD, NAMED. Asking for the first `image-meta remove` would
	// let a rollback take a fresh bound for the kernel and the removal's for the
	// runner version, which is a cleanup that stops halfway in production and looks
	// complete here.
	for _, step := range []struct {
		what      string
		fragments []string
	}{
		{"the absence check", []string{"snap", "ls", "ubuntu-2404-x64"}},
		{"the " + KernelKey + " removal", []string{"image-meta", "remove", KernelKey + "." + generation}},
		{
			"the " + RunnerVersionKey + " removal",
			[]string{"image-meta", "remove", RunnerVersionKey + "." + generation},
		},
	} {
		at, ok := f.deadlineOf(step.fragments...)
		if !ok {
			t.Errorf("%s never ran under a deadline of any kind; billet ran %v", step.what, f.calls)

			continue
		}

		if !at.After(removeAt.Add(widenedWait / 2)) {
			t.Errorf("%s ran on a bound taken no later than the removal's: its deadline is %s "+
				"past the removal's, where one constructed after the removal returned would be "+
				"close to %s past it", step.what, at.Sub(removeAt), widenedWait)
		}
	}
}
