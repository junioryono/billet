...
func runPastAHeldPipe(t *testing.T, script string) heldPipeResult {
	...
	//
	// IT IS STILL A TIMER, AND IT HAS TO BE, which is worth stating rather than
	// leaving for somebody to try to remove. What would be better is observing the
	// shell EXIT, and that cannot be done from here: the obvious probe is to have
	// the script write its own $$ and poll kill(pid, 0) for ESRCH, but a process
	// that has exited and not been reaped is a ZOMBIE and kill(0) succeeds on one.
	// Nothing here can reap it either — the reaper is exec.Cmd.Wait, which is
	// blocked on the very pipe this helper arranges for a descendant to hold. So
	// the exit is unobservable by construction and what is left is a bound on it.
	//
	// TWO SECONDS RATHER THAN 100ms, because what it costs and what it buys are
	// wildly asymmetric. If cancel() lands while the shell is still alive the
	// shell is KILLED, ExitCode() goes negative, and execRunner reports ctx.Err()
	// — which is the exact outcome the two callers assert must not happen, so the
	// test fails for the one reason it exists to rule out. Against that, the cost
	// of the larger number is nothing at all: it is a maximum, and the shell has
	// only to return from touch(1).
