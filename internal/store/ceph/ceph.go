...
func exitedWith(err error, code int) bool {
	// errors.As RATHER THAN errors.AsType, because exitCoder is a BEHAVIOURAL
	// interface — it has ExitCode and no Error — and AsType's type parameter is
	// constrained to error. Asking "does anything in this chain report an exit
	// code" is what this needs, and errors.As is the only one that can express it.
	var exitError exitCoder

	//nolint:forbidigo // exitCoder has no Error method, so errors.AsType cannot take it.
