...
func (mtlsStore) EndedLeaseNode(context.Context, string) (string, error) {
...
func TestAnAnonymousConnectionIsRefused(t *testing.T) {
	...
	// REFUSED IN THE HANDSHAKE, and that is now the whole point rather than an
	// implementation detail. This test used to assert a 401 from the handler,
	// which meant the connection had already completed a handshake and taken a
	// slot out of a budget shared with real nodes — collect enough of those and
	// no node is admitted at all. The two routes that must serve a machine with
	// no certificate are on their own listener, so nothing certless has business
	// here and the TLS layer says so first.
	//
	// ASSERTED STRUCTURALLY, not as "an error": MEASURED, the alert reaches the
	// caller inside a *net.OpError whose Op is "remote error" — crypto/tls's own
	// alert type is unexported, so errors.As against tls.AlertError is false.
	req, err := http.NewRequestWithContext(t.Context(), http.MethodGet,
		base+"/v1/nodes/n1/launched", http.NoBody)
	if err != nil {
		t.Fatalf("build: %v", err)
	}

	res, err := anonymous.Do(req)
	if err == nil {
		_ = res.Body.Close()

		t.Fatalf("a connection presenting no certificate was answered with %d; the handshake "+
			"should have ended it", res.StatusCode)
	}

	opErr, ok := errors.AsType[*net.OpError](err)
	if !ok || opErr.Op != "remote error" {
		t.Fatalf("the refusal was %v; want a TLS refusal from the server", err)
	}
}
