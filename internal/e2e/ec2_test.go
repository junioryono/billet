...
func newFakeCloud(t *testing.T) *fakeCloud {
	...
	f.Server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "DescribeImages":
			...
			// and this fixture described an image it correctly refuses.
...
func newCloudStack(t *testing.T) *cloudStack {
	...
	prov, err := ec2.New("e2e-deployment", config.EC2Config{
		...
		ec2.WithCredentials(awscreds.Static{AccessKeyID: "AKID", SecretAccessKey: "s"}))
	if err != nil {
...
func TestAJobReachesACloudInstance(t *testing.T) {
	...
	// on it. The reason that is worse than an over-commit is that Destroy is
	// reached while the guest is still working: a drain, a custody teardown, an
