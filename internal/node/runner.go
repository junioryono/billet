...
func (r *Runner) Launch(
	...
	if err != nil {
		// SAID HERE, ON THE NODE. Everything below is about custody, and the
		// reason the launch failed is carried away in the report — so an
		// operator watching the machine that actually tried to start the guest
		// saw only "a failed launch had in fact started something" with no
		// cause, while the plane logged a cancelled job. Two logs, neither
		// naming the fault. The backend's own error is the one thing that says
		// what to fix.
		r.log.Error("could not launch the compute for a job",
			"lease", lease.ID, "name", name, "error", err)
		...
		// THIS EXIT CARRIES THE SAME PROOF CUSTODY SETTLES ON, so it owes the same
		// cleanup — and it was the one path that skipped it.
		//
		// destroyStray confirms on an explicit TERMINAL RECORD without calling
		// Destroy at all, which is right (there is nothing to destroy) and is
		// exactly how a staged credential got orphaned: Destroy is where the
		// ordinary teardown removes it, custody's finish is where the reaper runs,
		// and this returns before either. It is the SECOND authorised caller, and
		// what authorises it is the same thing that authorises the first — proof
		// the compute is gone, which is what `confirmed` means here. For an ambiguous CodeBuild launch whose
		// build was already over by the time Find looked, the SecureString the
		// launch path deliberately preserved then stayed forever — and repeated fast
		// failures walk the account into its Parameter Store quota.
		r.reapStagedCredential(ctx, name)
...
//
// A REFUSAL IS NOT A REMOVAL, whatever it says about the lease. A "not found"
// from the control plane means the registration was not withdrawn, and
// destroying the guest on that would leave a runner GitHub can still route
// work to — so it is retried like any other failure. The wire performs the
// removal for a superseded process whose lease the ledger has ended, which is
// what keeps a drain from retrying this forever.
...
func (r *Runner) handleInterruption(ctx context.Context, notice *provider.InterruptionNotice) error {
	...
	if lease != nil {
		...
		entry := &custody{
			...
			observed:  true,
			since:     r.now(),
			// The interruption's own reason was just written above.
	...
	for _, entry := range r.custodySnapshot() {
		...
		entry.failureReason, entry.failureRecorded = reason, true
		if entry.phase != "" {
...
func (r *Runner) destroyStray(ctx context.Context, name string) (strayCleanup, error) {
	...
	// is the correct treatment of an unconfirmed teardown. It is also the right
	// answer for a create the backend has not yet listed — this is the path most
	// likely to be destroying an instance EC2 launched moments ago, so it is the
	// one whose "already gone" is least trustworthy.
	if state != provider.TeardownStopped {
		return strayCleanup{evidence: custodyEvidence{observed: true}},
...
func (r *Runner) Recover(ctx context.Context) error {
	...
	for _, inst := range instances {
		...
		// THE RELEASE WAITS FOR CONFIRMATION, exactly as Sweep's does.
		//
		...
		// registration reports, so a teardown AWS does not confirm holds this
		// capacity longer than one sweep interval, and a held teardown is bounded by
		// nothing.
		//
...
func (r *Runner) Sweep(ctx context.Context) error {
	...
	for _, inst := range instances {
		...
		// THE RELEASE IS SKIPPED UNLESS THE GUEST IS CONFIRMED GONE.
		//
