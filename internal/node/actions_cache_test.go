...
func TestActionsFinalizeRechecksThePointerAfterAConflict(t *testing.T) {
	...
	tests := []struct {
		...
		// Every path out of a conflict gives writer-1 back: the retry releases it
		// before acquiring writer-2, and a receipt completed from the pointer the
		// store shows never uses its lease again.
		wantReleased []string
	}{
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if finalized["ok"] != test.wantOK || storage.published != test.wantPublishes ||
				...
				!slices.Equal(storage.released, test.wantReleased) {
				t.Fatalf("finalization=%v published=%d current=%q acquisitions=%d writers=%v fences=%v released=%v",
					finalized, storage.published, storage.current, storage.writerAcquisitions,
					storage.publishedWriterIDs, storage.publishedFences, storage.released)
			}
		})
	}
}
