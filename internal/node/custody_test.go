...
func TestAConfiguredBoundDestroysAndRecordsAFailure(t *testing.T) {
	...
	// AND THE HISTORY SAYS BILLET DID IT, under the operator's bound: the one
	// disruption billet chooses rather than observes, which the attribution
	// report must be able to show.
	if got, err := a.HistoryFailureReason(t.Context(), lease.ID); err != nil ||
		got != alloc.HeldPastLimitReason {
		t.Errorf("a job billet killed carries reason %q err=%v, want %q", got, err,
			alloc.HeldPastLimitReason)
	}
}
