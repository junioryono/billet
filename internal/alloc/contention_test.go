...
func TestEveryReadOnlyOperationRunsWhileTheWriterSlotIsHeld(t *testing.T) {
	...
	// The control plane holds the ledger as it does for its whole life, and the
	// second handle is the operator command beside it. What "the same ledger"
	// means differs between the backends — a directory here, a DSN there — which
	// is why the pair comes from one helper.
	server, adminDB := openTestLedgerPair(t, true)
	...
	for _, tc := range []struct {
		...
		{"HistoryPlacement", func() error { _, err := a.HistoryPlacement(read, lease.ID); return err }},
