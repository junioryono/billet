...
func (a *Allocator) RevokeCert(ctx context.Context, serial, node, reason string) error {
	...
	return a.db.Tx(ctx, func(tx *sql.Tx) error {
		if err := state.WriteQueries(tx).RevokeCert(ctx, ledgerdb.RevokeCertParams{
			Serial:    serial,
			Node:      node,
			Reason:    reason,
			RevokedAt: ts(a.now().UTC()),
		}); err != nil {
...
func (a *Allocator) CertRevoked(ctx context.Context, serial string) (bool, error) {
	...
	err := a.db.View(ctx, func(tx querier) error {
		switch _, err := state.ReadQueries(tx).CertRevocation(ctx, serial); {
...
func (a *Allocator) RevokedCerts(ctx context.Context) ([]RevokedCert, error) {
	...
	err := a.db.View(ctx, func(tx querier) error {
		...
		rows, err := state.ReadQueries(tx).ListRevokedCerts(ctx)
		if err != nil {
			return fmt.Errorf("alloc: list revoked certificates: %w", err)
		}

		for _, row := range rows {
			out = append(out, RevokedCert{
				Serial:    row.Serial,
				Node:      row.Node,
				Reason:    row.Reason,
...
func (a *Allocator) RecordIssuedCert(ctx context.Context, c IssuedCert) error {
	...
	return a.db.Tx(ctx, func(tx *sql.Tx) error {
		return recordIssuedCertTx(ctx, state.WriteQueries(tx), c, ts(a.now().UTC()))
	})
}
...
func (a *Allocator) RecordRenewedCert(
	...
	return a.db.Tx(ctx, func(tx *sql.Tx) error {
		...
		switch _, err := q.CertRevocation(ctx, parent); {
		case errors.Is(err, sql.ErrNoRows):
		case err != nil:
			return fmt.Errorf("alloc: read the revocation list: %w", err)
		default:
		...
		switch cutoff, err := q.NodeRevocationCutoff(ctx, cert.Node); {
		case errors.Is(err, sql.ErrNoRows):
		case err != nil:
		...
		return recordIssuedCertTx(ctx, q, cert, ts(a.now().UTC()))
	})
}
...
func (a *Allocator) LiveCertsFor(ctx context.Context, node string) ([]IssuedCert, error) {
	...
	err := a.db.View(ctx, func(tx querier) error {
		...
		out, err = liveCertsForTx(ctx, state.ReadQueries(tx), node, ts(a.now().UTC()))
...
func liveCertsForTx(ctx context.Context, q state.ReadOps, node, now string) ([]IssuedCert, error) {
	rows, err := q.LiveCertsFor(ctx, ledgerdb.LiveCertsForParams{Node: node, Now: now})
	if err != nil {
...
// recordIssuedCertTx is the one place a credential is written down, so the two
// callers cannot drift on which columns a renewal records.
func recordIssuedCertTx(
	ctx context.Context, q state.WriteOps, c IssuedCert, now string,
) error {
	if err := q.RecordIssuedCert(ctx, ledgerdb.RecordIssuedCertParams{
		Serial:   c.Serial,
		Node:     c.Node,
		Source:   c.Source,
		NotAfter: c.NotAfter,
...
func (a *Allocator) RevokeNode(ctx context.Context, node, reason string) ([]IssuedCert, error) {
	...
	err := a.db.Tx(ctx, func(tx *sql.Tx) error {
		...
		live, err := liveCertsForTx(ctx, q, node, now)
		if err != nil {
			return err
		}

		for i := range live {
			if err := q.RevokeCert(ctx, ledgerdb.RevokeCertParams{
				Serial:    live[i].Serial,
				Node:      node,
				Reason:    reason,
				RevokedAt: now,
			}); err != nil {
		...
		if err := q.RecordNodeRevocation(ctx, ledgerdb.RecordNodeRevocationParams{
			Node:          node,
...
func (a *Allocator) CertRevokedFor(
	...
	err := a.db.View(ctx, func(tx querier) error {
		...
		switch _, err := q.CertRevocation(ctx, serial); {
		case errors.Is(err, sql.ErrNoRows):
		case err != nil:
			return fmt.Errorf("alloc: read the revocation list: %w", err)
		default:
		...
		cutoff, err := q.NodeRevocationCutoff(ctx, node)
