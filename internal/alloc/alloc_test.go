...
// Draining is how a host is emptied, and it must not disturb what is already on
// it.
//
// That distinction is the whole reason drain exists as a separate concept from
// removal. Fencing a lease can revoke escrow a listener has already promised to
// GitHub, which is a commitment no local decision may take back; refusing a new
// binding cannot, because a promised lease has not been bound yet. So draining
// only ever says no to work that has not started.
func TestDrainingAHostRefusesNewWorkAndKeepsTheOld(t *testing.T) {
	a := newAllocator(t,
		Limits{MaxVCPU: 16, MaxMemory: 64 * config.GiB},
		[]config.Tier{tier("small", 4, 16*config.GiB)})
	...
	// Already running when the drain starts.
	running, err := a.Reserve(ctx, "small")
	if err != nil {
		t.Fatalf("Reserve: %v", err)
	}

	if err := a.Assign(ctx, running.ID, running.Epoch, 101, 11); err != nil {
		t.Fatalf("Assign: %v", err)
	}

	if err := a.Bind(ctx, running.ID, running.Epoch, "epyc-1"); err != nil {
		t.Fatalf("Bind: %v", err)
	}

	// A second lease, reserved and assigned but not yet placed anywhere.
	pending, err := a.Reserve(ctx, "small")
	if err != nil {
		t.Fatalf("Reserve: %v", err)
	}

	if err := a.Assign(ctx, pending.ID, pending.Epoch, 102, 12); err != nil {
		t.Fatalf("Assign: %v", err)
	}

	if err := a.SetDrained(ctx, "epyc-1", true); err != nil {
		t.Fatalf("SetDrained: %v", err)
	}

	// NOT taken: the host is on its way out.
	if err := a.Bind(ctx, pending.ID, pending.Epoch, "epyc-1"); !errors.Is(err, ErrNodeDrained) {
		t.Errorf("a draining host accepted a new binding: %v", err)
	}

	// UNTOUCHED: the job already there runs to completion. Fencing it would be
	// the destructive act drain exists to avoid.
	if err := a.Advance(ctx, running.ID, running.Epoch, PhaseLaunching); err != nil {
		t.Errorf("draining a host disturbed the work already on it: %v", err)
	}

	if err := a.Heartbeat(ctx, running.ID, running.Epoch); err != nil {
		t.Errorf("draining a host stopped its existing lease from heartbeating: %v", err)
	}

	// And it is reversible, because an operator who drains by mistake needs a way
	// back that does not involve re-registering the host.
	if err := a.SetDrained(ctx, "epyc-1", false); err != nil {
		t.Fatalf("SetDrained back: %v", err)
	}

	if err := a.Bind(ctx, pending.ID, pending.Epoch, "epyc-1"); err != nil {
		t.Errorf("a host that stopped draining still refuses work: %v", err)
	}
}

// A tier pinned to a draining host reports no headroom, which is what actually
// empties the host: the listener advertises zero and GitHub sends nothing.
func TestATierPinnedToADrainingHostHasNoHeadroom(t *testing.T) {
	mac := config.Tier{
	...
	a := newAllocator(t, Limits{MaxVCPU: 16, MaxMemory: 64 * config.GiB}, []config.Tier{mac})

	ctx := t.Context()

	registerNode(t, a, "mac-mini-1", config.ProviderTart)

	if got := headroom(t, a, "mac"); got == 0 {
		t.Fatal("the tier had no headroom before the drain; this proves nothing")
	}

	if err := a.SetDrained(ctx, "mac-mini-1", true); err != nil {
		t.Fatalf("SetDrained: %v", err)
	}

	if got := headroom(t, a, "mac"); got != 0 {
		t.Errorf("headroom is %d for a tier pinned to a draining host; the listener would "+
			"advertise capacity that can never be placed", got)
	}

	// Escrow follows headroom, so nothing is taken from the global budget for a
	// host that cannot run it.
	leases, err := a.Escrow(ctx, "mac", 2)
	if err != nil {
		t.Fatalf("Escrow: %v", err)
	}

	if len(leases) != 0 {
		t.Errorf("escrowed %d leases for a tier pinned to a draining host", len(leases))
	}
}

// An unpinned tier is unaffected by one host draining: its work can go anywhere.
func TestDrainingOneHostDoesNotStarveUnpinnedTiers(t *testing.T) {
	a := newAllocator(t,
		Limits{MaxVCPU: 16, MaxMemory: 64 * config.GiB},
		[]config.Tier{tier("small", 4, 16*config.GiB)})
	...
	if got := headroom(t, a, "small"); got == 0 {
		t.Error("draining one host removed the headroom of a tier that is not pinned to it")
	}
	...
	if err := a.Bind(ctx, lease.ID, lease.Epoch, "epyc-2"); err != nil {
		t.Errorf("the healthy host refused work while a different host was draining: %v", err)
	}
}

// Draining a host nobody registered is a typo worth hearing about.
func TestDrainingAnUnknownHostIsAnError(t *testing.T) {
	a := newAllocator(t,
		Limits{MaxVCPU: 16, MaxMemory: 64 * config.GiB},
		[]config.Tier{tier("small", 4, 16*config.GiB)})

	if err := a.SetDrained(t.Context(), "never-registered", true); !errors.Is(err, ErrNodeNotFound) {
		t.Errorf("SetDrained on an unknown host returned %v, want ErrNodeNotFound", err)
	}
}
