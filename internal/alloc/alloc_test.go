...
// TestActiveRunnerLeasesExcludeDeregisteredNotStillRegisteredCleanup pins the
// count that drives the assignment deficit and pool scale. A teardown lease
// whose runner is still registered (deregistration not recorded) must keep
// counting, or a replacement double-schedules the live runner; the same lease
// once MarkDeregistered records its removal must stop counting, or its lingering
// compute-destroy over-counts and drops a freshly acquired job.
func TestActiveRunnerLeasesExcludeDeregisteredNotStillRegisteredCleanup(t *testing.T) {
	tiers := []config.Tier{tier("linux", 1, config.GiB)}
	a := newAllocator(t, Limits{MaxVCPU: 16, MaxMemory: 16 * config.GiB}, tiers)
	label := tiers[0].Label

	count := func() int {
		t.Helper()
		n, err := a.ActiveRunnerLeases(t.Context(), label)
		if err != nil {
			t.Fatalf("ActiveRunnerLeases: %v", err)
		}
		return n
	}
	var seq int64
	teardownLease := func() *Lease {
		t.Helper()
		seq++
		l := reserve(t, a, label)
		if err := a.Assign(t.Context(), l.ID, l.Epoch, 100+seq, seq); err != nil {
			t.Fatalf("Assign: %v", err)
		}
		if err := a.Bind(t.Context(), l.ID, l.Epoch, "test-host-firecracker"); err != nil {
			t.Fatalf("Bind: %v", err)
		}
		for _, p := range []Phase{PhaseLaunching, PhaseTeardown} {
			if err := a.Advance(t.Context(), l.ID, l.Epoch, p); err != nil {
	...
	// A teardown lease whose runner is still registered counts.
	l := teardownLease()
	if n := count(); n != 1 {
		t.Fatalf("still-registered teardown lease = %d; want 1", n)
	}

	// Recording its deregistration drops it from the count.
	if err := a.MarkDeregistered(t.Context(), l.ID); err != nil {
		t.Fatalf("MarkDeregistered: %v", err)
	}
	if n := count(); n != 0 {
		t.Fatalf("deregistered teardown lease = %d; want 0", n)
	}

	// A second still-registered teardown lease still counts: the exclusion is the
	// deregistration flag, not the teardown phase.
	teardownLease()
	if n := count(); n != 1 {
		t.Fatalf("second still-registered teardown lease = %d; want 1", n)
	}
}

// TestMarkDeregisteredSurvivesAReapAndIsIdempotent proves the mark is NOT
// epoch-fenced: a reap quarantines a compute-bearing lease and bumps its epoch
// between RemoveRunner and the mark, and the mark must still land — otherwise the
// gone runner's quarantine lease stays counted forever and starves the pool. It
// is also monotonic and a no-op on a missing lease.
func TestMarkDeregisteredSurvivesAReapAndIsIdempotent(t *testing.T) {
	now := time.Date(2026, time.August, 22, 12, 0, 0, 0, time.UTC)
	clock := func() time.Time { return now }
	tiers := []config.Tier{tier("linux", 1, config.GiB)}
	a := newAllocator(t, Limits{MaxVCPU: 4, MaxMemory: 4 * config.GiB}, tiers,
		WithClock(func() time.Time { return clock() }), WithLeaseTTL(30*time.Second))
	label := tiers[0].Label

	l := reserve(t, a, label)
	if err := a.Assign(t.Context(), l.ID, l.Epoch, 101, 11); err != nil {
		t.Fatalf("Assign: %v", err)
	}
	if err := a.Bind(t.Context(), l.ID, l.Epoch, "test-host-firecracker"); err != nil {
		t.Fatalf("Bind: %v", err)
	}
	if err := a.Advance(t.Context(), l.ID, l.Epoch, PhaseLaunching); err != nil {
		t.Fatalf("Advance: %v", err)
	}

	// The lease expires and the reaper quarantines it (compute-bearing, so not
	// terminalized), bumping its epoch out from under the caller's stale copy.
	// Reap's return counts terminalized leases, not quarantined ones, so the
	// proof that it quarantined rather than dropped this one is that it is still
	// counted below.
	now = now.Add(90 * time.Second)
	if _, err := a.Reap(t.Context()); err != nil {
		t.Fatalf("Reap: %v", err)
	}
	if active, err := a.ActiveRunnerLeases(t.Context(), label); err != nil || active != 1 {
		t.Fatalf("quarantined lease before mark = %d, err %v; want 1", active, err)
	}

	// The unfenced mark lands despite the epoch change, and repeats harmlessly.
	if err := a.MarkDeregistered(t.Context(), l.ID); err != nil {
		t.Fatalf("MarkDeregistered after reap: %v", err)
	}
	if err := a.MarkDeregistered(t.Context(), l.ID); err != nil {
		t.Fatalf("MarkDeregistered again (idempotent): %v", err)
	}
	if active, err := a.ActiveRunnerLeases(t.Context(), label); err != nil || active != 0 {
		t.Fatalf("ActiveRunnerLeases after mark = %d, err %v; want 0", active, err)
	}

	// A missing lease is a harmless no-op, not an error.
	if err := a.MarkDeregistered(t.Context(), "no-such-lease"); err != nil {
		t.Fatalf("MarkDeregistered on a missing lease = %v; want nil", err)
	}
}
...
// AND THE SAME LIMIT UNDER CONCURRENCY, which the sequential test above cannot
// show. Apple's cap is enforced from a COUNT of the host's open macOS leases,
// and a count taken outside the reservation transaction is a read followed by a
// hopeful write — the defect TestConcurrentReservationsNeverOvercommit exists
// for, one dimension along and with a worse consequence: vCPU overcommit slows
// a machine down, while a third concurrent macOS guest is REFUSED BY THE HOST
// ("The number of VMs exceeds the system limit"), so the job dies at launch.
//
// Two tiers racing on one Mac is the shape that produces it. Neither tier alone
// exceeds the licence and both together do, so a per-tier cap would pass this
// while the machine is oversubscribed.
func TestConcurrentMacOSReservationsNeverExceedApplesLimit(t *testing.T) {
	six := config.Tier{
		Label: "mac-6", Provider: config.ProviderTart, GuestOS: config.GuestMacOS,
		Node: "mac-mini-1", VCPU: 6, Memory: 24 * config.GiB, Image: "macos-26",
	}
	...
	// DELIBERATELY ENORMOUS, so the LICENCE is the only thing that can bind. At
	// 256 vCPU the host has room for dozens of either tier, and a test that
	// passed because vCPU ran out first would prove nothing about Apple's cap.
	a := newAllocator(t,
		Limits{MaxVCPU: 256, MaxMemory: 512 * config.GiB},
		[]config.Tier{six, twelve})
	...
	if n := headroom(t, a, "mac-6"); n != config.DefaultMacOSVMLimit {
		t.Fatalf("mac-6 Headroom = %d, want %d: the licence is not what binds here, so this "+
			"test would be measuring the wrong ceiling", n, config.DefaultMacOSVMLimit)
	}
	...
	for i := range attempts {
		...
		go func() {
			...
			// BOTH TIERS AT ONCE, because the licence belongs to the Mac and not
			// to either of them. Racing one tier against itself would be caught
			// by a per-tier cap that leaves the host oversubscribed.
			label := "mac-6"
			if i%2 == 1 {
				label = "mac-12"
			}

			switch _, err := a.Reserve(ctx, label); {
			case err == nil:
				granted.Add(1)
			case errors.Is(err, ErrNoCapacity):
				refused.Add(1)
	...
	if g := granted.Load(); g != int32(config.DefaultMacOSVMLimit) {
		t.Fatalf("granted %d concurrent macOS guests on one Mac, want exactly %d — the host "+
			"would refuse the surplus at launch and those jobs would die", g,
			config.DefaultMacOSVMLimit)
	}

	if r := refused.Load(); r != attempts-int32(config.DefaultMacOSVMLimit) {
		t.Errorf("refused %d, want %d", r, attempts-config.DefaultMacOSVMLimit)
	}

	// AND THE DURABLE STATE AGREES. The counters above are what the callers were
	// told; this is what the ledger holds, and a grant that raced its way past
	// the check would show up here even if the caller was told no.
	for _, label := range []string{"mac-6", "mac-12"} {
		if n := headroom(t, a, label); n != 0 {
			t.Errorf("%s Headroom = %d after the licence was filled, want 0", label, n)
		}
	}
}
...
func TestReapFencesTheOldHolder(t *testing.T) {
	...
	if err := a.db.Reader().QueryRowContext(ctx,
		`SELECT epoch FROM leases WHERE id = $1`, lease.ID).Scan(&stored); err != nil {
...
func TestReapedLeaseKeepsItsAttribution(t *testing.T) {
	...
	if err := a.db.Reader().QueryRowContext(ctx,
		`SELECT run_id, request_id, conclusion, assigned_at FROM job_history WHERE lease_id = $1`, lease.ID).
...
func TestReleaseArchivesToHistory(t *testing.T) {
	...
	err := a.db.Reader().QueryRowContext(ctx,
		`SELECT tier, run_id, conclusion FROM job_history WHERE lease_id = $1`, lease.ID).
...
func TestUnverifiableLegacyLeaseFailsClosed(t *testing.T) {
	...
	if err := a.db.Tx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, `UPDATE leases SET providers = '' WHERE id = $1`, lease.ID)
...
func TestOnlineRequiresABoundNode(t *testing.T) {
	...
	if err := a.db.Tx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx,
			`UPDATE leases SET phase = 'launching', node = NULL WHERE id = $1`, lease.ID)
...
func TestEveryRunningPhaseRejectsAnUnboundLease(t *testing.T) {
	for _, phase := range runningPhases {
		t.Run(string(phase), func(t *testing.T) {
			...
			if err := a.db.Tx(ctx, func(tx *sql.Tx) error {
				_, err := tx.ExecContext(ctx,
					`UPDATE leases SET phase = $1, node = NULL WHERE id = $2`, from, lease.ID)
...
func TestOrphanAdviceDependsOnTheCurrentPhase(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := a.db.Tx(ctx, func(tx *sql.Tx) error {
				_, err := tx.ExecContext(ctx,
					`UPDATE leases SET phase = $1, node = NULL WHERE id = $2`, string(tc.seed), lease.ID)
...
func TestSamePhaseAdvanceStillChecksPlacement(t *testing.T) {
	...
	if err := a.db.Tx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx,
			`UPDATE leases SET phase = 'launching', node = 'epyc-1', providers = '' WHERE id = $1`,
			lease.ID)
...
func TestBindRefusesToAdoptAnAlreadyRunningLease(t *testing.T) {
	...
	if err := a.db.Tx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx,
			`UPDATE leases SET phase = 'online', node = NULL WHERE id = $1`, lease.ID)
...
func TestBindRefusesANodeRunningAnotherProvider(t *testing.T) {
	...
	if err := a.db.Tx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx,
			`UPDATE nodes SET provider = $1 WHERE name = $2`, config.ProviderTart, "shapeshifter")
...
func TestHistoryAttributesALeaseToTheMachineEscrowChose(t *testing.T) {
	...
	if err := a.db.Reader().QueryRowContext(t.Context(),
		`SELECT node FROM job_history WHERE lease_id = $1`, lease.ID).Scan(&node); err != nil {
...
// REAPING A HELD LEASE INTO QUARANTINE KEEPS THE AGE OF THE OBLIGATION.
//
// held_at is when somebody became responsible for proving compute is gone, and
// it is what `billet leases` and `billet status` age a proof obligation from. A
// lease that is already in teardown -- held_at set -- and then expires is reaped
// into quarantine, and resetting the clock there would report an obligation that
// has been outstanding for hours as brand new, which is the one number an
// operator uses to decide whether to intervene.
//
// Measured: dropping the reclaim CASE's empty-string test on held_at left every other
// test in this package green.
func TestReapingAHeldLeaseKeepsTheAgeOfItsObligation(t *testing.T) {
	now := time.Now().UTC()
	...
	if len(held) != 1 || held[0].Since == "" {
		t.Fatalf("the teardown recorded no held_at: %+v", held)
	}

	original := held[0].Since

	// TIME PASSES AND THE HOLDER STOPS ANSWERING, so the reaper quarantines it.
	...
	after, err := a.Held(t.Context())
	if err != nil {
		t.Fatalf("Held after the reap: %v", err)
	}

	if len(after) != 1 {
		t.Fatalf("the reaped lease is no longer held: %+v", after)
	}

	if after[0].State != PhaseQuarantine {
		t.Fatalf("the reaped lease is %s, want quarantine", after[0].State)
	}

	if after[0].Since != original {
		t.Errorf("the obligation's age was reset from %s to %s; an operator reads that "+
			"as a problem that just started", original, after[0].Since)
	}
}

// ARCHIVING KEEPS A WORKFLOW RUN THE COMPLETION SUPPLIED.
//
// A pooled runner is launched before GitHub chooses its job, so its LEASE records
// run 0 for ever; the run an operator needs arrives on the completion and is
// written straight into job_history. Archiving that lease then presents run 0
// again, and without the COALESCE the archive would erase the only record of
// which workflow actually ran -- in the report whose whole purpose is telling
// somebody which of their builds billet disrupted.
//
// Measured: replacing the archive's COALESCEs with plain assignments left every
// other test in this package green.
func TestArchivingKeepsAWorkflowRunTheCompletionSupplied(t *testing.T) {
	now := time.Now().UTC()
	a := quarantineFleet(t, &now)
	...
	// RUN 0, THE WAY A POOL SLOT IS ASSIGNED: nothing knew the job yet. Walked by
	// hand rather than through advanceTo, which assigns run 1 and would hide the
	// case this test exists for.
	if err := a.Assign(t.Context(), fresh.ID, fresh.Epoch, 0, 77); err != nil {
		t.Fatalf("Assign: %v", err)
	}

	for _, step := range []Phase{PhaseLaunching, PhaseOnline, PhaseBusy} {
		if err := a.Advance(t.Context(), fresh.ID, fresh.Epoch, step); err != nil {
	...
	fresh, err = a.Lease(t.Context(), lease.ID)
	if err != nil {
		t.Fatalf("Lease after assign: %v", err)
	}

	// THE DISRUPTION IS RECORDED FIRST, because the guard refuses one once a
	// result exists.
	if err := a.MarkFailure(t.Context(), fresh.ID, fresh.Epoch, "ec2 spot interruption"); err != nil {
		t.Fatalf("MarkFailure: %v", err)
	}

	// AND THE COMPLETION SUPPLIES THE RUN.
	if err := a.RecordJobResult(t.Context(), fresh.ID, "failed", 4242); err != nil {
		t.Fatalf("RecordJobResult: %v", err)
	}

	if err := a.Release(t.Context(), fresh.ID, fresh.Epoch, PhaseFailed); err != nil {
		t.Fatalf("Release: %v", err)
	}
	...
	if failures[0].RunID != 4242 {
		t.Errorf("run = %d, want 4242; archiving the lease erased the run the "+
			"completion supplied", failures[0].RunID)
	}
}

// AND ASKING FOR ONE LEASE'S OUTCOME WHILE IT IS STILL RUNNING IS ErrLeaseNotFound.
//
// The history row is opened at ASSIGNMENT and the conclusion is written only when
// the lease terminalizes, so a job in flight has a row and no verdict --
// indistinguishable, to this function's caller, from a lease that was never
// archived. Both are ErrLeaseNotFound.
//
// THE CONVERSION CHANGED WHAT SILENCE LOOKS LIKE HERE, which is why this exists.
// The hand-written scan took a NULL conclusion into a plain string and FAILED on
// it -- loudly and unhelpfully. Generated code reads a sql.NullString, so the
// same row would otherwise arrive as "" beside a nil error, and nothing would
// separate an unfinished job from an empty verdict.
func TestARunningLeaseHasNoHistoryOutcome(t *testing.T) {
	now := time.Now().UTC()
	a := quarantineFleet(t, &now)

	lease := busyLease(t, a)

	// THE ROW HAS TO BE PROVED PRESENT FIRST, or this test passes for the wrong
	// reason: with no history row at all HistoryOutcome answers ErrLeaseNotFound
	// too, and the release below would then create one and satisfy the rest.
	// failure_reason is NOT NULL, so it comes back empty rather than absent for a
	// row that exists and has no verdict -- which is exactly the state under test.
	if _, err := a.HistoryFailureReason(t.Context(), lease.ID); err != nil {
		t.Fatalf("the assignment opened no history row, so the case this test is "+
			"about is not reachable from this fixture: %v", err)
	}

	if _, err := a.HistoryOutcome(t.Context(), lease.ID); !errors.Is(err, ErrLeaseNotFound) {
		t.Fatalf("HistoryOutcome for a running lease: %v, want ErrLeaseNotFound", err)
	}
	...
	if err != nil {
		t.Fatalf("HistoryOutcome after the release: %v", err)
	}

	if outcome != string(PhaseDone) {
		t.Errorf("outcome = %q, want %q", outcome, PhaseDone)
	}
}

// A LEASE STILL IN FLIGHT IS NOT A HISTORY OUTCOME.
//
// A job can have more than one lease across a restart -- GitHub redelivers an
// unacknowledged assignment and the listener escrows a fresh one -- so the
// listener reads every archived outcome for a request to decide what to do. A row
// exists from ASSIGNMENT with no conclusion, and admitting one would put an empty
// verdict beside the real ones.
//
// THE CONVERSION MADE THIS SHARPER RATHER THAN SAFER: the hand-written scan failed
// outright on a NULL conclusion, so the filter was partly a convenience; generated
// code reads it as a sql.NullString, so the same row now arrives silently as "".
// Measured: dropping `conclusion IS NOT NULL` left every other test in this
// package green.
func TestALeaseStillInFlightIsNotAHistoryOutcome(t *testing.T) {
	now := time.Now().UTC()
	a := quarantineFleet(t, &now)

	// busyLease ASSIGNS RUN 1 REQUEST 1, so two of them are two leases for one
	// request -- which is the redelivery case this is about.
	const request = 1

	first := busyLease(t, a)
	if err := a.Release(t.Context(), first.ID, first.Epoch, PhaseDone); err != nil {
		t.Fatalf("Release the first lease: %v", err)
	}

	// A SECOND LEASE FOR THE SAME REQUEST, still running.
	second := busyLease(t, a)

	// AND IT HAS A ROW WITH NO VERDICT, which is the state being excluded below.
	// Without this the count could be right because the second lease left no row
	// at all, which is a different situation and not the one under test.
	if _, err := a.HistoryFailureReason(t.Context(), second.ID); err != nil {
		t.Fatalf("the second lease opened no history row: %v", err)
	}

	if _, err := a.HistoryOutcome(t.Context(), second.ID); !errors.Is(err, ErrLeaseNotFound) {
		t.Fatalf("the second lease already has an outcome: %v", err)
	}

	outcomes, err := a.HistoryOutcomesForRequest(t.Context(), request)
	if err != nil {
		t.Fatalf("HistoryOutcomesForRequest: %v", err)
	}

	if len(outcomes) != 1 || outcomes[0] != string(PhaseDone) {
		t.Fatalf("outcomes = %q, want exactly one %q; a lease still in flight was "+
			"reported as an outcome", outcomes, PhaseDone)
	}
}
