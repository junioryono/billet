...
func nodeEpoch(t *testing.T, a *Allocator) int64 {
	...
	if err := a.db.Reader().QueryRowContext(t.Context(),
		`SELECT epoch FROM nodes WHERE name = $1`, name).Scan(&epoch); err != nil {
...
// A COMPLETION CORRECTS INVENTORY'S PROVISIONAL VERDICT AND SETTLES NOTHING
// ITSELF. Arriving first, it leaves the quarantined lease exactly as it found
// it — charged, open, waiting for a fresh observation — because the only thing
// it could settle from is a snapshot with no ordering against the quarantine.
// Arriving after the host's inventory settled the lease, it replaces the
// provisional failure with GitHub's own outcome.
func TestCompletionAbsenceKeepsItsOutcomeAcrossInventoryOrdering(t *testing.T) {
	for _, inventoryFirst := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if !inventoryFirst {
				settled, err := a.SettleCompletionOnTerminalLease(
					t.Context(), lease.ID, lease.Epoch, PhaseDone)
				if err != nil {
					t.Fatalf("SettleCompletionOnTerminalLease: %v", err)
				}
				if settled {
					t.Fatal("a completion settled a quarantined lease nothing had observed absent")
				}
				if _, err := a.Lease(t.Context(), lease.ID); err != nil {
					t.Fatalf("the completion changed the open lease: %v", err)
				}
			}
			...
			var reason string
			if err := a.db.Reader().QueryRowContext(t.Context(),
				`SELECT failure_reason FROM job_history WHERE lease_id = $1`, lease.ID).
				Scan(&reason); err != nil {
			...
			if err := a.db.Reader().QueryRowContext(t.Context(),
				`SELECT phase, failure_reason FROM leases WHERE id = $1`, lease.ID).
...
// AN ABSENT LEASE WHOSE RESULT GITHUB ALREADY REPORTED SETTLES AS DONE.
//
// The listener records a completion's result before it asks anything to
// destroy the compute, so a teardown holder that died with the completion
// delivered leaves a lease that carries the result. Inventory then finds the
// guest gone: that is a job that finished and a lifecycle that ended tidily,
// not a provisional failure for a later completion to correct — no completion
// is coming, because it already came. A lease with no result keeps the
// provisional verdict, and a reason something else recorded stands.
func TestAnAbsentLeaseWithARecordedResultSettlesAsDone(t *testing.T) {
	cases := []struct {
		name       string
		result     string
		reason     string
		...
		{"a result GitHub reported", "succeeded", "", PhaseDone, "", ""},
		{"no result yet", "", "", PhaseFailed, inventoryAbsenceFailureReason, string(DisruptionGuestAbsent)},
		{"an earlier reason", "failed", "ec2 spot interruption: terminate", PhaseFailed,
			"ec2 spot interruption: terminate", string(DisruptionReclaimed)},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.result != "" {
				if err := a.RecordJobResult(t.Context(), lease.ID, tc.result, 1); err != nil {
			...
			now = now.Add(2 * quarantineGrace)
			if freed, err := a.ResolveQuarantineFor(t.Context(), "epyc-1", nil, nodeEpoch(t, a)); err != nil || freed != 1 {
				t.Fatalf("inventory reconciliation: freed=%d err=%v, want 1", freed, err)
			}

			if outcome, err := a.HistoryOutcome(t.Context(), lease.ID); err != nil || outcome != string(tc.wantPhase) {
				t.Fatalf("settled as %q err=%v, want %s", outcome, err, tc.wantPhase)
			}
			if reason, err := a.HistoryFailureReason(t.Context(), lease.ID); err != nil || reason != tc.wantReason {
				t.Fatalf("history reason = %q err=%v, want %q", reason, err, tc.wantReason)
			}
			if token, _ := historyDisruption(t, a, lease.ID); token != tc.wantToken {
				t.Fatalf("archived disruption = %q, want %q", token, tc.wantToken)
			}

			// AND THE COMPLETION'S RETRY FINDS THE LEASE SETTLED: with nothing to
			// correct where GitHub's word was already in, and the provisional
			// verdict corrected where it was not.
			settled, err := a.SettleCompletionOnTerminalLease(t.Context(), lease.ID, lease.Epoch, PhaseDone)
			if err != nil || !settled {
				t.Fatalf("SettleCompletionOnTerminalLease = %v err=%v, want settled", settled, err)
			}
			want := tc.wantPhase
			if tc.wantReason == inventoryAbsenceFailureReason {
				want = PhaseDone
			}
			if outcome, err := a.HistoryOutcome(t.Context(), lease.ID); err != nil || outcome != string(want) {
				t.Fatalf("after the completion the history says %q err=%v, want %s", outcome, err, want)
			}
		})
	}
}
...
// WHAT A HOST LAST SAID IS RECORDED UNDER THE FENCE THAT ADMITTED IT.
//
// The record exists so `billet status` can show an operator the hosts' own last
// word about what they are running — evidence the ledger cannot supply. It is
// written inside the transaction that has just proved the reporting epoch
// current, because an observation whose fence was checked separately is the
// defect this file exists to prevent.
func TestWhatAHostReportsRunningIsRecordedAgainstItsEpoch(t *testing.T) {
	t.Parallel()

	a := newAllocator(t, Limits{MaxVCPU: 64, MaxMemory: 128 * config.GiB},
		[]config.Tier{tier("inv-tier", 2, 4*config.GiB)})

	epoch, err := a.RegisterNode(t.Context(), NodeRegistration{
		Name: "inv-host", Provider: config.ProviderDocker,
	...
	if _, err := a.ResolveQuarantineFor(t.Context(), "inv-host", []string{"a", "b"}, epoch); err != nil {
		t.Fatalf("ResolveQuarantineFor: %v", err)
	}

	got := inventoryOf(t, a, "inv-host")

	if got.Report == nil {
		t.Fatal("no report recorded, though it came from the registration in force")
	}
	if got.Report.ReportedRunning != 2 {
		t.Errorf("the host is recorded running %d, want the 2 it reported",
			got.Report.ReportedRunning)
	}
	if got.Report.ReceivedAt == "" {
		t.Error("no receipt time was recorded")
	}

	// A REPORT FENCED OUT CHANGES NOTHING. An overtaken registration's inventory
	// must not overwrite what the current one vouched for.
	if _, err := a.ResolveQuarantineFor(t.Context(), "inv-host", nil, epoch-1); err != nil {
		t.Fatalf("stale ResolveQuarantineFor: %v", err)
	}

	if got := inventoryOf(t, a, "inv-host"); got.Report == nil || got.Report.ReportedRunning != 2 {
		t.Errorf("a report from a superseded registration overwrote the current one: %+v",
			got.Report)
	}
}

// RE-REGISTERING MAKES THE OLD REPORT STALE RATHER THAN WRONG.
//
// "Has not reported since it reconnected" and "reported nothing running" are
// opposite facts, and a host that just came back has said nothing yet.
func TestAHostThatReconnectedHasNotReportedYet(t *testing.T) {
	t.Parallel()

	a := newAllocator(t, Limits{MaxVCPU: 64, MaxMemory: 128 * config.GiB},
		[]config.Tier{tier("inv-tier2", 2, 4*config.GiB)})

	reg := NodeRegistration{
		Name: "inv-host2", Provider: config.ProviderDocker,
		VCPU: 8, Memory: 32 * config.GiB,
	}
	...
	if _, err := a.ResolveQuarantineFor(t.Context(), "inv-host2", nil, epoch); err != nil {
		t.Fatalf("ResolveQuarantineFor: %v", err)
	}

	if got := inventoryOf(t, a, "inv-host2"); got.Report == nil || got.Report.ReportedRunning != 0 {
		t.Fatalf("the fixture did not record an empty current report: %+v", got.Report)
	}
	...
	got := inventoryOf(t, a, "inv-host2")

	if got.Report != nil {
		t.Errorf("a report from before the host reconnected is still offered as this host's "+
			"word; that reads as 'it says it is running nothing' when it has said "+
			"nothing: %+v", got.Report)
	}
}

// A CONTROL PLANE THAT JUST STARTED HAS NO REPORTS.
//
// The node epoch does not move when the plane restarts, so without clearing
// these the record would survive with a matching epoch and read as current --
// a report received by a process that is gone, describing a host this one has
// not spoken to.
func TestAPlaneThatForgetsTheFleetForgetsWhatItReported(t *testing.T) {
	t.Parallel()

	a := newAllocator(t, Limits{MaxVCPU: 64, MaxMemory: 128 * config.GiB},
		[]config.Tier{tier("inv-tier3", 2, 4*config.GiB)})

	epoch, err := a.RegisterNode(t.Context(), NodeRegistration{
		Name: "inv-host3", Provider: config.ProviderDocker,
		VCPU: 8, Memory: 32 * config.GiB,
	})
	if err != nil {
		t.Fatalf("RegisterNode: %v", err)
	}
	if _, err := a.ResolveQuarantineFor(t.Context(), "inv-host3", nil, epoch); err != nil {
		t.Fatalf("ResolveQuarantineFor: %v", err)
	}
	...
	got := inventoryOf(t, a, "inv-host3")

	if got.Report != nil {
		t.Errorf("a report from before this plane started survived as current: %+v", got.Report)
	}
}

func inventoryOf(t *testing.T, a *Allocator, node string) NodeInventory {
	t.Helper()

	all, err := a.NodeInventories(t.Context())
	if err != nil {
		t.Fatalf("NodeInventories: %v", err)
	}
	...
	t.Fatalf("no inventory row for %s in %+v", node, all)
