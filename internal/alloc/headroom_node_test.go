...
//
// THROUGH THE PLACER, BECAUSE THAT IS WHAT PRODUCTION ASKS. This helper used to
// call headroomOn, a second per-host implementation that no production path ever
// reached — so the four tests below asserted real invariants against code
// nothing ran, and deleting the LIVE macOS clamp left every one of them green.
// A test that cannot fail when the shipped code is wrong is decoration, however
// exactly it describes the property.
func roomOn(t *testing.T, a *Allocator, node nodeRow, tier config.Tier) int {
	...
	if err := a.db.Tx(t.Context(), func(tx *sql.Tx) error {
		// headroomWithPlacer rather than placerWithFloors: it is what Reserve
		// calls, and it sets the deployment terms roomFor also clamps by. Taking
		// the placer without them would leave those at their sentinel maximums
		// and measure a bound production never applies.
		_, place, err := a.headroomWithPlacer(t.Context(), tx, tier)
		if err != nil {
			return err
		}

		n = place.roomFor(node.name, tier)

		return nil
	}); err != nil {
		t.Fatalf("roomOn: %v", err)
	}
...
//
// A single-backend macOS tier is always pinned — alloc.New refuses one that
// names no node — so the host this counts against is never in doubt; a tier
// spanning several backends is counted per host at placement instead
// (macos_unpinned_test.go). What matters here is that the count
