...
// real build from a real moment. This asserts the age notice reads it both ways, so
// the fixture going out of date cannot quietly turn the check into a no-op.
//
// AND AGE IS ALL IT ASSERTS. There used to be a Stale() here that REFUSED an import
// at built_at + 30 days; a published manifest names a runner VERSION, and whether
// github still accepts that version is settled by the release history rather than
// by how long ago somebody ran a build.
func TestTheRealManifestExercisesTheAgeNoticeBothWays(t *testing.T) {
	data, err := os.ReadFile("testdata/published-manifest.json")
	if err != nil {
		t.Fatalf("read: %v", err)
	}
	...
	if m.Aging(m.BuiltAt.Add(time.Hour)) {
		t.Error("an hour after it was built, the image was called old")
	}

	if !m.Aging(m.BuiltAt.Add(45 * 24 * time.Hour)) {
		t.Error("forty-five days after it was built, the image was not called old")
	}
}
