...
func TestParseManifestRefusesASchemaItDoesNotKnow(t *testing.T) {
	// NOT `SchemaVersion + 1`. This build deliberately reads one schema further
	// than it writes, so deriving the unknown one from the written version made
	// this test refuse exactly the layout the reader had just learned.
	for _, schema := range []int{0, SchemaV2 + 1, -1, 99} {
		m := validManifest()
		m.Schema = schema
...
// TestTheReaderIsAheadOfTheWriter is the migration invariant, asserted directly.
//
// PUBLISHING A LAYOUT THE FLEET CANNOT READ IS A FLAG DAY: the next release
// becomes unreadable to every deployed binary, and the thing that would fix them
// is the image they can no longer pull. So the writer must never move ahead of
// what a reader in the wild accepts, and this fails the moment somebody bumps
// SchemaVersion without a released reader for it.
func TestTheReaderIsAheadOfTheWriter(t *testing.T) {
	...
	if SchemaVersion != SchemaV1 {
		t.Errorf("SchemaVersion is %d. Moving the writer to a new layout is safe only once "+
			"every deployment in the wild already reads it; if that rollout has happened, "+
			"update this test deliberately rather than to make it pass", SchemaVersion)
	}

	if !readableSchemas[SchemaV2] {
		t.Error("schema 2 is not readable, so the fleet cannot be rolled forward before the " +
			"writer moves")
	}
}
...
// IMAGE AGE IS A FACT ABOUT THE ARTIFACT, AND NOTHING ELSE.
//
// There used to be a Stale() beside Aging that REFUSED an import at built_at + 30
// days, reasoning that a runner baked N days ago is at least N days old. The
// arithmetic is true and the conclusion does not follow, in both directions:
// github's window opens when the first release NEWER than the baked one appears, so
// an image built the day a release shipped is still current a year later if nothing
// else shipped, and one built yesterday around a runner three releases behind is
// already refused. The check rejected images that worked and accepted images that
// could not.
//
// So this asserts what age DOES mean -- a note about billet's weekly build -- and
// there is deliberately nothing here that returns a refusal.
func TestImageAgeIsANoticeRatherThanAVerdict(t *testing.T) {
	m := validManifest()
	...
	for _, tc := range []struct {
		...
		{"fresh", built.Add(24 * time.Hour), false},
		{"just before the notice", built.Add(AgeNotice - time.Hour), false},
		{"at the notice", built.Add(AgeNotice), true},
		{"a month later", built.Add(30 * 24 * time.Hour), true},
		{"long past any window", built.Add(90 * 24 * time.Hour), true},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
// AND AN OLD IMAGE STAYS OLD. Aging used to stop being true once the image passed
// thirty days, because Stale took over there -- so the one image most worth
// mentioning was the one nothing mentioned once the refusal was removed.
func TestAnOldImageKeepsReportingAsOld(t *testing.T) {
	m := validManifest()

	for d := AgeNotice; d < 200*24*time.Hour; d += 12 * time.Hour {
		if !m.Aging(m.BuiltAt.Add(d)) {
			t.Fatalf("at %v the image stopped reporting as old", d)
		}
	}
}
