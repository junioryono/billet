...
// GitHub reporting an empty scale set is what releases a promise it has no
// record of. It is the only thing that recovers this escrow.
//
// A timer cannot: an acquisition is a commitment to GitHub and no local clock
// revokes one. What makes this different is that billet decides nothing — GitHub
// reports, in statistics it already sends, that the scale set has no job of any
// kind outstanding, and a promise cannot be outstanding against a scale set with
// nothing outstanding. The commitment is released by the party holding it.
func TestAnEmptyScaleSetReleasesAPromiseGitHubHasNoRecordOf(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}
	...
	session.onGet = func() (*Message, error) {
		switch delivered.Add(1) {
		case 1:
			// Acquired, and then GitHub never mentions the job again.
			return &Message{MessageID: 1, Available: []Job{{RequestID: 11, RunID: 101}}}, nil
		case 2:
			// Too soon: the promise is not stale yet, so an empty scale set must
			// NOT release it. Statistics describe the moment the message was
			// generated and can predate an acquisition made a round trip ago.
			return &Message{MessageID: 2, Statistics: &Statistics{}}, nil
		case 3:
			if l.Acquiring() != 1 {
				t.Error("a promise was released before it was even stale; a fresh acquisition " +
					"can legitimately be missing from statistics that predate it")
			}

			time.Sleep(120 * time.Millisecond)

			// Now stale, and GitHub still reports nothing outstanding.
			return &Message{MessageID: 3, Statistics: &Statistics{}}, nil
		case 4:
			freed.Store(l.Acquiring() == 0)
	...
	l = NewListener(a, tiers[0].Label, session, WithStalePromiseAfter(100*time.Millisecond))
	...
	if !freed.Load() {
		t.Error("the promise survived GitHub reporting an empty scale set; nothing else " +
			"can ever release it, so that capacity is held for the life of the session")
	}
}

// Statistics that report ANY outstanding job leave every promise alone.
//
// The reconciliation turns on all four job counters being zero rather than on
// TotalAcquiredJobs alone, because whether "acquired" includes "assigned" is not
// documented and a rule resting on how those categories nest would be a guess
// about someone else's API. This asserts the conservative reading: a scale set
// with anything at all outstanding tells billet nothing about a specific promise.
func TestOutstandingWorkAnywhereLeavesPromisesAlone(t *testing.T) {
	for name, stats := range map[string]*Statistics{
		"available": {TotalAvailableJobs: 1},
		"acquired":  {TotalAcquiredJobs: 1},
		"assigned":  {TotalAssignedJobs: 1},
		"running":   {TotalRunningJobs: 1},
	} {
		t.Run(name, func(t *testing.T) {
			...
			session.onGet = func() (*Message, error) {
				...
				case 1:
					return &Message{MessageID: 1, Available: []Job{{RequestID: 11, RunID: 101}}}, nil
				case 2:
					// Well past stale, so only the counters can save the promise.
					time.Sleep(120 * time.Millisecond)

					return &Message{MessageID: 2, Statistics: stats}, nil
				case 3:
					kept.Store(l.Acquiring() == 1)
			...
			if !kept.Load() {
				t.Error("the promise was released while GitHub still reported outstanding " +
					"work; billet owes a runner for a job that may still be assigned")
			}
		})
	}
}

// A release that fails is not the same as a release that finds nothing, and the
// reconciliation has to tell them apart.
//
// Driven directly rather than through Run, because both outcomes are decided by
// one allocator call and a live heartbeat would reach the same leases first,
// making whichever assertion won the race the one being tested.
func TestReconciliationKeepsAPromiseItCouldNotRelease(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}

	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 64 * config.GiB}, tiers)

	leases, err := a.Escrow(t.Context(), tiers[0].Label, 1)
	if err != nil {
		t.Fatalf("Escrow: %v", err)
	}

	l := NewListener(a, tiers[0].Label, &fakeSession{},
		WithStalePromiseAfter(time.Millisecond))

	// A promise old enough to be eligible.
	l.acquiring[11] = &promise{lease: leases[0], at: time.Now().Add(-time.Hour)}

	// A cancelled context makes the allocator write fail the way a busy or
	// interrupted database would, without pretending the lease is gone.
	dead, cancel := context.WithCancel(t.Context())
	cancel()

	l.reconcileAgainst(dead, &Statistics{})

	if l.Acquiring() != 1 {
		t.Fatal("the promise was dropped after its release failed; the ledger still counts " +
			"that lease and nothing references it any more, so only the reaper gets it back")
	}

	// And the retry succeeds, because Release is idempotent and the entry was
	// kept for exactly that.
	l.reconcileAgainst(t.Context(), &Statistics{})

	if l.Acquiring() != 0 {
		t.Error("the retry did not release the promise")
	}
}

// A lease that is already terminal is dropped rather than retried forever.
//
// The mirror of the case above, and the reason the two must be distinguished:
// every retry against a reaped lease fails identically, so keeping it would
// advertise capacity belonging to someone else for the life of the session.
//
// It is reaped rather than released to get there. Release is IDEMPOTENT for the
// same outcome — releasing an already-`done` lease as `done` returns nil — so
// the first version of this test released the lease by hand, got nil back, and
// asserted the happy path while claiming to test the terminal one. Mutation
// caught it. Reap is what produces a genuinely unreleasable lease: it marks the
// row `failed` AND bumps the epoch, so the promise's own epoch is stale.
func TestReconciliationDropsAPromiseWhoseLeaseIsAlreadyGone(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}

	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 64 * config.GiB}, tiers,
		alloc.WithLeaseTTL(20*time.Millisecond))
	...
	// Abandoned long enough for the reaper to take it.
	...
	if reaped != 1 {
		t.Fatalf("reaped %d leases, want 1; the promise's lease is still live and this test "+
			"would be exercising the ordinary release path", reaped)
	}

	l.reconcileAgainst(t.Context(), &Statistics{})

	if l.Acquiring() != 0 {
		t.Error("a promise whose lease is already terminal was kept; every retry fails the " +
			"same way, so it would be advertised for the life of the session")
	}
}
