...
//
// It no longer ENDS anything. drain_timeout stopped being a deadline — a drain
// waits for as long as the work takes, because a timer failing somebody's build
// is a defect — so this only decides how soon such a test would start logging
// that its drain is running long. Use stopsWithoutWaiting to make a shutdown
// return.
const notDrainingHere = 50 * time.Millisecond

// stopsWithoutWaiting ends a shutdown drain at once, the way a second signal does.
//
// THE TEST SUITE USED TO END ITS DRAINS BY LETTING THE DEADLINE EXPIRE, which was
// available only while expiry destroyed the jobs still running. Nothing expires
// now, and these fake sessions never send a completion, so a listener cancelled
// with work in flight waits forever — correctly. A second signal is production's
// own answer to "stop waiting", it leaves the compute alone, and it is therefore
// the mechanism a test should be using to reach a shutdown at all.
func stopsWithoutWaiting() Option {
	hurry := make(chan struct{})
	close(hurry)

	return WithHurrySignal(hurry)
}
...
func TestReturnedLowerPollReleasesSurplusBeforeAcquireBlocks(t *testing.T) {
	...
	first := NewListener(a, tiers[0].Label, firstSession, stopsWithoutWaiting())
...
func TestACompletionForAnotherTierRemainsFatalAndUnacknowledged(t *testing.T) {
	...
	wrongTier := NewListener(a, tiers[1].Label, session,
		WithMaxCapacity(0), WithDrainGrace(notDrainingHere), stopsWithoutWaiting())
...
func TestCapacityIsHeldWhenTheComputeWillNotDie(t *testing.T) {
	...
	l := NewListener(a, tiers[0].Label, session, WithRunner(runner), WithDrainGrace(notDrainingHere), stopsWithoutWaiting())
...
func (f *fakeRunnerRegistry) RemoveRunner(_ context.Context, id int64, name string) error {
...
func (f *fakeSession) GetMessage(ctx context.Context, _ int64, maxCapacity int) (*Message, error) {
	...
	// onPollCtx SEES THE CONTEXT THE LOOP IS POLLING WITH, which onPoll cannot.
	// During a drain that context is the DRAIN's, with the drain's budget on it —
	// so a test can block here until that budget expires and hand back its error,
	// which is the only way to stage a failure landing inside a call with the
	// budget genuinely gone. A sleep cannot do it: the drain context does not
	// exist until the loop next reaches the top.
	if f.onPollCtx != nil {
		if err := f.onPollCtx(ctx); err != nil {
			return nil, err
		}
	}
...
func TestShutdownUsesTheAuthoritativeResultForPendingCompletion(t *testing.T) {
	...
	done := l.destroyAll(t.Context(), true, nil)
...
func TestShutdownRetainsACompletionUntilItsCapacityReleaseSettles(t *testing.T) {
	...
	destroyed := l.destroyAll(t.Context(), true, nil)
	interrupted, cancel := context.WithCancel(t.Context())
	cancel()
...
func TestShutdownPreservesARestoredCompletionUntilItsHolderReturns(t *testing.T) {
	...
	destroyed := restarted.destroyAll(t.Context(), true, nil)
...
func TestAStuckCleanupRetryDoesNotStarveTheDestroys(t *testing.T) {
	...
	holdRunningOwedDestroy(t, l, a, tiers[0].Label, 9)

	// ADDED, NOT ASSIGNED. Replacing the map wipes the destroy request 9 is owed,
	// and the shutdown then has nothing to reach past the stuck retry — so the
...
//
// Found by review of the custody handoff, which it undid at exactly the moment
...
func TestShutdownDoesNotReleaseALeaseTheRunnerIsHolding(t *testing.T) {
	...
	// provably gone.
	//
	// COUNTED, because the observation this test ends on — one slot short — is
	// ALSO what a shutdown produces for an ordinary running lease it never asked
	// about. Without proof the runner answered, a regression that skipped this
	// cleanup record entirely would leave the same headroom and pass.
	var custodyAnswers atomic.Int32
	...
	// OWED A DESTROY, so the runner is actually ASKED and can answer ErrCustody.
	// Seeded as `running` alone this never reached the runner at all, and passed
	// only because a shutdown now keeps every running lease — the right answer
	// arrived at without the mechanism under test ever running.
	holdRunningOwedDestroy(t, l, a, tiers[0].Label, 7)
	...
	// EXACTLY ONE, not "not two". Zero headroom satisfies `room != 2` as well,
	// and would mean an unrelated escrow release had failed — the right answer
	// arrived at for a reason this test is not about.
	if room != 1 {
		t.Errorf("headroom = %d, want 1: the escrowed leases come back and the one the "+
			"runner took into custody does not, because its guest may still be on that "+
			"machine", room)
	}

	if got := custodyAnswers.Load(); got != 1 {
		t.Errorf("the runner was asked to destroy %d times, want exactly 1; without that "+
			"this test never observed ErrCustody at all and its headroom assertion is "+
			"satisfied by an ordinary running lease", got)
	}
}
...
// holdRunningOwedDestroy is holdRunning plus the destroy that job's completion
// left behind, which is what a shutdown still tears down.
//
// A SHUTDOWN NO LONGER DESTROYS A RUNNING JOB, so a test about the teardown's
// MECHANICS — its budgets, its ordering, whether one stuck retry starves the
// pass — needs its destroyable material to be work GitHub has already concluded.
// `running` alone gives the teardown nothing to do, and those tests then pass or
// fail for a reason that has nothing to do with what they are named for.
//
// The staged state is a real one rather than a convenience: a completion whose
// destroy has not landed keeps its lease in `running` AND a record in `cleanup`,
// and that destroy is owed no matter what else the process is doing.
func holdRunningOwedDestroy(t *testing.T, l *Listener, a *alloc.Allocator, tier string,
	requestID int64,
) *alloc.Lease {
	t.Helper()

	lease := holdRunning(t, l, a, tier, requestID)

	l.mu.Lock()
	if l.cleanup == nil {
		l.cleanup = map[int64]*pendingCleanup{}
	}
	// NO LEASE ON THE RECORD, because production does not put one there. `complete`
	// creates `&pendingCleanup{job: job}` for a request it still holds in
	// `running`, and the lease stays where it is.
	//
	// The first version of this helper attached it, which looked harmless and was
	// not: an entry carrying a lease is ALSO classified as parked, so releaseAll
	// hands its capacity back through a second, independent loop. The headroom
	// assertions in the slow-destroy and slow-close tests would then still pass
...
// TestDestroyCompletedRecordsRunnerDeregistration proves destroyCompleted marks
// the lease deregistered once RemoveRunner succeeds, so a teardown whose compute
// destroy is still retrying stops counting against the assignment deficit. The
// Destroy here fails, keeping the lease non-terminal, so only the deregistration
// mark can drop it from ActiveRunnerLeases.
func TestDestroyCompletedRecordsRunnerDeregistration(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}
	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 32 * config.GiB}, tiers)
	registry := &fakeRunnerRegistry{}
	runner := &fakeRunner{onDestroy: func(int64) error {
		return errors.New("compute destroy still retrying")
	}}
	l := NewListener(a, tiers[0].Label, &fakeSession{}, WithRunner(runner),
		WithRunnerRegistry(registry))
	...
	if n, err := a.ActiveRunnerLeases(t.Context(), tiers[0].Label); err != nil || n != 1 {
		t.Fatalf("before: ActiveRunnerLeases = %d, err %v; want 1", n, err)
	}
	...
	if len(registry.names) != 1 {
		t.Fatalf("RemoveRunner called %d times; want 1", len(registry.names))
	}
	if n, err := a.ActiveRunnerLeases(t.Context(), tiers[0].Label); err != nil || n != 0 {
		t.Fatalf("after: ActiveRunnerLeases = %d, err %v; want 0 (lease marked deregistered "+
			"even though Destroy is still retrying)", n, err)
	}
}

// TestDestroyCompletedDoesNotMarkWhenRemoveRunnerFails proves the mark happens
// only AFTER a successful GitHub deregistration: a runner whose removal failed is
// still registered and schedulable, so its lease must keep counting or a
// replacement double-schedules it. This also pins the remove-then-mark ordering.
func TestDestroyCompletedDoesNotMarkWhenRemoveRunnerFails(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}
	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 32 * config.GiB}, tiers)
	registry := &fakeRunnerRegistry{err: errors.New("github unavailable")}
	runner := &fakeRunner{onDestroy: func(int64) error { return nil }}
	l := NewListener(a, tiers[0].Label, &fakeSession{}, WithRunner(runner),
		WithRunnerRegistry(registry))
	...
	if n, err := a.ActiveRunnerLeases(t.Context(), tiers[0].Label); err != nil || n != 1 {
		t.Fatalf("ActiveRunnerLeases = %d, err %v; want 1 (a still-registered runner "+
			"must not be marked deregistered)", n, err)
	}
}

// TestDestroyCompletedSkipsRemovalForAnIdentitylessCompletion proves a completion
// that carries no lease and no runner name does not wedge on RemoveRunner. There
// is nothing registered to withdraw, so RemoveRunner(0, "") — which the scale-set
// client refuses forever — is skipped and teardown proceeds, instead of the
// completion retrying that impossible removal and holding its capacity.
func TestDestroyCompletedSkipsRemovalForAnIdentitylessCompletion(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}
	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 32 * config.GiB}, tiers)
	// RemoveRunner would fail if it were called at all.
	registry := &fakeRunnerRegistry{err: errors.New("scaleset: runner removal needs an id or name")}
	...
	if len(registry.names) != 0 {
		t.Fatalf("RemoveRunner was called %d times for a completion with no id or name; want 0",
			len(registry.names))
	}
	if destroyed.Load() != 1 {
		t.Fatalf("compute destroy ran %d times; want 1 — teardown must proceed", destroyed.Load())
	}
}

// TestDestroyCompletedRemovesAnIdOnlyCompletionsRunner proves a completion that
// carries a runner id but no name and no lease still deregisters: the binding is
// resolved from the lease and runner name, so it stays identity-less, and the
// removal must fall back to the completion's own id rather than skip it and leak
// a routable registration whose compute is about to be destroyed.
func TestDestroyCompletedRemovesAnIdOnlyCompletionsRunner(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}
	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 32 * config.GiB}, tiers)
	registry := &fakeRunnerRegistry{}
	var destroyed atomic.Int32
	...
	if !slices.Equal(registry.ids, []int64{91}) {
		t.Fatalf("RemoveRunner ids = %v; want [91] (the completion's own id must be "+
			"withdrawn, not skipped)", registry.ids)
	}
	if destroyed.Load() != 1 {
		t.Fatalf("compute destroy ran %d times; want 1", destroyed.Load())
	}
}
...
// A COMPLETION WHOSE HOLDER CANNOT BE REACHED STOPS THIS LISTENER RENEWING.
//
// The plane binds a completion to the process that launched the compute; when
// that process is gone the bound destroy answers "holder unavailable". Read as
// an ordinary failed destroy, the lease stayed in `running`, was renewed on
// every heartbeat pass and never expired — so it was never quarantined, so no
// inventory could ever settle it, and the retry got the same answer for the
// life of the process. This asserts the exit from that loop: the obligation is
// kept, the lease is not, and the reaper can now see it.
func TestAHolderUnavailableCompletionStopsRenewingItsLease(t *testing.T) {
	now := time.Now().UTC()
	tiers := []config.Tier{tier("billet-4vcpu-a")}
	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 32 * config.GiB}, tiers,
		alloc.WithClock(func() time.Time { return now }), alloc.WithLeaseTTL(30*time.Second))
	...
	// THE OBLIGATION IS KEPT, WITHOUT PROOF. The entry carries the lease so the
	// retry addresses the same holder, and is not release-only, so nothing may
	// release it on the strength of it.
	l.mu.Lock()
	_, running := l.running[job.RequestID]
	entry := l.cleanup[job.RequestID]
	l.mu.Unlock()

	if running {
		t.Fatal("the lease is still in `running`, so this listener still renews a lease whose holder it cannot reach")
	}
	if entry == nil || entry.lease == nil || entry.lease.ID != lease.ID {
		t.Fatalf("the completion's obligation was dropped: %+v", entry)
	}
	if entry.releaseOnly || entry.retireOnly {
		t.Fatalf("an unreachable holder was recorded as proof of absence: %+v", entry)
	}

	// NOT RENEWED: a heartbeat pass leaves the expiry where it was, and the
	// reaper takes the lease into quarantine — charged, and visible.
	now = now.Add(31 * time.Second)
	l.heartbeatHeld(t.Context())
	if _, err := a.Reap(t.Context()); err != nil {
		t.Fatalf("Reap: %v", err)
	...
	if err != nil {
		t.Fatalf("read lease after reap: %v", err)
	}
	if current.Phase != alloc.PhaseQuarantine {
		t.Fatalf("a TTL after the holder became unreachable the lease is %s, want quarantine "+
			"(the listener is still renewing it)", current.Phase)
	}

	// THE RETRY STILL ASKS THE SAME HOLDER, with the lease and node it was bound to.
	runner.leaseID, runner.node = "", ""
	l.retryCleanup(t.Context())
	if runner.leaseID != lease.ID || runner.node != lease.Node {
		t.Fatalf("the retry addressed lease %q on %q, want %q on %q",
			runner.leaseID, runner.node, lease.ID, lease.Node)
	}

	// AND A SHUTDOWN RELEASES NOTHING, because nothing proved the compute gone.
	destroyed := l.destroyAll(t.Context(), false, nil)
	l.releaseAll(t.Context(), destroyed)
	if _, err := a.Lease(t.Context(), lease.ID); err != nil {
		t.Fatalf("shutdown released a lease whose compute was never proved gone: %v", err)
	}
}

// A CONCLUSIVE LAUNCH FAILURE IS ARCHIVED WITH ITS REASON. The lease never
// held compute and archives at once, so nothing outlives the process here; the
// reason is for the report, where a failure nothing explains is a row an
// operator cannot act on. On both paths: the release that lands at once, and
// the one parked for a retry because the ledger could not answer the first
// time.
func TestAConclusiveLaunchFailureIsArchivedWithItsReason(t *testing.T) {
	tiers := []config.Tier{tier("billet-4vcpu-a")}
	a := newAllocator(t, alloc.Limits{MaxVCPU: 8, MaxMemory: 32 * config.GiB}, tiers)
	runner := &fakeRunner{onLaunch: func(int64) error { return errors.New("no space left on device") }}
	l := NewListener(a, tiers[0].Label, &fakeSession{}, WithRunner(runner),
	...
	if reason, err := a.HistoryFailureReason(t.Context(), direct.ID); err != nil ||
		reason != alloc.LaunchFailedReason {
		t.Fatalf("history reason = %q err=%v, want %q", reason, err, alloc.LaunchFailedReason)
	}

	// PARKED, THEN RELEASED ON THE RETRY, with the same reason.
	...
	if _, err := a.Lease(t.Context(), parked.ID); err != nil {
		t.Fatalf("the parked release landed on a cancelled context: %v", err)
	}
	l.retryCleanup(t.Context())
	if outcome, err := a.HistoryOutcome(t.Context(), parked.ID); err != nil || outcome != string(alloc.PhaseFailed) {
		t.Fatalf("retried history outcome = %q err=%v, want failed", outcome, err)
	}
	if reason, err := a.HistoryFailureReason(t.Context(), parked.ID); err != nil ||
		reason != alloc.LaunchFailedReason {
		t.Fatalf("retried history reason = %q err=%v, want %q", reason, err, alloc.LaunchFailedReason)
	}
}

// AND THE REASON SURVIVES THE REAPER GETTING THERE FIRST (round five). A parked
// launch failure whose retry lands after the lease expired meets one of two
// rows: an escrow-only lease the reaper failed outright, with no reason, or a
// lease that had reached `launching` and was quarantined. The retry explains
// the first and resolves the second, and the history carries the reason either
// way.
func TestAParkedLaunchFailureKeepsItsReasonAcrossTheReaper(t *testing.T) {
	for _, launching := range []bool{false, true} {
		name := "an escrow-only lease the reaper failed"
		if launching {
			name = "a launching lease the reaper quarantined"
		}
		...
		t.Run(name, func(t *testing.T) {
			...
			lease := holdRunning(t, l, a, tiers[0].Label, 9)
			if launching {
				if _, err := a.RegisterNode(t.Context(), alloc.NodeRegistration{
					Name: lease.TargetNode, Provider: lease.Providers[0],
					VCPU: 1 << 20, Memory: 1 << 20 * config.GiB,
			...
			// THE REAPER ARRIVES BEFORE THE RETRY.
			now = now.Add(31 * time.Second)
			if _, err := a.Reap(t.Context()); err != nil {
				t.Fatalf("Reap: %v", err)
			}
			current, err := a.Lease(t.Context(), lease.ID)
			switch {
			case launching && (err != nil || current.Phase != alloc.PhaseQuarantine):
				t.Fatalf("a launching lease after the reaper = %+v err=%v, want quarantine", current, err)
			case !launching && !errors.Is(err, alloc.ErrLeaseNotFound):
				t.Fatalf("an escrow-only lease after the reaper = %+v err=%v, want failed outright", current, err)
			}

			l.retryCleanup(t.Context())

			if outcome, err := a.HistoryOutcome(t.Context(), lease.ID); err != nil || outcome != string(alloc.PhaseFailed) {
				t.Fatalf("history outcome = %q err=%v, want failed", outcome, err)
			}
			...
			if _, err := a.Lease(t.Context(), lease.ID); !errors.Is(err, alloc.ErrLeaseNotFound) {
				t.Fatalf("the lease is still open after the retry: %v", err)
			}
		})
	}
}
