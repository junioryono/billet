...
func TestOpenAppliesMigrations(t *testing.T) {
	...
	for rows.Next() {
		...
		if i >= len(sqliteTimeline.migrations) {
			t.Fatalf("recorded more migrations than defined")
		}
		want := sqliteTimeline.migrations[i]
	...
	if i != len(sqliteTimeline.migrations) {
		t.Errorf("recorded %d migrations, want %d", i, len(sqliteTimeline.migrations))
	}
...
func TestMigrationVersionsAreUniqueAndAscending(t *testing.T) {
	...
	seen := make(map[int]string, len(set))
...
func openAt(t *testing.T, dir string, version int) *DB {
	...
	full := sqliteTimeline.migrations

	t.Cleanup(func() { sqliteTimeline.migrations = full })
	...
	sqliteTimeline.migrations = truncated
	...
	sqliteTimeline.migrations = full
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if n != len(sqliteTimeline.migrations) {
		t.Errorf("reopening applied migrations twice: got %d, want %d", n, len(sqliteTimeline.migrations))
	}
}
...
func TestEditedMigrationIsRejected(t *testing.T) {
	...
	if err := db.Tx(t.Context(), func(tx *sql.Tx) error {
		_, err := tx.ExecContext(t.Context(),
			...
			sqliteTimeline.migrations[0].Version)
		return err
	}); err != nil {
...
func TestEveryTableIsStrict(t *testing.T) {
	...
	// THE UNDERSCORE IS ESCAPED for the reason userTables escapes it: `_` is a LIKE
	// wildcard, so the unescaped form also skips legal names like `sqliteX…`. Here
	// that would silently exempt such a table from the STRICT check rather than
	// exempt it from a row count, which is less severe and is the same defect — and
	// fixing one of two identical predicates is how the next reader concludes the
	// escape is optional.
...
func TestRebuildingMigrationsCopyEveryColumn(t *testing.T) {
	// BY VERSION, because a version is the migration's identity and a filename or
	// a Go variable name is not. These are lease_quarantine, strict_trust_tables,
	// custody_visibility and direct_assignment_identity — every migration that
	// rebuilds a table by copying it.
	for _, version := range []int{16, 17, 20, 26} {
...
func indexNamesInMigrations(t *testing.T) []string {
	t.Helper()
...
func TestRebuildingMigrationsKeepRowsIndexesAndKeys(t *testing.T) {
	...
	for _, want := range indexNamesInMigrations(t) {
		var name string
...
func TestADatabaseWrittenByAnEarlierBilletUpgrades(t *testing.T) {
	...
	if err := db.Tx(t.Context(), func(tx *sql.Tx) error {
		for _, stmt := range []string{
			...
			`ALTER TABLE job_history DROP COLUMN workflow_ref`,
			`ALTER TABLE job_history DROP COLUMN job_name`,
			`ALTER TABLE job_history DROP COLUMN event`,
			...
			`DROP TABLE controller_claim`,
			`DROP TABLE admission`,
			`DROP TABLE scale_set`,
			`DROP TABLE node_inventory`,
			`ALTER TABLE nodes DROP COLUMN incarnation`,
			`ALTER TABLE leases DROP COLUMN holder_incarnation`,
			`DROP TABLE credential_sweeps`,
			`ALTER TABLE nodes DROP COLUMN codebuild_jit_path`,
			`ALTER TABLE nodes DROP COLUMN codebuild_region`,
			`ALTER TABLE nodes DROP COLUMN node_digest`,
			`ALTER TABLE nodes DROP COLUMN codebuild_fleet`,
			`DROP TABLE rollout_nodes`,
			`DROP TABLE rollouts`,
			`DROP TABLE force_destroy_target`,
			`DROP TABLE force_destroy`,
			`DROP TABLE compute_barrier`,
			`DROP TABLE compute_barrier_nodes`,
			`ALTER TABLE nodes DROP COLUMN ec2_shapes`,
			`ALTER TABLE nodes DROP COLUMN node_release`,
			`ALTER TABLE nodes DROP COLUMN wire_min`,
			`ALTER TABLE nodes DROP COLUMN wire_max`,
			`ALTER TABLE nodes DROP COLUMN wire_version`,
			`ALTER TABLE nodes DROP COLUMN dispatch_generation`,
			`ALTER TABLE nodes DROP COLUMN decommissioned_at`,
			`ALTER TABLE nodes DROP COLUMN decommission_proven`,
			`ALTER TABLE nodes DROP COLUMN decommission_actor`,
			...
			`ALTER TABLE leases DROP COLUMN disruption`,
			`ALTER TABLE leases DROP COLUMN disrupted_at`,
			`ALTER TABLE job_history DROP COLUMN failure_reason`,
			// BEFORE the columns it names: SQLite refuses to drop a column an
			// index still references.
			`DROP INDEX job_history_disrupted_idx`,
			`ALTER TABLE job_history DROP COLUMN disruption`,
			`ALTER TABLE job_history DROP COLUMN disrupted_at`,
			`ALTER TABLE job_history DROP COLUMN result`,
			`ALTER TABLE job_history DROP COLUMN result_at`,
	...
	// nothing having decided that.
	if a, err := upgraded.Admission(t.Context()); err != nil {
		t.Errorf("an upgraded deployment has no admission state: %v", err)
	} else if a.Mode != AdmissionOpen || a.Generation != 0 || a.Provenance != "" {
		t.Errorf("an upgraded deployment is %+v, want open at generation zero", a)
	}
...
func TestAPendingCompletionWrittenAtVersion22SurvivesVersion23(t *testing.T) {
	...
	if err := db.Tx(t.Context(), func(tx *sql.Tx) error {
		for _, stmt := range []string{
			// BEFORE the column it names: SQLite refuses to drop a column an
			// index still references. Its migration is wound back too, so the
			// reopen rebuilds it rather than leaving this database without one.
			`DROP INDEX pending_completions_lease_idx`,
			...
			`DELETE FROM schema_migrations WHERE version IN (23, 36)`,
	...
	// AND THE INDEX THE REWIND DROPPED IS BACK. This fixture has to wind back
	// migration 36's bookkeeping too, because SQLite will not drop a column an
	// index references — so if it ever stopped doing that, the reopen would skip
	// the migration, leave the index absent, and every assertion above would
	// still pass while the disruption guard went back to scanning.
	requireIndex(t, upgraded, "pending_completions_lease_idx")
}

// requireIndex asserts a named index exists, so a fixture that winds a migration
// back is shown to have let the reopen rebuild it.
func requireIndex(t *testing.T, db *DB, name string) {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("index %s is missing after the upgrade: %v", name, err)
	}
}
...
// THE SCHEMA REFUSES A COUNT THAT IS NOT A COUNT.
//
// `running` is the number of billet instances one host said it saw, written only
// on the path where that host vouched for its list. A negative is not a smaller
// number, it is a different KIND of claim — and an earlier draft of this table
// carried a -1 sentinel for "could not tell" that nothing ever wrote, which is a
// schema documenting behaviour the code does not have.
//
// The epoch has the same shape of guard for the same reason: it is a fence, and
// a negative one fences nothing.
func TestNodeInventoryRefusesValuesThatAreNotCounts(t *testing.T) {
	t.Parallel()
			...
			err := db.Tx(t.Context(), func(tx *sql.Tx) error {
				_, err := tx.ExecContext(t.Context(),
					`INSERT INTO node_inventory (node, node_epoch, received_at, running)
					      VALUES (?, ?, ?, ?)`,
					"h-"+tc.name, tc.epoch, "2026-08-27T00:00:00Z", tc.running)
	...
	// AND IT ACCEPTS AN ORDINARY ONE, so the guards above are not simply
	// refusing everything.
	if err := db.Tx(t.Context(), func(tx *sql.Tx) error {
		_, err := tx.ExecContext(t.Context(),
			`INSERT INTO node_inventory (node, node_epoch, received_at, running)
			      VALUES (?, ?, ?, ?)`,
			"ordinary-host", 3, "2026-08-27T00:00:00Z", 0)

		return err
	}); err != nil {
		t.Fatalf("the schema refused an ordinary empty report: %v", err)
	}
}
