...
const (
	...
	//
	// THE DEFAULT, because the failure it prevents is worse than the one it causes.
	...
	//
	// The usual argument for spreading is contention, and it is weaker here than it
	// looks: billet escrows vCPU and memory and the provider enforces both as hard
	// per-container limits, so two jobs on one host do not take each other's cores or
	// RAM. What they share is disk, page cache and network. Packing is also what makes
	// a cloud host affordable — an instance with one job on it cannot be shut down —
	// and it is what Nomad does by default.
