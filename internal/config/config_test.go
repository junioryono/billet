...
func TestTrustedPoolPolicyMustBeCompleteAndInternallyConsistent(t *testing.T) {
	...
	for name, body := range map[string]string{
		"omitted group": strings.Replace(base, "    runner_group: trusted\n", "", 1),
		// GitHub's built-in group is displayed "Default" and resolves
		// case-insensitively, so every casing names the same all-repositories group
		// a trusted pool must never use — the guard is EqualFold, not ==. Both the
		// literal lowercase and a mixed case are checked, so a mutation to a
		// case-sensitive compare fails here rather than reopening one casing.
		"default group, lowercase": strings.Replace(base,
			"    runner_group: trusted\n", "    runner_group: default\n", 1),
		"default group, capitalized": strings.Replace(base,
			"    runner_group: trusted\n", "    runner_group: Default\n", 1),
		"default group, uppercase": strings.Replace(base,
			"    runner_group: trusted\n", "    runner_group: DEFAULT\n", 1),
...
// EVERY WAY A STREAM CAN END, told apart.
//
// The second decode exists so a second document cannot be silently ignored, and
// it treated EVERY error from that decode as end-of-input. Only io.EOF is: a
// valid first document followed by `---` and a truncated token then reached
// defaults, validation and a running control plane exactly as though the file
// had ended there, discarding whatever the operator wrote after it.
//
// The four cases are separated because two of them are NOT the same path — a
// malformed token with no `---` in front of it fails in the FIRST decode, and a
// test that only covered that one would pass against the bug.
func TestMalformedTrailingYAMLIsNotEndOfInput(t *testing.T) {
	t.Parallel()
	...
	for name, tc := range map[string]struct {
		...
		"an empty second document": {
			body: validConfig + "\n---\n",
			want: "more than one YAML document",
		},
		"a well-formed second document": {
			body: validConfig + "\n---\nserver:\n  max_vcpu: 999\n",
			want: "more than one YAML document",
		},
		"a malformed second document": {
			body: validConfig + "\n---\n[\n",
			want: "parse config",
		},
		"malformed trailing tokens with no separator": {
			body: validConfig + "\n[\n",
			want: "parse config",
		},
	} {
		t.Run(name, func(t *testing.T) {
			...
			if tc.want == "" {
				if err != nil {
					t.Fatalf("a config whose stream ends cleanly was refused: %v", err)
				}
			...
			// The distinction is the point: malformed content must never be
			// reported as, or mistaken for, a second document that parsed.
			if tc.want == "parse config" &&
				strings.Contains(err.Error(), "more than one YAML document") {
				t.Errorf("malformed content was reported as a second document: %v", err)
			}
		})
	}
}

// AND THE OVERRIDE IN A MALFORMED TAIL IS NOT APPLIED, which is the consequence
// the guard exists for rather than the error it returns.
//
// Asserting only that an error came back would agree with a Load that parsed the
// first document, ignored the wreckage after it, and handed back a config — the
// caller sees an error either way. This asserts there is no config at all.
func TestAMalformedTailYieldsNoConfigAtAll(t *testing.T) {
	t.Parallel()

	cfg, err := Load(writeConfig(t, validConfig+"\n---\n[\n"))
	if err == nil {
		t.Fatal("Load accepted a truncated trailing document")
	}
	if cfg != nil {
		t.Fatalf("Load returned a usable config alongside the error: %+v", cfg)
	}
}
...
// A WILDCARD AND A CONCRETE HOST ON ONE PORT ARE ONE SOCKET.
//
// The first version of this rule compared the two strings, which accepts
// 0.0.0.0:7717 beside 127.0.0.1:7717 and leaves the collision to bind time —
// where whichever listener binds SECOND reports it, so the operator is sent to
// the setting that is fine.
//
// BOTH DIRECTIONS AND BOTH WILDCARD SPELLINGS, because a check that only looked
// at one side, or only knew "0.0.0.0", would pass every case it was written for
// and miss its mirror.
func TestBootstrapListenIsRefusedWhenItOverlapsTheNodeWire(t *testing.T) {
	t.Parallel()

	for name, pair := range map[string][2]string{
		"identical":             {"10.0.0.4:7717", "10.0.0.4:7717"},
		"wildcard wire":         {"0.0.0.0:7717", "10.0.0.4:7717"},
		"wildcard bootstrap":    {"10.0.0.4:7717", "0.0.0.0:7717"},
		"v6 wildcard wire":      {"[::]:7717", "10.0.0.4:7717"},
		"v6 wildcard bootstrap": {"10.0.0.4:7717", "[::]:7717"},
		"both wildcard":         {"0.0.0.0:7717", "[::]:7717"},
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()

			body := strings.Replace(validConfig, "  listen: 127.0.0.1:7717\n",
				"  listen: \""+pair[0]+"\"\n  bootstrap_listen: \""+pair[1]+
					"\"\n  node_tls_hosts: [billet.example]\n", 1)
			body = strings.Replace(body, "  server_addr: 127.0.0.1:7717\n",
				"  server_addr: billet.example:7717\n  tls:\n"+
					"    cert: /etc/billet/node.crt\n    key: /etc/billet/node.key\n"+
					"    ca: /etc/billet/ca.crt\n", 1)

			_, err := Load(writeConfig(t, body))
			if err == nil {
				t.Fatalf("%s was accepted; the two listeners would contend for one socket", name)
			}

			if !strings.Contains(err.Error(), "the same socket") {
...
// AND DIFFERENT PORTS, OR DIFFERENT CONCRETE HOSTS, ARE NOT AN OVERLAP.
//
// Without this the rule above could be a validator that refuses every
// bootstrap_listen there is, which would pass every case in it.
func TestSeparateAddressesAreNotAnOverlap(t *testing.T) {
	t.Parallel()

	for name, pair := range map[string][2]string{
		"different ports":       {"0.0.0.0:7717", "0.0.0.0:7718"},
		"different hosts":       {"10.0.0.4:7717", "10.0.0.5:7717"},
		"loopback bootstrap":    {"10.0.0.4:7717", "127.0.0.1:7718"},
		"wildcard, other ports": {"0.0.0.0:7717", "[::]:7718"},
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
...
// node.bootstrap_addr IS AN ADDRESS, AND NOTHING CHECKED IT.
//
// It is used once, by `billet node --enroll`, which prefixes it with https://
// and dials — so a value that is already a URL loads cleanly and fails at the
// one moment an operator is standing at a new machine with a join token in hand.
func TestBootstrapAddrMustBeHostPort(t *testing.T) {
	t.Parallel()

	for name, addr := range map[string]string{
		"a url":       "https://billet.example:7718",
		"no port":     "billet.example",
		"with a path": "billet.example:7718/v1/enroll",
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()

			body := strings.Replace(validConfig, "  server_addr: 127.0.0.1:7717\n",
				"  server_addr: 127.0.0.1:7717\n  bootstrap_addr: \""+addr+"\"\n", 1)

			_, err := Load(writeConfig(t, body))
			if err == nil {
				t.Fatalf("node.bootstrap_addr %q was accepted", addr)
			}

			if !strings.Contains(err.Error(), "node.bootstrap_addr") {
...
// AND A LOOPBACK CONTROL PLANE HAS NOTHING TO ENROLL INTO.
//
// It serves plain HTTP and issues no certificates, so an enrollment listener
// beside it would hand out an authority that decides nothing and admit machines
// to a wire that authenticates nobody. Same rule that refuses node.tls against a
// loopback server.
func TestBootstrapListenIsRefusedOnALoopbackWire(t *testing.T) {
	body := strings.Replace(validConfig, "  listen: 127.0.0.1:7717\n",
		"  listen: 127.0.0.1:7717\n  bootstrap_listen: 0.0.0.0:7718\n", 1)

	_, err := Load(writeConfig(t, body))
	if err == nil {
		t.Fatal("a loopback control plane was allowed to serve enrollment")
	}

	if !strings.Contains(err.Error(), "nothing for a node to enroll into") {
		t.Errorf("the refusal does not say why: %v", err)
	}
}

// AND THE ORDINARY SHAPE LOADS. Without this the two refusals above could both
// be produced by a validator that rejects every bootstrap_listen there is.
func TestASeparateBootstrapListenLoads(t *testing.T) {
	body := strings.Replace(validConfig,
		"  listen: 127.0.0.1:7717\n",
		"  listen: 0.0.0.0:7717\n  bootstrap_listen: 0.0.0.0:7718\n"+
			"  node_tls_hosts: [billet.example]\n", 1)
	body = strings.Replace(body, "  server_addr: 127.0.0.1:7717\n",
		"  server_addr: billet.example:7717\n  bootstrap_addr: billet.example:7718\n"+
			"  tls:\n    cert: /etc/billet/node.crt\n    key: /etc/billet/node.key\n"+
			"    ca: /etc/billet/ca.crt\n", 1)

	cfg, err := Load(writeConfig(t, body))
	if err != nil {
		t.Fatalf("a control plane with a separate enrollment address was refused: %v", err)
	}

	if cfg.Server.BootstrapListen != "0.0.0.0:7718" {
		t.Errorf("server.bootstrap_listen = %q, want 0.0.0.0:7718", cfg.Server.BootstrapListen)
	}

	if cfg.Node.BootstrapAddr != "billet.example:7718" {
		t.Errorf("node.bootstrap_addr = %q, want billet.example:7718", cfg.Node.BootstrapAddr)
	}
}
