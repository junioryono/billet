...
func TestThePhaseTableHoldsTheSpecifiedRows(t *testing.T) {
	...
	cases := []struct {
		...
		{"config-rewritten always restarts, unknown", VariantRetainedNode, PhaseConfigRewritten, with(ok, func(f *Facts) { f.Identity = IdentityArchive; f.Config = ConfigStaged; f.NodeChanged = VerdictUnknown }), ActionRestart, ""},
