...
// ErrEnrollUnavailable means the control plane could not answer right now — a
// busy ledger, or more enrollments in flight than it will hold at once.
//
// SEPARATE FROM A REFUSAL BECAUSE THE CALLER'S MOVE IS OPPOSITE. Every 503 on
// this route came back as an unadorned error, so a caller waiting for a human to
// approve it abandoned the wait over a condition whose own message said to try
// again — with the pending request still recorded and the join token already
// spent, so the operator's approval landed on a node that had stopped asking.
var ErrEnrollUnavailable = errors.New("nodeclient: the control plane cannot take this enrollment right now")
...
func FetchCA(ctx context.Context, base, wantFingerprint string) ([]byte, string, error) {
	...
	if err != nil {
		// THE HINT IS UNCONDITIONAL, because the interesting failure cannot be
		// recognised. A control plane that serves enrollment separately requires a
		// client certificate on its node wire, so dialling this route there ends in
		// the handshake and the server never gets to say anything — and MEASURED,
		// the alert reaches the caller as crypto/tls's unexported alert type, so
		// errors.As against tls.AlertError is false and there is nothing structural
		// to match. Matching the prose instead would be a string comparison against
		// somebody else's error text.
		return nil, "", fmt.Errorf("nodeclient: read the control plane's authority: %w\n"+
			"(a control plane that serves enrollment on an address of its own — "+
			"server.bootstrap_listen — cannot be reached for this on its node wire, which "+
			"requires a client certificate this machine does not have yet. Dial that address "+
			"with --bootstrap-addr)", err)
	}
...
func Enroll(
	...
	if res.StatusCode == http.StatusServiceUnavailable {
		return nil, nil, fmt.Errorf("%w (%s)", ErrEnrollUnavailable, res.Status)
	}
