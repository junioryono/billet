...
// the same, which is why the return distinguishes them at all — and why the
// confirmation here is worth stating rather than left to be assumed by a caller
// that has no way to check.
func (p *Provider) Destroy(ctx context.Context, id string) (provider.Teardown, error) {
	...
	// running.
	// ALSO CONFIRMING. Unlike EC2's InvalidInstanceID.NotFound, which comes from
	// an eventually-consistent control plane that may be describing an instance
	// it has simply not seen yet, the docker daemon is authoritative about its
	// own containers: "no such container" means there is not one.
