...
// one every other fixture happens to use. A single Launch can make several of
// these, walking the operator's declared shapes.
...
// the whole defect the explicit flag closed — worth checking wherever the request
// is read rather than only in the test that happens to be about it.
	...
	for i := 1; i <= highest; i++ {
		...
		if flag == "" {
			t.Errorf("device %s left with no DeleteOnTermination, which is the whole defect "+
				"the explicit flag exists to close", device)
		}
...
func newTestProvider(t *testing.T, f *fakeEC2, mutate func(*config.EC2Config)) *Provider {
	...
	p, err := New("dep-1", cfg,
		...
		WithCredentials(awscreds.Static{AccessKeyID: "AKID", SecretAccessKey: "secret"}))
	if err != nil {
		t.Fatalf("New: %v", err)
	}

	// THE VERIFICATION'S POLLS DO NOT WAIT IN A TEST, and they still honour the
	// context: two of those loops are bounded by cancellation rather than by a
	// count, so a replacement that returned nil unconditionally would spin instead
	// of stopping. A millisecond keeps the interleaving real without spending a
	// minute of wall clock per test on sleeping.
	p.sleep = func(ctx context.Context, _ time.Duration) error {
		return sleepFor(ctx, time.Millisecond)
	}
...
// A SHAPE AWS HAS NONE OF FALLS THROUGH TO THE NEXT DECLARED ONE.
//
...
// AND THE CLIENT TOKEN CHANGES WITH THE SHAPE, which is the half of the fallback
// that is easy to get wrong and impossible to see afterwards.
//
...
// to equal the instance name exactly, and then the shape fallback gave each
// attempt its own token — a change that is correct and that a literal comparison
// reports as a regression. What has to hold is that the token is derived from
// this lease and from nothing outside it, so no two leases can ever collide and
// a re-send of the same request reproduces it.
...
// A TERMINATE REQUEST IS NOT A STOPPED GUEST, and this backend must not
// claim otherwise.
//
...
// NOT-FOUND IS NOT PROOF EITHER, because DescribeInstances is eventually
...
func TestAProviderKeepsItsOwnCopyOfTheNetwork(t *testing.T) {
	...
	p, err := New("dep-1", cfg,
		WithHTTPClient(f.Client()),
		WithCredentials(awscreds.Static{AccessKeyID: "AKID", SecretAccessKey: "s"}))
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
//
// `p.api.creds == nil` is FALSE for (*awscreds.IMDS)(nil) — the interface holds
...
// EVERY EBS DEVICE THE IMAGE DECLARES LEAVES WITH AN EXPLICIT FLAG.
//
...
// AN INSTANCE-STORE-BACKED AMI IS REFUSED, AND REFUSED EARLY.
//
