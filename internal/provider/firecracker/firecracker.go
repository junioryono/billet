...
type Provider struct {
	...
	// syncFile and syncDir are the durability boundary for ownership and resource
	// records. They are seams so crash-ordering tests can observe the operations a
	// normal filesystem cannot report after the fact.
	...
	// removeCgroupFn is the cgroup teardown boundary. It is a seam so lifecycle
	// recovery tests can prove that a failed removal retains its retry authority.
	removeCgroupFn   func(jail) error
	removeCgroupAtFn func(string, jail) error
...
// withDurability replaces filesystem syncs for crash-ordering tests.
func withDurability(file func(*os.File) error, dir func(string) error) Option {
	return func(p *Provider) {
		if file != nil {
			p.syncFile = file
		}
...
func New(owner string, cfg config.FirecrackerConfig, disk RootDisk, opts ...Option) (*Provider, error) {
	if err := deploymentid.Validate(owner); err != nil {
		return nil, fmt.Errorf("firecracker: invalid deployment identity: %w", err)
	}
	...
	if err := validateLifecycleExecName(p.execName); err != nil {
		return nil, fmt.Errorf("firecracker: resolved binary name %q cannot be recorded: %w",
			p.execName, err)
	}
...
func (p *Provider) Launch(ctx context.Context, spec provider.Spec) (*Instance, error) {
	...
	// A jail with a durable owner marker exists first, so every later clone can be
	// attributed. A crash before that marker becomes durable leaves an ambiguous
	// directory that inventory and teardown refuse rather than adopting.
	//
	// ErrJailExists MUST NOT UNWIND. Refusing a jail that already exists is a refusal
	// to touch a previous microVM's state, so tearing it down would destroy exactly
	// what the error says was left alone. claim cleans up only a directory whose
	// successful Mkdir proved this call created it.
	if err := p.claim(j); err != nil {
		if _, statErr := os.Stat(j.dir()); errors.Is(statErr, os.ErrNotExist) {
			return nil, errors.Join(err, p.releaseLifecycle(spec.Name))
		} else if statErr != nil {
			return nil, errors.Join(err, fmt.Errorf("firecracker: verify failed claim cleanup for %s: %w",
				j.dir(), statErr))
		}
...
// executing. That is the same promise docker makes and the one EC2 cannot — so a
// success here may be read as proof, and a failure may not.
...
func (p *Provider) destroy(ctx context.Context, id string) error {
	...
	authorized, err := p.authorizeLifecycle(id, j, found)
	if err != nil {
		return err
	}
	if !authorized {
	...
	if !found {
		// NO JAIL, BUT NOT NECESSARILY NOTHING. Claims outlive the jail they
		// named, and a root clone is addressed independently by lease id.
		return p.cleanupLifecycleOnly(ctx, id)
	}
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("firecracker: destroy %s was canceled before stopping its microVM: %w", id, err)
	}
...
// cgroup or root disk could exist.
func (p *Provider) unwindBeforeClone(j jail, res resources) error {
	if err := j.remove(); err != nil {
		return err
	}
	if err := p.syncDir(filepath.Dir(j.dir())); err != nil {
		return fmt.Errorf("firecracker: sync removal of the failed jail %s: %w", j.dir(), err)
	}
	if err := p.releaseResources(res, j.id); err != nil {
		return err
	}

	return p.releaseLifecycle(j.id)
}
...
func (p *Provider) unwind(
	...
	return p.releaseLifecycle(spec.Name)
}
...
func (p *Provider) Find(ctx context.Context, name string) (*Instance, bool, error) {
	...
	if err != nil {
		...
		// started, and a missing or unreadable ownership marker is not authority to
		// adopt a jail another deployment may have created.
...
func (p *Provider) List(ctx context.Context) ([]*Instance, error) {
	...
	if err := p.reconcileLifecycleOnly(ctx, jails); err != nil {
		return nil, err
	}
