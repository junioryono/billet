...
func TestAMicroVMSurvivesTheBinaryBeingUpgradedUnderIt(t *testing.T) {
	...
	// The immediately preceding binary stored both fields in one file. Stage its
	// exact bytes rather than approximating them with the current writer.
	if err := os.WriteFile(h.p.lifecycleFile(theInstance),
		[]byte(testDeployment+"\n"+h.p.execName+"\n"), 0o600); err != nil {
		t.Fatalf("stage predecessor lifecycle authority: %v", err)
	}
	if err := os.Remove(h.p.lifecycleExecFile(theInstance)); err != nil {
		t.Fatalf("remove the sidecar the predecessor did not write: %v", err)
	}
	...
	upgraded, err := New(testDeployment, h.p.cfg, h.disk,
		withRunner(h.record))
	if err != nil {
		t.Fatalf("New after the upgrade: %v", err)
	}
	upgraded.procMountsPath = h.p.procMountsPath
...
func TestDestroyRefusesAnotherDeploymentsMicroVM(t *testing.T) {
	...
	stranger, err := New(otherTestDeployment, h.p.cfg, &fakeDisk{device: "/dev/rbd1"})
	if err != nil {
...
func TestStopVMMRechecksCancellationBeforeItsFirstSignal(t *testing.T) {
	requireProc(t)

	h := newHarness(t)
	j := h.p.jailFor(theInstance)
	if err := h.p.claim(j); err != nil {
		t.Fatalf("claim the stand-in jail: %v", err)
	}

	pid := standIn(t, theInstance, false)
	...
	if err := h.p.stopVMM(ctx, j); !errors.Is(err, context.Canceled) {
		t.Fatalf("stopVMM error = %v, want context cancellation", err)
	}
	if owns, err := pidIsVMM(pid, theInstance); err != nil || !owns {
		t.Fatalf("canceled stopVMM signalled the live process: owns=%v err=%v", owns, err)
	}
}
...
func TestStopVMMRechecksCancellationBeforeEscalatingToSIGKILL(t *testing.T) {
	...
	pid := standIn(t, theInstance, true)
	if err := os.MkdirAll(filepath.Dir(j.pidFile()), 0o700); err != nil {
		t.Fatalf("make the pid directory: %v", err)
	}
	if err := os.WriteFile(j.pidFile(), []byte(strconv.Itoa(pid)+"\n"), 0o600); err != nil {
...
func TestALeaseCannotHoldTwoJailsAcrossAnUpgrade(t *testing.T) {
	...
	upgraded, err := New(testDeployment, h.p.cfg, h.disk, withRunner(h.record))
	if err != nil {
		t.Fatalf("New after the upgrade: %v", err)
	}
...
func TestTheJailIsClaimedBeforeTheDiskIsCloned(t *testing.T) {
	...
	if _, err := os.Stat(h.p.lifecycleFile(theInstance)); !errors.Is(err, os.ErrNotExist) {
		t.Errorf("the failed clone left its lifecycle permanently reserved: %v", err)
	}
}
