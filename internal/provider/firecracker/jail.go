...
// jail attributable. It is synced with both directory entries before claim returns,
// so anything allocated after this call has durable deployment ownership. A crash
// before it returns may leave an ownerless directory, which inventory and teardown
// refuse rather than claiming on the strength of an ambiguous path.
func (p *Provider) claim(j jail) error {
	execDir := filepath.Dir(j.dir())
	...
	claimErr := p.writeOwner(j)
	if claimErr == nil {
		claimErr = p.syncDirectoryAncestors(execDir)
	}
	if claimErr != nil {
		// THIS CALL CREATED THE DIRECTORY, so unlike ErrJailExists there is no
		// previous microVM to preserve. Leaving a failed marker write behind would
		// make the next inventory correctly fail closed until an operator removed a
		// jail that never held any allocation.
		removeErr := j.remove()
		if removeErr == nil {
			removeErr = p.syncDir(filepath.Dir(j.dir()))
		}

		return errors.Join(claimErr, removeErr)
	}

	return nil
}

// syncDirectoryAncestors persists every parent edge to the filesystem root. An
// existing directory is not a durability boundary: another process may have made
// its entry visible without syncing the parent yet.
func (p *Provider) syncDirectoryAncestors(path string) error {
	for current := filepath.Dir(path); ; current = filepath.Dir(current) {
		if err := p.syncDir(current); err != nil {
			return fmt.Errorf("firecracker: sync the ownership path through %s: %w",
				current, err)
		}
...
func (p *Provider) writeOwner(j jail) error {
	marker, err := os.OpenFile(j.ownerFile(), os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0o600)
	if err != nil {
		return fmt.Errorf("firecracker: record which deployment owns %s: %w", j.dir(), err)
	}

	if _, err := marker.WriteString(p.owner + "\n"); err != nil {
		return errors.Join(fmt.Errorf("firecracker: write which deployment owns %s: %w", j.dir(), err),
			marker.Close())
	}

	if err := p.syncFile(marker); err != nil {
		return errors.Join(fmt.Errorf("firecracker: sync which deployment owns %s: %w", j.dir(), err),
			marker.Close())
	}

	if err := marker.Close(); err != nil {
		return fmt.Errorf("firecracker: close the owner marker for %s: %w", j.dir(), err)
	}

	if err := p.syncDir(j.dir()); err != nil {
		return fmt.Errorf("firecracker: sync the owner marker in %s: %w", j.dir(), err)
	}

	if err := p.syncDir(filepath.Dir(j.dir())); err != nil {
		return fmt.Errorf("firecracker: sync the jail entry for %s: %w", j.dir(), err)
	}
...
func (p *Provider) writeResources(j jail, res resources) error {
	...
	if err := staged.Chmod(0o600); err != nil {
		return errors.Join(fmt.Errorf("firecracker: set the mode of the resources of %s: %w", j.id, err),
			staged.Close())
	}
	...
	if err := p.syncDir(j.dir()); err != nil {
		return fmt.Errorf("firecracker: sync what %s was allocated: %w", j.id, err)
	}
...
func parseOwnerRecord(raw []byte) (string, error) {
	if len(raw) < 2 || raw[len(raw)-1] != '\n' || bytes.Count(raw, []byte{'\n'}) != 1 {
		return "", errors.New("deployment owner record is not one newline-terminated identity")
	}

	owner := string(raw[:len(raw)-1])
	if err := deploymentid.Validate(owner); err != nil {
		return "", fmt.Errorf("deployment owner record: %w", err)
	}
