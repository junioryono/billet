...
const (
	...
	standInEnv = "BILLET_STANDIN_VMM"
	// standInReadyEnv names the file the child creates only after its signal policy
	// is installed. Seeing argv in /proc does not establish that readiness.
	standInReadyEnv = "BILLET_STANDIN_VMM_READY"
	// standInDelaySignalEnv makes the pre-handler race wide enough that deleting the
	// readiness handshake reliably breaks the escalation tests.
	standInDelaySignalEnv = "BILLET_STANDIN_VMM_DELAY_SIGNAL"
)
...
func TestMain(m *testing.M) {
	mode := os.Getenv(standInEnv)
	if mode == "" {
		os.Exit(m.Run())
	}

	if mode == "ignore-term" {
		if os.Getenv(standInDelaySignalEnv) != "" {
	...
	ready := os.Getenv(standInReadyEnv)
	if ready == "" {
		os.Exit(2)
	}
...
func standIn(t *testing.T, jailID string, ignoreTerm bool) int {
	...
	cmd.Env = append(os.Environ(), standInEnv+"="+mode, standInReadyEnv+"="+ready)
	if ignoreTerm {
		cmd.Env = append(cmd.Env, standInDelaySignalEnv+"=1")
	}
	...
	// ESTABLISHED BEFORE THE TEST USES IT. Seeing the argv in /proc proves identity,
	// not that TestMain has installed the requested signal policy. Both facts must
	// hold before teardown is allowed to send its first signal.
	//
	...
	for time.Now().Before(deadline) {
		...
		if owns, err := pidIsVMM(pid, jailID); readyErr == nil && err == nil && owns {
			return pid
		}
	...
	// AND IT SAYS WHAT IT ACTUALLY SAW. "never became ready" is not something anyone
	// can act on from a CI log; the argv, readiness marker, and read failures are.
	raw, err := os.ReadFile(procCmdline(pid))
	_, readyErr := os.Stat(ready)
	t.Fatalf("the stand-in vmm never became ready in /proc carrying %q: cmdline %q "+
		"(read error: %v, readiness error: %v)", jailID,
		strings.ReplaceAll(string(raw), "\x00", " "), err, readyErr)
