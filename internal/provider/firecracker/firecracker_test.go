...
func TestABinaryTheJailerWouldRefuseIsCaughtAtConstruction(t *testing.T) {
	...
	_, err := New(testDeployment, config.FirecrackerConfig{
		BinaryPath:   renamed,
...
func TestAVMMThatDiesOnStartIsAFailedLaunchEvenThoughTheJailerExitedZero(t *testing.T) {
	...
	if _, err := os.Stat(h.p.lifecycleFile(theInstance)); !os.IsNotExist(err) {
		t.Errorf("the fully unwound launch left its lifecycle permanently reserved: %v", err)
	}
}
...
func TestAnotherDeploymentsMicroVMIsInvisible(t *testing.T) {
	...
	stranger, err := New(otherTestDeployment, h.p.cfg, other)
	if err != nil {
...
// A JAIL WITH NO MARKER MAKES THE INVENTORY UNKNOWABLE. Ownership is the authority
// for deciding whose compute this process may act on, so a crash before the durable
// marker leaves work for an operator rather than permission to adopt the directory.
func TestAnInterruptedLaunchIsStillReported(t *testing.T) {
	...
	if _, err := h.p.List(t.Context()); err == nil {
		t.Fatal("List reported an authoritative inventory while a jail had no owner marker")
	}
}
