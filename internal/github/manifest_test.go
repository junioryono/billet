...
func TestPermissionsAreMinimal(t *testing.T) {
	...
	got := Permissions(ScopeOrganization, false)
...
func TestPermissionsReturnsACopy(t *testing.T) {
	Permissions(ScopeOrganization, false)["contents"] = "write"

	if _, leaked := Permissions(ScopeOrganization, false)["contents"]; leaked {
...
func TestManifestCarriesAWebhookURLEvenThoughItIsInactive(t *testing.T) {
	m := NewManifest("billet", "http://127.0.0.1:1/callback", "http://127.0.0.1:1/installed", ScopeOrganization, false)
