...
and the fallback reader judges a record's takeover chain by it, because a
receipt or a record the command wrote carries a holder it accepted, and a
character class approximated on its own admitted what the command refuses
(a count of characters rather than bytes, U+FFFD, the C1 controls) or refused
what it accepts (a format character such as U+200B, which Go's IsControl
does not name). THE ONE PREDICATE lives here, imported by the filter and by
the module, and is held to `tests/fixtures/holder-vectors.json`, written by
the Go test from `checkHolder`'s own verdicts.
"""
