...
retirement_current=
: >"$work/retirement-selection.tsv"
retirement_section() {
  if [ -z "${BILLET_RETIREMENT_SHARD:-}" ] || [ "$BILLET_RETIREMENT_SHARD" = "$1" ]; then
    retirement_sections=$((retirement_sections + 1))
    retirement_current=$1
    return 0
  fi
  return 1
}
# One record per case; a second invocation inside one case stays in that case.
# The shared machinery calibration is deliberately outside this accounting.
retirement_case_seen() {
  [ -n "$retirement_current" ] || return 0
  printf '%s\t%s\n' "$retirement_current" "$1" >>"$work/retirement-selection.tsv"
}
retirement_section_finished() {
  "$python" "$repo_root/scripts/check-retirement-shards.py" --section "$retirement_current" --selection "$work/retirement-selection.tsv"
  sections_ran="$sections_ran, retirement $retirement_current"
  retirement_current=
}
...
# A second answer naming another guard's id, every other member intact: refused at the parser naming the id, no fact taken, and the cleanup releases with the first call's token.
plant b12-second-id-other
...
expect_calls b12-second-id-other managed "converge-guard release --holder h1 --cleanup --token" 1
...
if retirement_section parser; then
  . "$here/retirement-parser-cases.sh"
  retirement_section_finished
...
if [ -n "${BILLET_RETIREMENT_SHARD:-}" ] && [ "$retirement_sections" -ne 1 ]; then
  fail "retirement shard selected $retirement_sections sections, expected exactly one"
fi
