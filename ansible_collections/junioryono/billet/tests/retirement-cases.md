...
The complete-classifier floor is `v0.11.0`, the next minor cut from `main`; `v0.10.1` remains an incapable maintenance-release control. The caller accepts one UTF-8 YAML mapping with string mapping keys, no anchors, aliases, merge keys, explicit tags or duplicate keys anywhere, and non-empty absolute string locators without padding. It validates both supplied locators before selecting identity_dir over state_dir. A null state block is absent; a non-null state block supplies no default, but inherits a supplied state_dir just as applyStateDefaults does before semantic validation. The caller uses the packaged identity path for a positively absent configuration or a server with neither locator and no state block; undecodable bytes or an unestablished locator hold by name. The latter fallback is the requested packaged compatibility policy, not Go's account-dependent defaultStateDir, which uses os.UserConfigDir and falls back to .billet/server if that lookup fails. This deliberately tightens revision 12's undecodable-config fallback in the caller; the Go classifier's accepted custom-locator residual is unchanged. The Go classifier's recovery reason says to retry classification, while revision 12's explicit caller boundary requires a fresh converge: this implementation ends the invocation and lets the next preparation/classification do that retry. The older `dry-run-request`, `dry-run-adopt` and `dry-run-unknown-ledger` filenames describe dispatch, not route; the command's route wins. No command behaviour was changed.
