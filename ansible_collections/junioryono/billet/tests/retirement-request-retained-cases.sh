...
#
# WHAT THESE CASES PROVE AND WHAT THEY DO NOT. The role is real, the files are real and the calls are real, but every collected report is a committed corpus fixture a fake answerer replays. What these cases establish is the ROLE's construction — the variant it selects, the refusals it reaches before reserving, the operands it passes, the bytes it renders and the answers it accepts — not that a real deployment shaped this way is eligible.
#
# NO ROLE-GATE DOCUMENT IS ONE THE REAL COMMAND WOULD ACCEPT, and that is structural rather than an omission here. `normaliseReport` redacts every 64-hex string in the corpus to a placeholder so the fixtures stay stable, while the role computes `desired_nodes[host].sha256` from the bytes it actually rendered; `judgeNodeChain` requires the receipt digest, the inspected installed digest and the desired digest to agree, and a redacted digest can never equal a real one. This is true of `node-a` and of the server-only request cases too, and has been since before the retained ones existed. Supplying a both-role inspection fixture would make this host's other members realistic and would not change that, so the composition of a real inspection with the real command belongs to commit 5's fleet acceptance BY CONSTRUCTION, not because anything here was left undone. The command's own judgement of a whole retained document, digests included, is proved separately and with real hashes by `TestServerRetireRequestStagesARetainedNodesRendering`.
