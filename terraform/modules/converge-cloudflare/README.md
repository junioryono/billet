...
`cloudflared` runs with the `tunnel_token` output, installed by the collection's `junioryono.billet.cloudflared_connector` role, which the shipped fleet playbook runs on every host. Export the token as `BILLET_CLOUDFLARED_TOKEN_<HOST>` (the inventory hostname upper-cased, everything outside `[A-Z0-9]` an underscore) in the converge's environment, never as `-e`, and set `billet_cloudflared_expected_tunnel_id` to this module's `tunnel_id` output so a token for another tunnel is refused before anything is installed. The role writes `/etc/cloudflared/token` and the unit itself rather than running `cloudflared service install`, whose only interface for the token is argv.
