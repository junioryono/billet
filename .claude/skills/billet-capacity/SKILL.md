...
- **An EC2 lease is charged for the SHAPE, not the tier request.** The node registers its ordered `instance_types`; placement picks the first fitting one and stores both the immutable requested resources and the charged shape on the lease. A fallback is a new purchase decision and must resize the charge atomically against both the node and deployment ceilings before `RunInstances` is sent. A budget refusal may try another declared shape; it may never reach AWS first and reconcile the overspend later.
...
- **EC2 prices report exposure; they do not gate admission.** The ordered shape allowlist plus node and deployment resource ceilings are the enforceable policy. `billet check` bounds one configured node; `billet status` bounds the registered EC2 fleet by the tighter of the shared deployment ceiling and the sum of node ceilings, including disconnected registrations because their instances may still be billable. AWS Budgets is the account-wide backstop.
