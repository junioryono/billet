...
**A message session is single-holder, GitHub will not hand it over, and the wait was measured once as still refused at 60 seconds and open at 91.** Opening a second session for a scale set whose first was abandoned answers `409 Conflict … RunnerScaleSetSessionConflictException … already has an active session`, measured against a real organization, and measured again on 2026-09-04 under a DIFFERENT owner with the same refusal: the conflict is the scale set's, not the holder's name, so a promoted standby on another host waits exactly as a restart on the same host does. `openSession` waits on `ErrSessionHeld` (`sessionRetryFor`) and returns every other reason at once; a control plane killed and restarted used to not start at all. Nothing is lost while it waits: GitHub queues a job for 24 hours with no runner available. The abandoned session was still refused at 60 seconds and open at 91 (`TestLiveSessionReplacement`, 30-second polls, so the expiry is bracketed rather than named), which is a measurement rather than a promise, so the wait stays a loop rather than a sleep.
...
- An abandoned session: still refused at 60 seconds, successor open at 91 (2026-09-04, 30-second polls, so the expiry is bracketed rather than named).
