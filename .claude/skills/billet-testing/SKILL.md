...
## Proving the mechanism is not proving it is USED

Three instances of this in one day, across two sessions, all the same shape: the test exercises the production mechanism correctly **in isolation**, and nothing ties it to the call site that is supposed to use it. Delete the wiring and the test stays green.

| What was tested | What was not |
|---|---|
| `printAdmission` renders a seal correctly | that `billet status` ever calls it — the whole block could go from `cmdStatus` |
| the AMI stamp helper produces the right stamp | that anything invokes it |
| `StopAndProve` refuses a unit that came back | that the refusal is production's — the test **fake** carried its own copy of the guard, so deleting the real one left it green |
| `TidyStagedRegistration` deletes a consumed registration | that anything calls it — its own doc comment said the node's tending sweep did, and only tests ever had; every lease that settled without a `Destroy` leaked its credential |
| an unreadable admission state sets `quiesced` | that the loop survives it, or that the journal line changed — the assertion re-checked `isQuiesced()` while its comment claimed to check the log |

**Drive the caller, not the helper.** If the property is "status reports the seal", call `cmdStatus`. If it is "the loop keeps polling", drive `Run`. A helper test is fine as well, never instead.

**A fake must not reimplement the production rule it is used to test.** That one is the sharpest, because it looks like faithfulness: the fake mirrors the real converger's refusal, the command test asserts the refusal, and the assertion is about the fake. Test the rule where it lives, and let the fake return what it is told and judge nothing.

**And a duplicate assertion dressed as a new one proves nothing twice.** Checking `isQuiesced()` under a comment that says "and it said so honestly" is not a log assertion. When a comment names a property, make sure the line under it can fail for that property alone.
...
- **One timeout knob must never serve both a deadline that must EXPIRE and deadlines that must NOT.** There is no value that satisfies both, so such a test is not tuned but wedged between two failures: small enough that it does not sit waiting out the first, and therefore small enough that one descheduling under `-race -covermode=atomic` expires the others. `TestImportGenerationConfirmsRemovalAfterCallerAndRemovalDeadlinesExpire` set `c.wait` to 20ms so its fake could wait out a `snap rm`, which left the absence check and the metadata cleanup that follow bounded by that same 20ms — two distinct CI failures, both measured by injecting a 30ms stall before either step. The same shape, a wall clock standing in for a property that was never about time, is what an earlier fix removed one file over.

  What replaces it is arithmetic on the deadlines the code actually handed out — the fake records the deadline of every invocation's context — and **`context.WithTimeout` on an already-bounded parent keeps the PARENT's deadline**, so an inherited context compares *exactly equal* where a fresh one does not. Success cannot distinguish those two at all; a comparison always can, on any machine under any load.

  But a bare "later than" is not enough, and a review round is what caught it: contexts constructed **up front**, before the operation whose budget they must outlive, also compare strictly later — by microseconds — while being just as exhausted in production as an inherited one. The separation has to be MATERIAL and it has to be created by the thing under test reaching the point in question, so the fake widens the client's command budget at the instant the removal is issued and the assertion is "at least half an hour further out", not "later". Nothing waits on that hour; it is only ever subtracted. And ask for each call **by name** — the ceph rollback attempts two metadata keys, and a lookup that took the first match passed a mutant that cleaned one on a fresh bound and the other on the dead one.
