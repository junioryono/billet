...
The combined `junioryono/billet/actions/build-push-action@v0.1.0` wrapper is available for common `docker/build-push-action` inputs and follows an exact reviewed commit of upstream v7 rather than vendoring its implementation. Its default BuildKit image is pinned to an immutable multi-platform digest too. Reference it as `@v0` to follow Billet's releases, or as an exact tag or commit SHA to pin; [Action versioning](../../docs/reference/action-versioning.md) explains the tradeoff. Every sibling reference resolves to the same Billet whichever you pick, because the release workflow rewrites them to the tag being cut before it creates that tag, while `main` deliberately composes the current `main` actions. Use the setup action directly when an upstream input is not exposed by the wrapper.

BuildKit garbage collection keeps records unused for eight days and constrains the 100 GiB default disk to 80–90 GiB of retained state. The tier's `buildkit_cache_mount_limit` independently caps each `RUN --mount=type=cache` record; when a tier omits it, the action uses a 20 GiB per-mount ceiling. The post hook reports each mount's exact size and growth, resets only a mount that exceeds the ceiling, and discards the whole publication if the ceiling cannot be enforced. Set `reset: true` on one trusted run to replace a poisoned or bloated key with fresh BuildKit state.
