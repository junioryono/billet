...
6. Whatever happened, and before the credentials are shredded, releases the converge guard this run holds, over the same inventory and `--limit`. A host held by another name is left alone and reported; a host holding nothing is not an error; a run that never reached a host releases nothing. A guard nobody releases stands until a person removes it, and every later converge and every rollout dispatch on that host refuses while it does.
7. Whatever happened, removes the key files it wrote and, only when this run enrolled a WARP device, deletes that registration and its `mdm.xml`; with `reach: none` nothing WARP-related is touched.
