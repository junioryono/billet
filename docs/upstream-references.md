...
- `buildkite/buildkite-gha` at `027892a251938f3dc561a1ccf9339af736741fe7` (MIT) — a P4 cache-conformance reference, not an execution model. It replaces GitHub's runner with a Buildkite workflow compiler and compatibility runtime, so billet takes none of its parser, expression evaluator, action downloader, native checkout/artifact adapters or credential architecture. The useful overlap is narrower: its cache integration catalog identifies `actions/cache` plus the cache clients embedded in `actions/setup-node`, `actions/setup-java`, `actions/setup-python`, `actions/setup-go` and `actions/setup-dotnet`; its tests exercise representative Node 20 and Node 24 clients, exact known-good action commits, fresh credentials and workflow-controlled environment poisoning. Billet keeps the official runner, authenticates interception by VM identity, and must pass through every unknown method, so the action allowlist and per-lifecycle credential model do not transfer. What does transfer to #29 is the conformance surface: direct and embedded cache clients, a pinned-known-good lane beside a moving-major lane, and hostile `ACTIONS_*`, Node, CA and proxy variables that must degrade to a cache miss without touching artifact traffic or failing the job.
