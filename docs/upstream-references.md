...
- GitHub REST API `2022-11-28`, organization self-hosted runners — the documented list response supplies the exact `id`, `name`, `status`, `busy`, and `ephemeral` fields billet needs to recover quarantined compute whose registration predates the durable pool journal. Billet requests the exact deterministic name, requires a complete bounded response and one exact match, refuses non-ephemeral or contradictory state, preserves `online` plus `busy`, and removes an idle or offline registration by the exact observed ID before compute teardown. An absent registration is safe to retire, but the atomic decision still creates a durable retiring fence so a delayed `JobStarted` cannot recreate it after recovery returns; any API, pagination, identity, or status ambiguity keeps the quarantined lease charged. Source: [GitHub REST API endpoints for self-hosted runners](https://docs.github.com/en/rest/actions/self-hosted-runners?apiVersion=2022-11-28).
