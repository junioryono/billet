...
### One claim, one lifecycle, for converges and transactions

A converge driven by the Ansible role and a rollout's host transaction are two writers of the same host, and before this decision nothing excluded one from the other: the role's transaction and `billet host-upgrade` each held the transaction lock for their own lifetime, and a converge that was not itself a binary transaction held nothing, so a rollout could replace the binary under a play that was rewriting units around it. The exclusion is the claim `active` itself, whose shape says what holds the host: a symlink is a Go transaction, a regular file a pre-R role transaction, and a directory a converge guard (`guard.json` inside it naming the holder, the time, the host and the release executable the converge runs, by path and digest). A directory, because every reader that predates this decision refuses a claim it cannot read as a symlink and none of them can release it, so the new shape is safe against every old binary that can be dispatched onto a host. Publication and release are each one durable order (tmp, fsync, rename, directory, root, the root's parent having been flushed by the acquisition that created the root; unlink, directory, rmdir, root), and a hold by the holder that already holds writes nothing and flushes the directory and the root again, which is what a retry of a hold interrupted between its rename and its flushes owes, so every interruption leaves a state a reader classifies, and the classifications are values, never guesses: a directory with a record is a guard, one without is an unpublished hold with its own recovery. Nothing expires a guard, because a timer that released one would release it under a converge that is merely slow; an operator's assertion that the old driver is stopped is what removes one, and a takeover keeps the role's transaction pointer so the role's own recovery finishes what it started. Every transaction entry classifies the claim immediately after the lock, before the platform check and before the network, and a guarded host's refusal is the retryable kind the coordinator already has. The role's half is one task file, `prepare-exclusion.yml`, run before every other task of the host role and as the first pre-task of every play of the fleet playbook, and it is a thin caller of ONE COMMAND UNDER ONE LOCK, `billet converge-guard prepare`, because every window the earlier role-side design left was one between two separately locked commands (ownership inferred from a status read before the hold, a pointer appearing between them, two invocations under one holder, an answer lost after publication): the command classifies the claim, validates the pointer, verifies the recorded executable, acquires or validates the guard, judges a staged candidate (the capability floor, the downgrade, the intent against the record) and re-binds the record to it, all under the transaction lock, and answers one JSON object the role reads through one parser. The role HOLDS FIRST (the first call, `--validate`, before anything is decided or staged) and stages under the guard; only a bootstrap and a pre-R managed binary with a candidate hold through the staged candidate after the staging, each a disclosed window. The record carries a public `id` (continuity across a run's inclusions, `--expect-id`, refused under the lock when the guard is gone or replaced), the acquiring invocation's `token`, and a `preparing` flag that `settle` clears when the preparation returns, so the role RELEASES only what this invocation acquired and only before it changed anything (`release --cleanup --token`, refused for a settled guard, a wrong token or a pointer), and the operator's `release` and `recover` stay the ways out otherwise; its transaction publishes its pointer inside the guard rather than replacing the claim. A candidate that cannot answer `converge-guard status` cannot take part in the exclusion and is refused at the floor, which is the one change this decision makes to what an older release's collection can install. Every JSON the role consumes has fixtures written by the Go test that produces it (`tests/fixtures/guard-prepare`, `guard-status`), and the collection's gate runs the real binary at its real paths inside a per-case mount namespace. The trust boundary of the upgrade root is proved through descriptors before any of it, because a symlink at `active`, at the lock or at the root would otherwise make a name checked a different thing from a name used.

### The guard beside its ancestors

Three mechanisms in public use do part of what the guard does, and their vocabulary was compared with the guard's before release R freezes `guard.json` and the answer schema. Zincati's FleetLock protocol (the reboot lock behind Fedora CoreOS updates) has a client id that persists across restarts, an "initialization releases whatever this id held" rule, and a bounded set of error kinds; the guard's `holder` is that id (persisted by the driver that exported it, and the same-holder rule is FleetLock's re-acquisition), the acquirer's `token` and `preparing` window are what FleetLock has no need of because its clients are one process each, and the refusal `reason` codes are the bounded error kinds. Puppet's `agent --disable "<reason>"` lock and balena's update locks each carry a free-text reason written by whoever took the lock, and it is what an operator reads first on a held host; the guard's record gains it as `note`, written at acquisition and never rewritten, named `note` rather than `reason` because every answer of the preparation already carries `reason` as the refusal's code and a record member of the same name would read as one. balena's locks are released by a one-shot `force` on the update that would otherwise wait, and the guard keeps the operator's `--old-driver-stopped` assertion instead of a sticky override or a one-shot force: a force is a decision about one update, and the guard's removal is a decision about whether a driver still exists, which only the operator can know and which no later update should inherit.

### A node's endpoint moves as one proved sequence

A running node dials the request base its client normalised at startup, so a configuration change to `node.server_addr` is not proved by the ordinary restart's stability samples, and what a node dialled before it published a record is not reconstructible. The migration is decided by a dry run over the running node's own record against the rendering (the record written after every successful registration under a `RuntimeDirectory` the node unit alone declares, so a colocated server's restart never re-owns it), the render installs the configuration, and the action stops the node under the migration's own deadline (signalling only its own `systemctl`, because a stop job systemd began is neither cancelled nor restarted by that), proves the stop by the triple `inactive`/`dead`/`Result=success` rather than by the stop's exit (systemd's own timeout completes a stop job as done), starts, waits for a record under the new invocation, and asks the controller for the registration under the incarnation that record names; the receipt binds the invocation, the incarnation, the installed digest and the endpoint installed and effective, and every ordinary converge refreshes it, so an operator restart or an unrelated configuration change never leaves a stale proof. The commands are `billet` subcommands with the Go harness's seams and hooks, and the role is a thin caller that orders them, because every window the earlier role-side design left was one between two separately observed facts in an eight-hundred-line task file.
...
**One thing is no longer assumed.** Whether GitHub redelivers an unacknowledged message to a new session after a controller restart was undocumented and unmeasured until `TestLiveSessionReplacement` ran against a real organization on 2026-09-04. It does: a session holding one `JobAssigned` message was abandoned without being closed, and the successor was handed the same message id back on all eight runs, with the same one assigned job — the run that compared identities rather than counts found the same run id and the same stable workflow-job id. A successor cannot open while the abandoned session is outstanding, under the old holder's own name or under another host's, so the wait is the recovery path either way; the abandoned session was still refused at 60 seconds on each of eight runs and open at 91 on three of them and 92 on the other five, which brackets the expiry rather than naming it. Every recovery path stays written to be safe whether or not a message comes back, because idempotence is what makes a redelivery harmless rather than what makes it necessary, and `TestARedeliveredAssignmentDoesNotStartASecondRunner` pins that a redelivered exchange after a restart starts no second runner. The crash-point suite still asserts what the *ledger* holds rather than what GitHub does.
