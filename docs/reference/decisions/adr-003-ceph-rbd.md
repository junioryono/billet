...
Status: accepted. Supersedes the ZFS layout in `docs/reference/reference-hardware.md`; the `Store` interface, the commit protocol and eviction are covered separately.
...
The reason is one sentence long. **A ZFS clone exists only on the machine that took it**, so a cache written to one belongs to that host and to no other — which is the storage half of billet being a one-machine product. The capacity, placement and teardown halves were fixed by per-node capacity, admission-time placement and addressed teardown; this is the last one. RBD presents the same drives as a pool any node at the site can map, and it solves golden-image *distribution* for free: a new machine maps the same base image instead of having a copy shipped to it.
...
Ubuntu 26.04 LTS, kernel 7.0.0-29-generic, on the reference host in `docs/reference/reference-hardware.md`.
...
## The four questions this ADR was opened to answer
...
The original proposal assumed a single-host cluster forces `size=1`, and called that a real data-loss window for golden images. It does not. **CRUSH's failure domain is configurable, and `cephadm bootstrap --single-host-defaults` sets it to the OSD** rather than the host:
...
`rbd snap create` → `snap protect` → `clone` → `map` is the recipe in Ceph's own docs and in the original proposal. **On a cluster as `cephadm` leaves it, the protect step is mandatory — and it makes eviction impossible.** Measured, in that order:
...
The parent snapshot is removed on request, the clone keeps reading it correctly, and the space is reclaimed when the last child goes. **Clone v2 is a requirement of billet's storage, not a preference**, and eviction rests on it. `mimic` is the minimum that enables it; anything newer would exclude clients for no gain.

There is a second deletion boundary: clone v2 lets the snapshot disappear while a child reads it, but `rbd rm` still refuses the parent image with `image has snapshots with linked clones`. Cache publication therefore moves the retired writable parent to RBD trash after cloning the immutable candidate. The candidate stays copy-on-write, the old pointer is never mutated, and cache eviction removes only trash entries whose original names begin with Billet's `cache-v-` prefix once their last child is gone. An expired generation that still parents a newer generation is deferred with its metadata intact, and eviction makes bounded dependency passes until every now-unblocked image and retired writer is gone. A cache that remains active needs one more boundary because every warm publication adds two links and would otherwise retain its entire history: before the next link would cross depth eight, Billet copies the verified staging snapshot directly into a new independent candidate, snapshots that complete copy, and only then retires the writable source. A generation made before lineage metadata existed is copied before Billet creates another writable child, because an unknown legacy depth must not be guessed below the hard limit. The published pointer is unchanged across either copy, so this is periodic chain compaction rather than the forbidden operation of flattening the candidate in place. The `rbd cp snapshot independent-image` behavior and absence of a parent in the copied image's JSON were measured against the reference cluster; the depth-eight bound matches Ceph CSI's documented hard maximum, while copying on every commit is deliberately not the protocol.

That bounded copy changes the request budget on both paths that can invoke it. Attach and commit each give storage twelve minutes and fifteen seconds of work inside a twelve-minute-and-forty-five-second handler envelope, reserving thirty seconds for cancellation-resistant RBD cleanup and lock release; the action waits thirteen minutes, and a commit's writer authority lasts fifteen. The cache-index lock may use twelve minutes of that work budget to recover a holder that leaked when the request began, including a thirty-second completion margin after its stale age and heartbeat observation. A canceled attach removes any partially copied image through a fresh bounded cleanup context instead of accumulating a new large orphan on every retry.

A writable clone remains eviction-safe even when a job runs beyond the seven-day inactivity window: an unexpired active lease preserves both its source generation and the pointer the eventual publication expects, mapping protects the writable child while the guest owns it, and commit refreshes that child's last-use metadata under the cache-index lock immediately before unmapping, so the copy and snapshot interval cannot turn an active update into an expired orphan.
...
**What still has to be established on the pilot host** is narrower than it was: not whether RBD is viable for build workloads or how a guest receives one. Firecracker now reserves five drives, the node hot-attaches a mapped clone through an instance-bound token, the action formats cold volumes as ext4 and mounts them, and its post hook unmounts before the node verifies and publishes. The remaining evidence is whether the *hydrate* step of a real `node_modules` volume stays acceptable at 4× on that host, plus the power-cut loop that proves the previous pointer stays mountable across real kernel and Ceph failure boundaries.
...
Both directions, on a clone neither machine created alone. **ZFS cannot do this at all** — a `zfs clone` is a dataset on one pool on one host, and there is no operation that hands it to another kernel. That difference is the whole of this decision.
...
`internal/store` now makes a cache volume concrete: a writable per-job clone has an opaque backend handle and a mapped host device; a candidate is a separately named immutable snapshot whose ext4 type, UUID and clean filesystem check were proved before publication. The node, not the control plane, owns that transition because it is the only process that can quiesce the guest and snapshot the device it holds.

Publication is not last-writer-wins disguised as locking. A writer first acquires a cache-specific lease and monotonically newer fencing token under the cluster's cache-index lock. `PublishCAS` rechecks the lease, expiry, fence and expected generation under that same lock, proves the candidate snapshot still exists, records the generation, and writes the `(site, key) → generation` pointer last. Every earlier crash leaves the previous pointer intact and at most an orphan candidate; eviction removes those after the inactivity window. Capacity leases are deliberately not reused: capacity remains held until compute is proved gone, while cache-writer authority exists only across quiesce, verification and pointer commit.

The cache-index lock has a ten-minute stale age chosen for its metadata-scale critical sections rather than inheriting the golden-image publisher's six hours. Ordinary same-node and cross-node contention is serialized by bounded, context-cancellable retry: a dated holder inside the stale bound is never observed or broken, and the waiting operation re-evaluates it after a short delay rather than turning a healthy one-second critical section into a cache miss. Age never authorises a break by itself: after the bound a contender still observes the holder's heartbeat for ninety seconds, and any movement preserves a live holder even during a longer eviction pass. A crashed, silent holder is therefore reclaimable by the first retry after ten minutes plus that observation window without manual intervention, while a lock whose cookie cannot be dated is refused rather than retried or broken. The destructive storage acceptance must cut power while this lock is held, keep retrying through the bound, prove the prior or complete new pointer remains valid, and observe cache service resume after automatic reclamation.

An active-clone lease is recorded before the cluster lock is released and before the clone is attempted. Eviction protects the current pointer and every generation named by an unexpired active lease, removes expired lease records, refuses to touch a still-mapped device, and deletes only billet's `cache-g-*` and `cache-v-*` namespace. A clone miss is `store.ErrMiss`: another site having the key does not change that answer, and a cold cache never fails a job.

## What is still open

- **The cluster is degenerate on one host and always was.** One mon is not a quorum. The mgr has a standby, the OSDs mirror across two drives, and the machine is still a single point of failure. This is the accepted cost recorded in the original proposal, restated here because "HEALTH_OK" makes it easy to forget.
- **A microVM now boots from a mapped root clone and can hot-attach five cache clones.** The root path was verified on the reference host, launched and destroyed leaving no clone, no mapping, no jail and no tap. The cache path reserves placeholders before boot, replaces one only after a token-bound request from that guest, and restores the placeholder before the store snapshots or unmaps the device. Two root-path measurements still govern it. **Mapping is not idempotent**: a second `rbd device map` of one image creates a SECOND device rather than returning the first, so a retry accumulates devices instead of converging — and `rbd device unmap <spec>` on an image mapped twice unmaps only one, which is why teardown reads the mapping table and unmaps by DEVICE. And **the exit codes are not what a first pass recorded**: reading `$?` after a pipe took `tail`'s status and made three of them look like zero; measured without one, removing an absent image exits 2, unmapping an unmapped spec exits 22, and cloning onto an existing name exits 17. Only the first two mean "already done".

  The storage layer now supplies the `Store` interface, commit protocol and eviction, and the sticky-disk work wires those mapped devices into the five placeholder drives Firecracker permits a cooperative guest to replace after boot. The question it inherits is no longer whether RBD can carry a build — it can, at 2% — but whether hydrating a real `node_modules` at 4× is better served by walking it as files or by streaming the volume.

- **An image reference must name a snapshot outright**, written `image@snapshot`. Choosing a generation for a caller who names only an image is exactly what the storage layer's generation selection is for, so the boot path refuses to invent a rule that would have to be unpicked when it arrives. That is why the example config's tiers now read `ubuntu-2404-x64@g1`.
...
- **The cache pool has no quota.** Inactivity eviction now exists, but it is not a hard byte ceiling: enough live or recently used caches can still fill the pool. `ceph osd pool set-quota billet-cache max_bytes <n>` remains the operator's hard stop.
