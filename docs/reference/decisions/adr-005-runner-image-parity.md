...
- **The Android SDK: decided 2026-09-24, in the published guest image.** The maintainer chose to carry the SDK in the guest image billet publishes, as GitHub's and Blacksmith's images carry it, after an Android build moved onto the fleet failed with "SDK location not found" (#209). A node-side install, which would have kept the published image free of it, was considered and set aside for the simpler shape. The consequence is stated rather than glossed: billet's public guest release now redistributes Google's Android SDK, whose terms restrict redistribution. `scripts/build-guest-image.sh` accepts the licences at its toolcache call, and `check-guest-image.sh`'s `check_android_sdk` refuses an image without it. Chrome and Edge are still undecided.
