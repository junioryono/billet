...
- **Tools GitHub installs outside its declaration are named one by one, and the GitHub CLI is the one that cost a fleet.** `toolset-2404.json` is most of GitHub's image, not all of it: some tools are installed by a script of their own and never appear in the declaration, so building from the declaration cannot bring them and the declaration-driven gate cannot notice them missing. `gh` is carried that way, in billet's own package list in both backends, and checked by name: by the guest's contents gate and by the AMI build executing it. `apparmor` and `python3-apt` are also on GitHub's image and outside its declaration; the guest names both in its own list, the AMI builder names neither and gets both from Canonical's Noble cloud image (both are in its published manifest, read 2026-09-22), and no gate names either. `gh` was found missing by a workflow on 2026-09-22: its first `gh` call exited 127, in a job that runs only after a merge, so no pull request's run had exercised it. GitHub installs the **latest `cli/cli` release** `.deb`, verified against that release's `checksums.txt` (`images/ubuntu/scripts/build/install-github-cli.sh`, read 2026-09-22); billet installs **Ubuntu's `gh` package** instead, signed by the archive key and installed in the one apt transaction, for the reason Compose and Buildx come from the archive — a hand-pinned binary is a second version to bump and a second digest to keep true. Ubuntu 24.04's package is far older than the release GitHub ships — 2.45.0 on a fleet guest against the current v2.101.0 on the date above — so a workflow depending on a newer `gh` subcommand will find it missing here; `gh attestation`, added in v2.49.0, is one. Closed on 2026-09-24 the way GitHub closes it: `install_github_cli` installs the current `cli/cli` release `.deb` over Ubuntu's, checked against that release's checksums file, and asserts the version it installed (#179).
...
- **The remaining heavyweight software** is now Swift, Rust, Haskell, and browsers with their drivers. .NET, PowerShell and its four modules, CodeQL, cmake, the compiler sections and the Android SDK with three NDKs have all landed, alongside the five JDKs with Ant, Maven and Gradle. Android is installed on both images: `BILLET_TC_ANDROID_ACCEPT_LICENSES` is set by the EC2 build and, since 2026-09-24, by the guest build. The gate reports what is missing rather than passing silently.
- **Licensing has not been resolved and is a human decision.** Chrome and Edge carry click-through terms. GitHub builds them into images it runs itself; billet would publish them as a downloadable release asset, which is a materially different act. This must be settled per component before that software is added, and the outcome recorded here.
- **What GitHub installs outside its declaration: first set landed 2026-09-24.** The rule is that a workflow moved onto billet changes its `runs-on` and nothing else, so everything GitHub's `ubuntu-24.04` installs with scripts of its own (read from `images/ubuntu/scripts/build/` at the pinned commit) belongs on the image too, and `check_hosted_tools` refuses an image missing any of it. The first set, in `install_hosted_tools`, on both backends: git from the git-core PPA with `safe.directory = *`, Git LFS, git-ftp, yq, kubectl, kind, minikube, Helm 3, Kustomize, Podman, Buildah and Skopeo (with GitHub's `registries.conf`), MySQL (root/root), Apache, nginx and the PostgreSQL server from PGDG (all stopped and disabled), PHP's extension set with Xdebug on and PCOV off, Composer, PHPUnit 8, the AWS CLI (its signature checked against AWS's key, which GitHub does not do), SAM, the Session Manager plugin, the ECR credential helper, Ninja, Bazelisk with `USE_BAZEL_FALLBACK_VERSION`, the runner's action archive cache, the gh-aw firewall bundle and Copilot CLI in the toolcache, GitHub's sysctl limits, the conntrack rule, github.com's SSH host keys from its API, and `::1 localhost`. Every download is checked against the vendor's published digest or signature and every vendor key against a pinned fingerprint, except Ninja, PHPUnit, the Session Manager plugin and the action archive, whose vendors publish neither. The second set, `install_hosted_languages`: rustup with stable, rustfmt and clippy for the runner; Swift, its signature checked against the nine keys GitHub's script trusts; GHCup with the latest GHC, Cabal and Stack; Kotlin; Julia, checked against the digest its own `versions.json` publishes (GitHub checks none); Miniconda, checked against the digest Anaconda lists; vcpkg; Homebrew, owned by the runner and off PATH as GitHub leaves it; nvm with `system` as its default; and the runner's PATH with GitHub's directories in front (`~/.local/bin`, `~/.cargo/bin`, Composer's global bin, `/usr/local/.ghcup/bin`, `~/.dotnet/tools`). What GitHub puts in `/etc/skel` goes into the runner's home, which exists before the installer runs on both backends. Still to come: the browsers with their drivers and the Azure and Google Cloud CLIs, AzCopy, Bicep, Packer and Pulumi. Ubuntu's zstd (1.5.5) stands in for GitHub's source-built 1.5.7.
- **The Android SDK: decided 2026-09-24, in the published guest image.** The maintainer decided the guest image carries the SDK, as GitHub's and Blacksmith's images do, after an Android build moved onto the fleet failed with "SDK location not found" (#209). `scripts/build-guest-image.sh` accepts the licences at its toolcache call, and `check-guest-image.sh`'s `check_android_sdk` refuses an image without it. Chrome and Edge are still undecided.
