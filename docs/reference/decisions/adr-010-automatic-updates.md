...
**Measured 2026-09-05: none of the downgrade refusals had ever held on a release binary.** The release build stamps `0.9.1`, without the tag's `v`; `version.Compare` accepted only `vX.Y.Z`; so `checkDowngrade`, the timer's "older than the release running here" refusal, the starter's own and this watermark's ordering were each "could not tell" against a real release, and the watermark was never even recorded, since recording is gated on the same grammar. A control plane an operator had moved to v0.9.1 obeyed a stale rollout to v0.9.0 the same afternoon. v0.9.2 spells a release as its tag everywhere and orders both spellings, and the starter finishes a rollout the control plane has been moved past as superseded rather than leaving it to be obeyed.
