...
| CA rotation | every state of the file machine under test; enrollment across a rotation in the end-to-end suite; on packaged hosts under real systemd against a real App: `ca rotate`, both nodes renewing onto the new authority on their own sweep 787 seconds after the rotation (twenty-minute leaves), `ca retire --force`, both nodes re-registering with a plane that trusts only the new authority, 905 seconds end to end (2026-09-04, `make ca-rotation-rehearsal`, [Host rehearsals](records/host-rehearsals.md)) | a rotation across a fleet whose leaves are a year long, where renewal is a matter of months |
