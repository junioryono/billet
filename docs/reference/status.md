...
| Rollouts | the coordinator's ordering, cohorts and failure budget under test; the upgrade transaction's ordering asserted structurally; every corrupt, unsigned, expired or incompatible manifest refused; two packaged hosts under real systemd reached `billet rollout start` and its refusal found that no binary through v0.6.0 verifies a published manifest ([Host rehearsals](records/host-rehearsals.md), 2026-09-04) | `cmd/billet`'s real host actions (stop a service, replace a binary, migrate) on a real host: reached on 2026-09-05 from v0.8.0 to v0.9.0 and found two defects, a probe that neither side of the transaction would ever end and a fence cleared under a reason it was never written with, which together stopped every self-upgrade at the probe and would have cordoned every commit and rollback on a local ledger ([Host rehearsals](records/host-rehearsals.md)); fixed in v0.9.1, and the v0.8.0 to v0.9.1 run of 2026-09-05 proved the prediction: the probe passed in under a second and the v0.8.0 parent cordoned at commit on its own fence. The same afternoon a real control plane moved to v0.9.1 by the role was downgraded by its own timer obeying a stale rollout, and the downgraded binary was one the service account could not execute: no downgrade guard held on a release binary because releases reported a bare `0.9.1` that `version.Compare` refused to order, a running rollout was never superseded when the channel moved past it, and the updater's copy honoured its `UMask=0077` ([The stale rollout that downgraded a control plane](records/stale-rollout-downgrade.md)); fixed in the release after v0.9.1. For a controller with a local ledger it is provable only from a FROM at or above v0.9.1, because the FROM host is the one that runs the transaction and an older one cordons at commit; a PostgreSQL controller and a node-only host fence nothing and take the fixed release by rollout |
