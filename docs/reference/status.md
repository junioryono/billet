...
| Targets and repository scope | two targets served by one control plane in the end-to-end suite, each job registered through its own App; a repository-scoped App created through the manifest flow on a personal account, verified by `billet check`, its scale set created, a session opened, and both torn down through billet's own path (2026-09-04, `TestLiveRepositoryScope`, [ADR-011](decisions/adr-011-targets-and-repository-scope.md)) | a job run on a repository-scoped tier of a real fleet; the host role's per-target key install on a real converge |
