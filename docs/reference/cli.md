...
The upgrade root's trust boundary is proved before any of these takes the lock, through descriptors and never by pathname: `/var/lib/billet` (or `/usr/local/var/lib/billet` on a Mac) a directory owned by the account running the command and writable by nobody else, the root a directory owned by it and not writable by group or others, `active` 0700 and owned by it, `transaction.lock` a regular file, none of them a symlink; the root is opened relative to the parent's descriptor and the lock relative to the root's, each `O_NOFOLLOW`, so a name replaced between the check and the use is not the thing used.
