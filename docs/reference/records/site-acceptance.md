...
The control plane listens on `0.0.0.0:7717` under the node-wire CA with `node_tls_hosts: [203.0.113.10, 10.0.2.50]`; the home nodes dial the Elastic IP and `aws-1` dials the private address. Bundles were issued with `billet ca issue` on the control plane and copied to each host.
