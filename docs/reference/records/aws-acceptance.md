...
**Nothing runs it on a schedule.** `.github/workflows/acceptance.yml` ran it weekly from 2026-09-04 until 2026-09-25, when the workflow, `scripts/acceptance.sh` and `make acceptance` were removed along with the dedicated test organization's repository and Apps they needed; a run is now started by hand with the commands above. The runs recorded below came from that workflow.

**`billet acceptance` does not dispatch a workflow.** `up` prints the tier labels it derived; start a run that names one. What billet owns is standing the deployment up, observing what happened, and destroying exactly what it made — deciding which workflow to run is the operator's, and a command that dispatched one would need write access to a repository for no reason. The removed workflow dispatched one with an installation token minted in the run from a second App's key, never the acceptance App's: `billet check` refuses an App whose installation holds a permission billet never requested, in both directions, and when the acceptance App briefly held `actions: write` on 2026-09-04 every rehearsal's `billet local up` refused it with `actions: granted write, but billet never requested it`; that refusal is the credential-isolation claim doing its job.
