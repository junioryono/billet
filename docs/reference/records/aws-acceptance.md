...
**And the deny does not touch a launch billet makes, which rests on two live measurements rather than on the simulator.** billet's own `RunInstances` names no snapshot — `setBlockDevices` writes `DeleteOnTermination` and `VolumeSize` and never a `SnapshotId` — and a cache clone goes through `CreateVolume`, a different action. The AMI's own backing snapshot is not in the launch's authorisation context either: run `33844696218` launched two instances at 06:33Z on 2026-09-04, four hours after the deny was attached to the role it ran as, from `ami-07219c3abc48abfc8`, whose backing snapshot `snap-0100bbd0c0d1d15b0` carries no tags at all. Had that snapshot been authorised, every launch in the run would have failed instead.

**What that run proves is the account-less spelling, and the rest is an inference worth naming.** The role it ran as denies `arn:aws:ec2:*::snapshot/*`; the generator's deny is broader, so a launch is only at risk from it if EC2 presents a snapshot resource with an account in the ARN — and the second measurement says it does not, because `BilletCacheCloneSource`'s account-less ARN matched a real clone on 2026-09-03, which it could not have done if EC2 rendered snapshot resources with an account. Two measurements and a step between them, not three measurements. The step is closed by one `ec2 run-instances --dry-run` from a real AMI under a role holding the generated document, which is worth doing on the next acceptance pass; the outcome to expect is `DryRunOperation`.
