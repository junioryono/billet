...
**`billet acceptance` does not dispatch a workflow.** `up` prints the tier labels it derived; start a run that names one. What billet owns is standing the deployment up, observing what happened, and destroying exactly what it made — deciding which workflow to run is the operator's, and a command that dispatched one would need write access to a repository for no reason. The workflow dispatches one, and the credential it uses is an installation token minted in the run from a second App's key. It cannot be the acceptance App's: `billet check` refuses an App whose installation holds a permission billet never requested, in both directions, and when the acceptance App briefly held `actions: write` on 2026-09-04 every rehearsal's `billet local up` refused it with `actions: granted write, but billet never requested it`; that refusal is the credential-isolation claim doing its job. So a dedicated dispatch App is installed on the guide repository alone with `actions: write` and the mandatory `metadata: read`, registers no runners, holds no organization permission and can mint tokens for the guide repository alone, and the acceptance App keeps exactly the two permissions the manifest flow asked for. The token is narrowed at minting to that repository and that permission, and the step after the dispatch revokes it and fails the job unless GitHub confirms the revocation; a job cancelled or a runner lost before that step runs leaves the token's own hour as the bound. No person's token is involved, which is the difference between a credential that expires on a date nobody watches and one that exists for seconds inside the job that needed it (measured 2026-09-04: the token the dispatch App mints carries exactly `actions: write` and `metadata: read` on that one repository, and `DELETE /installation/token` answers 204).
...
Until 2026-09-04 the workflow above had never run: the role it assumes did not exist, nothing in the account trusted GitHub's OIDC issuer, and the repository held none of its secrets. The consumer's own Terraform root now creates the GitHub OIDC provider and the `billet-acceptance` role, trusting exactly this repository's `environment:acceptance` subject, so the workflow's job declares that environment and a scheduled run from `main` and a dispatched run from a branch are one principal. The role carries the node policy `billet init iam --account-wide` renders over the lane's base config (account-wide because `billet acceptance up` mints a fresh deployment identity every run), plus one delete for the teardown's cache purge, and an egress-only security group that doubles as the untrusted group. The acceptance runner App, the one billet itself runs with, is a dedicated one, created with `billet github-app create` and installed on the organization for the guide repository alone, so the production controller's key never leaves the controller; the dispatch App below is a second, separate one. The AMI the lane boots, `billet-acceptance-x64-20260904` (`ami-07219c3abc48abfc8`, contract 2), was built by `billet ami build` and boot-verified by `billet ami verify` after the build's own credentials expired between registering the image and verifying it; a build takes longer than an hour of SSO session on an 80 GiB builder disk.

**One guard the account stated before the generator did.** The account-wide policy grants `ec2:RunInstances` on `*`, and RunInstances authorises any snapshot a block-device mapping names; the same account keeps the production ledger volume's daily snapshots, a volume holding the deployment identity and the node-wire CA key. The role has carried an explicit deny on `ec2:RunInstances` for `arn:aws:ec2:*::snapshot/*` when `ec2:ResourceTag/sh.billet.owner` is null since 2026-09-04T02:54Z (CloudTrail's `PutRolePolicy` on `billet-acceptance-guards`). Measured with `aws iam simulate-principal-policy` that day: `RunInstances` against a production ledger snapshot ARN is an explicit deny from that policy; the same with an owner tag in context is allowed; a plain instance launch is allowed. `internal/awspolicy` renders the same boundary for every ec2 node now (#39), so this inline statement is redundant with a rule the node policy already carries — redundant rather than identical: it names the account-less ARN and `ec2:ResourceTag`, where the generated one takes a wildcard account and `aws:ResourceTag`, and the table below shows the account-less form covering strictly less. It is kept deliberately rather than removed, because this role is what stands in front of the production ledger's snapshots and it costs nothing to say the same thing twice.

**What the generated deny answers, measured with `iam:SimulateCustomPolicy` in this account on 2026-09-04.** The documents are the exact bytes `billet init iam --account-wide` and `billet init iam --deployment …` print, and the snapshot is a real one: `snap-00a7dd8a1cdf7b2fa`, described in the console as `billet control-plane ledger: manual pre-cutover snapshot (deployment identity + node-wire CA)`, carrying no tags. Both modes and both directions each, on `aws:ResourceTag/sh.billet.owner` (the key every other condition in the generator uses; the inline guard above spells it `ec2:ResourceTag`, and EC2 supports both).

| Mode | Snapshot in the request context | Decision |
|---|---|---|
| per-deployment (`StringNotEquals` on the owner) | no owner tag | explicit deny |
| per-deployment | another deployment's owner | explicit deny |
| per-deployment | this deployment's own owner | allowed |
| per-deployment | none — a plain launch (instance + image) | allowed |
| account-wide (`Null` true) | no owner tag | explicit deny |
| account-wide | any owner tag | allowed |

**Two things that reading the documentation would have got wrong.** A negated matching operator evaluates TRUE when the key is absent, so the per-deployment `StringNotEquals` denies an untagged snapshot as well as a foreign one and needs no second statement — that is the row above, not an inference. And the ARN shape matters more than the documented one: an EBS snapshot ARN is account-less, but `arn:aws:ec2:*::snapshot/*` answers **allowed** for `arn:aws:ec2:us-west-2:810711872940:snapshot/snap-…`, while `arn:aws:ec2:*:*:snapshot/*` denies both spellings, so the generator takes the wildcard account and the inline guard above covers strictly less.

**And EC2 was asked directly, which a simulation cannot substitute for.** A throwaway role (`billet-issue39-dryrun`, created and deleted within the hour) held exactly the account-wide document `billet init iam` prints, and three `ec2 run-instances --dry-run` calls were made as it, against `ami-07219c3abc48abfc8` in the acceptance subnet, on 2026-09-04.

| Policy on the role | The request | EC2's answer |
|---|---|---|
| the generated one | billet's own launch shape — one mapping for the AMI root, no `SnapshotId` | `DryRunOperation` |
| the generated one | the same, plus `/dev/sdf` from `snap-00a7dd8a1cdf7b2fa` | `UnauthorizedOperation` … `not authorized to perform: ec2:RunInstances on resource: arn:aws:ec2:us-west-2::snapshot/snap-00a7dd8a1cdf7b2fa with an explicit deny in an identity-based policy` |
| the rendering **before** this change | the same snapshot-bearing request | `DryRunOperation` |

**The third row is the finding, and the first two are why the fix is safe.** Before the deny, EC2 authorised a request that attaches the control plane's identity-and-CA volume to a machine running somebody's workflow; after it, EC2 refuses and names the snapshot in the refusal. A dry run answers the authorisation question and no other — it does not say the instance would have reached `running`, and it does not have to: authorisation is the boundary this is about. The first row answers what no simulation could: that launch was allowed although the AMI's own backing snapshot (`snap-0100bbd0c0d1d15b0`) is untagged, so the AMI's backing snapshot was not evaluated while a snapshot the block-device mapping *names* was. That is measured for the image this lane boots and the request billet sends, which is the case that matters; were AWS to change it, the failure would be a refused launch rather than a widened grant. And the ARN in EC2's refusal is account-less, which is what the generator's wildcard account matches and would go on matching if that spelling ever changed. Acceptance run `33844696218` is the standing evidence beside it: two instances launched at 06:33Z, four hours after the inline guard landed on the role they ran as.

The workflow first expected a secret only a person can mint, a fine-grained personal access token that dispatches the guide repository's workflow, and nobody had minted one. It now mints an installation token from a second, dispatch-only App's key, as the paragraph on dispatch above says; the acceptance App itself keeps exactly the permissions billet requested, because `billet check` refuses anything else.
...
**The third run passed, 2026-09-04, run `33844696218`, dispatched from `main` after #52.** It assumed the role, derived deployment `b56e23a2fce775a021bce17a4d3b4b7f`, minted the dispatch token from the dispatch App, read the guide repository's default branch over REST, dispatched its workflow, revoked the token, and started the control plane and the EC2 orchestrator node at 06:33. Then nothing happened for eighteen minutes: the dispatched job sat `queued` at GitHub with the scale set registered and advertising. The cause was the organization's Default runner group, whose visibility is `selected` and whose repository list was empty, so GitHub routed nothing to it; `billet check` knows this trap and refuses it, but only for a trusted tier's named group, and an untrusted tier in the Default group is never asked (#53). Adding the guide repository to the group at 06:52 was enough: the control plane escrowed two jobs within seconds (the dispatched one and a push-triggered run that had been queued since the label changed at 03:06), launched two `c7i.xlarge` instances, and both jobs ran to `succeeded` on GitHub and `done` in the ledger; from billet's escrow of each lease to the job starting on its instance took 131 and 102 seconds (the evidence file's `queued_at` and `started_at`), while GitHub's own queue-to-start for the dispatched job was 23 minutes, almost all of it the runner-group wait. A third lease was capacity no job started on and was released. `acceptance run` then sealed the deployment, drained, asked the node what it held (two empty answers spanning the grace, `proved clear` at 07:02:56), stopped both services and wrote `evidence.json` with three leases and nothing outstanding; `acceptance down` deleted the scale set and decommissioned the run's resources, both instances read `terminated`, and `acceptance sweep` was green. From dispatch to green: 32 minutes, 18 of them the runner-group wait.

**And the lane had no cache at all, 2026-09-04, which nothing said anywhere (#55).** The base config carries `node.ebs_s3`, names a `site` whose store is `ebs-s3` and pins its tiers to it — and carries no `node.cache`. That loads, starts and runs jobs without a complaint: a job pulling an image reported `/dev/root 77G` for `/var/lib/docker` and `cache: COLD`, and afterwards the bucket held no state object, the account held no `sh.billet.cache-owner` snapshot and no `sh.billet.owner` volume, and `billet decommission` reported `purged 0 snapshot(s), 0 volume(s), 0 state object(s)`. The cause is that a guest asks for its Docker image store over the node's cache API, and the node builds that service — and constructs the ebs-s3 store at all — only when `node.cache` is set, so without it the guest is handed no `BILLET_CACHE_ENDPOINT` and asks for nothing. Config validated only the other direction (an EC2 `node.cache` needs `node.ebs_s3`), and `billet check` made it worse by probing the bucket regardless and printing that it answers. `billet check` now refuses `node.ebs_s3` with no listener, naming what the jobs get instead; it is a check refusal rather than a load one because `billet decommission` and `billet init iam` both read the block on a config whose listener is gone. The same question for Ceph is reported and not refused: `image_pool` still boots every guest, `cache_pool` is a required field, and `billet init --provider firecracker` writes exactly that shape.
