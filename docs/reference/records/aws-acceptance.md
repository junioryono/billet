...
**The third row is the finding, and the first two are why the fix is safe.** Before the deny, a node role could have launched an instance with the control plane's identity-and-CA volume attached, and EC2 would have started it; after, EC2 refuses and names the snapshot in the refusal. The first row settles what no simulation could: a launch from an EBS-backed AMI whose own backing snapshot is untagged (`snap-0100bbd0c0d1d15b0`) is allowed, so the AMI's backing snapshot is not in the authorisation context while a snapshot the block-device mapping *names* is. And the ARN in EC2's refusal is account-less, which is what the generator's wildcard account matches and would go on matching if that ever changed. Acceptance run `33844696218` is the standing evidence beside it: two instances launched at 06:33Z, four hours after the inline guard landed on the role they ran as.
