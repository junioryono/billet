...
| `runs_on` | the scale set's name on its target and the `runs-on` value workflows use; defaults to `label`. Unique within a target, so tiers on different targets can each answer to the same name (`billet-2vcpu-ubuntu-2404` on every organization) while their labels stay distinct. With `sizes` it expands as the label does. Changing it moves the tier to a new scale set; the old one is reported as undeclared and removed with `billet teardown` |
