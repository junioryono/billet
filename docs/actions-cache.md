...
The control plane owns a deny list in its state database, and the node asks it before every locally handled cache operation. Policy lookup failure is passthrough, never local permission. Organisation and repository names are case-insensitive.
...
Run the reusable or manually dispatched `Cache and artifact conformance` workflow with the cache-enabled label. Supply `expected_runner_version` and `expected_guest_contract` when qualifying a candidate image so a passing run cannot accidentally describe another generation.
