...
The package installs the binary at `/usr/bin/billet`, the units, and a config template at `/etc/billet/billet.yaml` if none exists. It creates the `billet` service account and its directories, prepares the RBD kernel client the Firecracker backend needs, and **does not enable or start the services**: installing billet must not connect a machine to GitHub before its config says something true. The one exception is two timers, `billet-upgrade.timer` and `billet-images-refresh.timer`, which the package does enable: they act only on a rollout the ledger already records and on guest images the config already names, connect nothing to GitHub, and are what lets a deployment take the update it decided on with nobody at a keyboard. `systemctl disable --now` either one to opt a host out; `release: {automatic: false}` in the config stops both from acting. Removing the package removes the units and binaries and deliberately keeps `/etc/billet`, `/var/lib/billet` and `/srv/jailer`, because they hold the deployment's identity, credentials and any recoverable guest state.
