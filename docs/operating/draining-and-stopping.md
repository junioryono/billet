...
- `billet node` stops taking new work, waits for the compute already running here, destroys it when each job completes, tells the control plane it is leaving, and exits. It notices that it holds nothing within about two seconds of the last compute going, whatever the sweep interval. The first launch it refuses while draining takes the host out of placement until it registers again, so the control plane stops sending it work it will only refuse.
