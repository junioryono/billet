...
## Who may touch a controller's identity, and when

A controller's identity directory (`server.identity_dir`: the deployment id, the node-wire authority under `ca/`, and on SQLite the ledger) is written by more than the server. `billet check` creates the ledger and the identity on a fresh host; `ca show`, `ca issue`, `ca rotate`, `ca retire`, `ca sync` and `nodes approve` create or replace the authority; `local backup`, `local restore` and `local recover` read or publish all of it. A controller's retirement (a later step of this design, `billet server retire`) renames the whole directory. So every one of those commands takes the same exclusion BEFORE its first access to the directory, and every helper it calls borrows that hold rather than taking its own.

On a Linux host an installer has prepared, the exclusion is two locks in one order: the global authority lock at `/var/lib/billet/authority.lock` (root:`billet` 0660, so the service account can take it read-only), then the directory's own `ca.lock`. Under the global lock the command reads `/var/lib/billet/authority-status`, which a retirement publishes with its phase; a status of `stopped` or later means the authority is closed, and an ordinary command refuses with that phase rather than creating anything beside a directory that is being moved. The record of the service account at `/var/lib/billet/service-account` is what the locks are owned for and what a privileged command hands its artefacts back to: a root `ca show` that mints the authority on a fresh host leaves it owned by the service account, not by root, and a root `billet check` that created the ledger and its lock hands those back too, whether or not its open succeeded (the opener creates the lock before it connects). The record is read when the command releases, not when it started, so a command that was already inside the directory when an installer published the record still hands its artefacts to the account the installer named. On a prepared host the directory exists from the installer's bootstrap onwards and is moved only by a retirement, so an ordinary command that finds it absent refuses and recreates nothing, the ledger opens included: a command that opens the ledger without holding the exclusion (`billet status`, the upgrade timer) takes it for the open, so the opener's creation of the directory can never race a retirement's rename. A fresh host is initialised by `billet check`, `billet local up` or the host role, never by a command that happened to run first, and the directory is created only under the initialisation lock beside it, after the record, the global lock and the status have all been found absent under that lock.

A host is prepared by one of three installers, never by an ordinary command: the package's postinstall, `billet local up`, and the host role. Each runs `billet local prepare`, which records the validated service account, creates the identity directory only on a positively fresh host (absent, with no global lock and no status beside it: a directory absent beside either was moved by a retirement or damaged by hand, and is left absent), creates or repairs both locks by descriptor with their inodes kept (a lock with a second link or on another filesystem is refused, since re-owning it would give away whatever else names that inode), and reports the published status. A status it cannot read or parse refuses the preparation, because the installer cannot tell whether it is restoring a retired host. A closed status does not stop the record being restored, and it does stop everything else: `local up` refuses to start a retired controller, and the package leaves its configuration unseeded and its timers as they were, saying so and naming `dpkg-reconfigure billet` as the retry once whatever held the lifecycle lock has finished; a status the package cannot re-read under that lock defers the same way, since a preparation that failed proves nothing about the host.

A host no installer has prepared (no record, and positively no global lock) is the world before this rule: the directory's own `ca.lock` is the whole exclusion, every privileged command takes it before its first access as it did not before, and the one caller left as it was is the unprivileged server's own startup, which cannot open a root-created lock and cannot meet a retirement there, since a retirement refuses on such a host and names the installers. A command on that path rechecks for the global lock once it holds `ca.lock`; finding one, it releases, takes the global lock, retakes `ca.lock`, looks the configured pathname up afresh and asks the status, so an installer's publication in the gap is honoured rather than raced. On macOS none of this exists: the login account owns everything, `ca.lock` is the exclusion, and no controller retires.

What this does not cover, said: a binary older than this rule holds none of these locks, so a host is prepared while no such command runs (the installers wait on `ca.lock` for one that holds it, and cannot see one that has not yet taken it); the service-account record is root's assertion of who the services run as, not a proof; and a tarball installation whose services run as a non-root account nobody recorded has root-owned artefacts left by root commands, as before.
