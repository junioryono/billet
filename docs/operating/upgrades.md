...
**The upgrade root is a trust boundary and every command proves it first**, through descriptors: `/var/lib/billet` and the root owned by the account running billet's transactions (root on Linux, the launch agent's account on a Mac) and writable by nobody else, `active` 0700, `transaction.lock` a regular file, none a symlink, the root opened relative to its parent and the lock relative to the root, each without following a link. A root that does not exist is created inside the validated parent; a parent that does not exist is refused, because the package and the role make it.
