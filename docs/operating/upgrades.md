...
`billet converge-guard hold --holder <run id>` publishes the guard under the transaction lock in one durable order, and `release --holder` removes it. The role holds and never releases: the operator releases with the holder they exported, and the preparation prints the command. A hold by the holder that already holds is a validation and touches nothing. Another holder's guard refuses, naming it and how long it has been held, and **nothing expires a guard**: a converge that died holding one leaves it, `billet check` warns past a day, and an operator removes it with `billet converge-guard recover --holder H --old-driver-stopped`, the assertion that the driver which held it is stopped or cannot dispatch further work and that its remote work has finished. The process table is scanned for Ansible's markers as an extra refusal (a recovery run from inside the driver it asserts is stopped is refused with "run this over a direct SSH shell"), but the assertion carries the weight: an unmarked driver, or work dispatched after the scan, is invisible to it.

**The role prepares the exclusion before anything else.** `prepare-exclusion.yml` is the host role's second task (after the refusal of a billet-managed runner) and the first pre-task of every play of the fleet playbook, so `ssh_access`, the connectors and the development role are under it too. It needs a holder, `billet_converge_guard_holder`, read from `BILLET_CONVERGE_GUARD_HOLDER` on the controller or passed with `-e` (which wins); a converge without one refuses before it looks at anything, and a dry run needs none because it holds nothing. On Linux it establishes the upgrade root (`/var/lib/billet` 0755 root, `/var/lib/billet/upgrades` 0700 root, refusing either when it exists as anything else), then classifies `active` by a stat that follows nothing: a symlink refuses naming `billet host-upgrade --status`, a regular file is a pre-R role transaction the role recovers before ending the play, a directory is asked through `billet converge-guard status --json`, and anything else refuses by type. The managed binary answers when it can; when it is absent or predates the guard, the executable the guard records answers, after every component from `/var/lib/billet` down to it is proved root-owned and writable by nobody else, the record read as exactly its five members, and the file's digest matched to the recorded one, and an executable that fails any step is never run. A guard by another holder refuses naming it, its age and the way out (`recover --holder` without a pointer, a takeover with one); a guard by this holder is a same-holder rerun, validated by one `hold` before anything else and admitted only when this run's intent matches the record (no binary change over a record naming the managed binary, or the same candidate digest over a record naming a candidate that was never installed), else refused naming `release --holder`; a guard by this holder with a pointer is this converge's interrupted transaction, recovered as before. Only then is the binary change decided and its candidate staged: the pin or channel resolved, the candidate copied into a recovery directory allocated by an exclusive `mkdir` (never `state=directory`, which accepts an existing one), the staged copy asked for its version (the downgrade refusal) and for `converge-guard status --json` (a candidate that answers "unknown command" predates the guard and cannot take part in the exclusion; that release's collection is the one to use), all before the hold. The hold runs through the candidate with `--candidate` when one is staged, else through the managed binary; a managed binary that predates the guard, with no candidate and not a PostgreSQL controller, runs the legacy protocol (the regular-file claim) with a warning, and a PostgreSQL controller whose binary predates the guard refuses instead, because the role cannot move it. A play that stops between the hold and its transaction's pointer leaves the guard with none, and what ran between the hold and the failure is not rolled back by the binary journal; the preparation says so before it holds.
