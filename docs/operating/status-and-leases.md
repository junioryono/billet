...
```text
          ...
          NO ADMISSION PROGRESS PUBLISHED for 18m2s (last 2026-09-23T15:58:38Z): unless its listener is in a launch, other tiers may buy ahead of it until it progresses
```

That line is an observation, not the queue's decision: a stalled listener cannot publish that it has stalled, so `billet status` compares the last progress it did publish with its own clock, and a listener in the middle of a long launch still holds the line although it has published nothing new. Under `fill` anything that fits starts instead, and the largest shape may wait indefinitely.
