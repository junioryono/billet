...
**Every client that reaches the origin has to trust the node's CA, and the job hook publishes the variable each one honours**, all naming one bundle that carries the distribution roots as well: `NODE_EXTRA_CA_CERTS` for Node, `SSL_CERT_FILE` for OpenSSL, Go, curl and rustls, `REQUESTS_CA_BUNDLE` for Python's `requests`, which reads certifi's bundle and ignores the system store otherwise, and `CURL_CA_BUNDLE` for curl. The `client-trust` conformance job proves curl, Python's standard library, `requests`, Go and `sccache` each reach the origin without a certificate error. A Java client with its own truststore is the one not covered; see the [compatibility page](compatibility.md).

**A job that runs in a `container:` gets the same shim in front of the image's own client.** On an interception tier the runner is pointed at billet's container hook (`ACTIONS_RUNNER_CONTAINER_HOOKS`), a small wrapper around GitHub's reference docker hook, vendored and pinned by version and sha256, that adds exactly one thing to the job container: a read-only bind of the shim at `/opt/billet/bin/docker`. The job hook prepends `/opt/billet/bin` to `GITHUB_PATH`, so the shim is first on every step's PATH on the host and in the container, and finds whatever client the image carries behind itself; it is never mounted over an image's own `/usr/local/bin/docker`. Everything else about job and service containers is the reference hook's behaviour, unchanged, and the `container-docker-buildkit` conformance job is the proof. Tiers without interception keep the runner's built-in docker path.

One limit is worth knowing. BuildKit writes one cache entry per layer blob while a job may hold at most 32 local archives and clones at once, so `mode=max` on a large image can reach that ceiling; entries past it are answered by GitHub individually, which leaves the build correct and partly accelerated rather than failed.
