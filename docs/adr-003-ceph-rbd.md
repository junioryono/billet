...
**What #26 still has to establish on the pilot host** is narrower than it was: not whether RBD is viable for build workloads or how a guest receives one. Firecracker now reserves five drives, the node hot-attaches a mapped clone through an instance-bound token, the action formats cold volumes as ext4 and mounts them, and its post hook unmounts before the node verifies and publishes. The remaining evidence is whether the *hydrate* step of a real `node_modules` volume stays acceptable at 4× on that host, plus the power-cut loop that proves the previous pointer stays mountable across real kernel and Ceph failure boundaries.
