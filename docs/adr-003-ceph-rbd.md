...
That bounded copy changes the request budget on both paths that can invoke it. Attach and commit each give storage eleven and a half minutes of work inside a twelve-minute handler envelope, reserving thirty seconds for cancellation-resistant RBD cleanup and lock release; the action waits thirteen minutes, and a commit's writer authority lasts fifteen. A canceled attach removes any partially copied image through a fresh bounded cleanup context instead of accumulating a new large orphan on every retry.
