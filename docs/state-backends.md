...
The decision is smaller than it looks. **Both backends run exactly one controller at a time**, both hold the same invariants, and the ledger's shape is identical on either — the same 45 migrations, the same 169 statements. What changes is where the rows are, and therefore what recovering the controller involves.
...
**The transactional host upgrade refuses too.** It works by snapshotting and restoring the state directory, which is not a thing an external ledger has. Upgrade a PostgreSQL controller with the service stopped — and note that the `junioryono.billet.host` role refuses a binary change on such a host up front, naming this, rather than failing partway through a transaction it cannot finish. **A first install is a binary change**, so put `/usr/bin/billet` in place yourself and then converge; with the binary already there the role does everything else.
