...
2. **In System Settings → General → Sharing**, turn on Remote Login, restricted to that account. In **Users & Groups**, set Automatically log in as to it. The account keeps its password; automatic login only skips the login window at boot, and SSH and `sudo` still ask for it. Leave **Screen Sharing off** on a Mac that will run untrusted tiers: it takes the account's password, and an untrusted guest reaches it at its gateway (below). Turn it on over SSH only while you need the screen, `sudo launchctl enable system/com.apple.screensharing && sudo launchctl kickstart -k system/com.apple.screensharing`, and off again with `sudo launchctl disable system/com.apple.screensharing && sudo launchctl bootout system/com.apple.screensharing`, which also holds across a reboot.
...
An untrusted generation (the default) writes `node.tart.untrusted_isolation: softnet`, and `billet check` **fails** until the grant is in place, because a node that offers to confine a fork's job on a host that cannot confine it is worse than one that refuses. tart's default NAT reaches the host and lets a guest spoof the bridge; softnet is the mechanism billet drives. **softnet does not close the host itself.** Its default lets a guest send to its vmnet gateway, and that gateway is the Mac, so an untrusted guest reaches every TCP service the host listens on. Measured from an untrusted macOS guest and an untrusted arm64 Linux guest on the reference mini: the private address space timed out, while the gateway answered on SSH (22) and Screen Sharing (5900). Until billet sets a softnet policy that blocks the host (#190), keep SSH key-only (the drop-in in [Headless operation](../reference/reference-hardware.md#headless-operation)) and keep Screen Sharing and any other password-accepting service off on a Mac that runs untrusted tiers. Pass `--runner-group` and `--workflow` for a trusted pool instead, and the generation omits the isolation rather than promising one nothing uses.
