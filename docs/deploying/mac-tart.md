...
Enable SSH first (System Settings → General → Sharing → Remote Login, or `sudo systemsetup -setremotelogin on`); on macOS 26, Screen Sharing after a reboot needs SSH already reachable, so it is what makes everything else recoverable. Harden with a drop-in that sorts ahead of Apple's own `/etc/ssh/sshd_config.d/100-macos.conf`, because the first value wins, and turn off `KbdInteractiveAuthentication` as well as `PasswordAuthentication`: keyboard-interactive is a second method that can take a password through PAM ([the drop-in](../reference/reference-hardware.md#headless-operation)). Do not reach for `pmset autorestart`: it is a silent no-op on Apple Silicon, and unnecessary, because an Apple Silicon mini is documented to power on when mains power returns after a loss. A Mac that was shut down is a different case: set *Start up when power is connected* (System Settings → Energy, or `sudo pmset -a autorestartatconnect 1`) if it should boot when power is reconnected, which Apple documents for 2024-and-later minis under macOS 26.5 and later. Neither that setting's effect nor recovery from a real power cut is measured on the reference mini yet.
