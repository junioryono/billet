...
- **FileVault off, decided in Setup Assistant.** macOS offers no automatic login while FileVault is on (Apple's documented rule, not one billet measured), so the requirement above settles the question Setup Assistant asks. It also decides what a power cut costs: with FileVault on, a Mac that comes back on waits at the unlock screen for a person, and until one arrives no agent starts and every job for its tiers queues. Once FileVault is off, set System Settings → Users & Groups → Automatically log in as, to the account that will run the node.
