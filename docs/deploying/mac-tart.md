...
Enable SSH first (System Settings → General → Sharing → Remote Login, or `sudo systemsetup -setremotelogin on`); on macOS 26, Screen Sharing after a reboot needs SSH already reachable, so it is what makes everything else recoverable. Harden with a drop-in that sorts ahead of Apple's own `/etc/ssh/sshd_config.d/100-macos.conf`, because the first value wins, and turn off `KbdInteractiveAuthentication` as well as `PasswordAuthentication`: macOS asks for passwords through PAM's keyboard-interactive path, which `PasswordAuthentication no` does not close ([the drop-in](../reference/reference-hardware.md#headless-operation)). Do not reach for `pmset autorestart`: it is a silent no-op on Apple Silicon, and unnecessary, because Apple Silicon minis power on when mains power returns.
