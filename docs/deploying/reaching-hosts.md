...
```hcl
module "converge" {
  source = "github.com/junioryono/billet//terraform/modules/converge-aws-ssm?ref=v0.12.13"
...
```hcl
module "converge" {
  source = "github.com/junioryono/billet//terraform/modules/converge-cloudflare?ref=v0.12.13"
...
It creates the tunnel, its ingress mapping the hostname to SSH, its DNS route, an Access application with the CI policy, and a service token. The host side is the collection's `junioryono.billet.cloudflared_connector` role, which the shipped `junioryono.billet.fleet` playbook runs on every host in its `control_plane` and `linux` groups (a macOS host gets no connector from it): it reads the tunnel token from `BILLET_CLOUDFLARED_TOKEN_<HOST>` in the environment (never `-e`, which lands in argv), requires `billet_cloudflared_expected_tunnel_id` and refuses a token whose embedded tunnel id is not it, verifies Cloudflare's package signing key against a pinned fingerprint before trusting the repository, writes the token file and renders the unit itself so the credential never reaches an installer's argv, and refuses any change that would restart a running connector the play may be reaching the host through (`billet_cloudflared_routed_address` derives that from `ansible_host`: equal is yes, a different IP literal is no, anything else is could-not-tell and refuses). Without a token it does nothing.
...
```hcl
module "converge" {
  source = "github.com/junioryono/billet//terraform/modules/converge-cloudflare-warp?ref=v0.12.13"
...
The workflow installs the WARP client from a fingerprint-verified signing key, writes `mdm.xml` with `install -m 0600` from stdin so the secret never lands in argv or a world-readable file, restarts the daemon (the client reads the file once at startup), waits for *Connected*, and **proves each host with a TCP check before trusting the path**, because a device that enrolled and is then denied by Gateway looks identical to a connected one until the first SSH times out. Host keys are committed pins, never an `ssh-keyscan` at job time. At the end it deletes its registration. The host half, a machine at home joined as a WARP Connector, is the collection's `junioryono.billet.warp_connector` role: the token from `BILLET_WARP_CONNECTOR_TOKEN_<HOST>` in the environment, the client from a key verified against the pinned fingerprint, the forwarding sysctls, and `warp-cli connector new` exactly on the daemon's own *Registration Missing* and never again, because re-enrolling tears down the interface the play may be running over; a rotated connector token is a manual `registration delete` on the host over a path that does not depend on the mesh. The controller in the VPC runs the `cloudflared_connector` role above, advertising its own address.
