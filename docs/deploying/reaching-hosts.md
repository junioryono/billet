...
It creates the tunnel, its ingress mapping the hostname to SSH, its DNS route, an Access application with the CI policy, and a service token. The host side is the collection's `junioryono.billet.cloudflared_connector` role, which the shipped fleet playbook runs on every host: it reads the tunnel token from `BILLET_CLOUDFLARED_TOKEN_<HOST>` in the environment (never `-e`, which lands in argv), refuses a token whose embedded tunnel id is not `billet_cloudflared_expected_tunnel_id`, verifies Cloudflare's package signing key against a pinned fingerprint before trusting the repository, writes the token file and renders the unit itself so the credential never reaches an installer's argv, and refuses to rotate a credential over a connector the play is reaching the host through (`billet_cloudflared_routed_address` derives that from `ansible_host`; nothing declared refuses). Without a token it does nothing.
