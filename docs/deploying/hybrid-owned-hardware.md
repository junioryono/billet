...
`--cache` adds the cloud half's cache in the same generation. The two halves cache in different places and neither can reach the other's storage — the Firecracker host writes RBD images into its own Ceph pools, the cloud writes EBS snapshots with a fenced pointer in S3 — so the generation declares both as **sites**, names each node's own, and gives the orchestrator the `node.ebs_s3` store plus the HTTPS `node.cache` listener its job instances fetch through. The one thing it cannot produce is that listener's certificate, and that costs you TWO steps rather than one, because the endpoint is on a private address and no public root signs for it. The **issuer** goes into the image, at build time, as `billet ami build --ca-cert`; the **leaf** goes to the converge as `BILLET_CACHE_TLS_CERT_PATH` and `BILLET_CACHE_TLS_KEY_PATH`, valid for the controller's address, because that is what a guest dials. They must be one pair. Skip the issuer and nothing reports an error: every guest's cache request fails its TLS handshake, every job fetches cold, and the bucket, the listener, the site and the security-group rule are all correct and all unused. The generated runbook asks for it in step 6 for that reason.
...
What the generator still leaves to you, deliberately: the Ceph bootstrap decision on the local host (destructive, never inferred), the local host's own address as Ansible reaches it, and the cache listener's issuer and leaf above.
