...
The design record is [ADR-007](../reference/decisions/adr-007-codebuild-provider.md). This page is what you need in order to run it.
...
The Terraform module [`fleet-codebuild`](https://github.com/junioryono/billet/tree/main/terraform/modules/billet/modules/fleet-codebuild) creates or adopts the project, an optional reserved fleet, both roles, the log group, the VPC configuration, the parameter path and an optional KMS key, and outputs the non-secret facts your `billet.yaml` needs. Linux on demand is the inexpensive default; macOS reserved capacity is an explicit opt-in. Its README documents every input.
