...
func retirePendingReason(err error) string {
	// EVERY QUESTION BELOW IS ASKED OF THE WHOLE ERROR, and each the same way:
	// an open that refuses joins its answer with its own cleanup, so a tree
	// can carry both what a survivor could finish and a failure of this
	// host's. `OnlyCause` requires every leaf to be the sentinel and
	// `Unreachable` requires every independent branch to establish an
	// availability failure; a rule that let one of them through a cleanup the
	// other refuses would hide that fault behind a row somebody else is
	// waiting to write.
	switch {
