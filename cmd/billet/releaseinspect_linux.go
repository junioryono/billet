...
// for its identity alone. A plain open of "<root>/<path>" is not that: the
// kernel resolves an absolute symlink met on the way against the caller's root,
// so a config that is a symlink into a directory the service has bind-mounted
// differently would be opened in the inspector's namespace and compare equal to
// a file the service never reads. openat2 with RESOLVE_IN_ROOT scopes every
// symlink and every ".." to the directory descriptor, which is the process's
// root in its own mount namespace when rootLink is /proc/<pid>/root;
