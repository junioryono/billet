...
func reapKernelDir(
	...
	for _, entry := range entries {
		...
		// AND NEITHER IS THE LOCK THAT MAKES THIS SAFE.
		//
		// PlanKernelReap's pattern cannot match it, so this is unreachable today --
		// and it is here for the reason the base-name guard below is: the thing on the
		// other side is os.Remove running as root, and "unreachable today" is a
		// property of two functions that can each change without the other noticing.
		//
		// What a single pattern change would cost is not one deleted file. Unlinking a
		// HELD lock file does not release the flock; it detaches the PATH from the
		// locked inode, so the next billet creates a fresh file under that name, locks
		// that, and both run -- with neither able to tell. That is the failure that
		// moved the deployment lock out of a cache directory.
		if entry.Name() == kernelLockName {
			continue
		}
