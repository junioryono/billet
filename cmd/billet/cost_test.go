...
func TestEC2FleetCostLeavesStatusUsableForARetiredLegacyNode(t *testing.T) {
	db, err := state.Open(t.Context(), t.TempDir())
	if err != nil {
		t.Fatalf("state.Open: %v", err)
	}
	...
	out := capture(t, func() {
		if err := printEC2FleetCost(t.Context(), a, cfg); err != nil {
			t.Fatalf("printEC2FleetCost made status fail: %v", err)
		}
	})
	for _, want := range []string{"ec2 peak  unavailable", "retired-cloud"} {
		if !strings.Contains(out, want) {
			t.Errorf("output does not contain %q: %s", want, out)
		}
	}
