...
func TestCurrentBinaryRejectsAPreComposeGuest(t *testing.T) {
	...
	if firecracker.GuestContract != "10" {
		t.Fatalf("the current image requirements speak contract %q, want 10",
...
func TestCurrentBinaryRejectsAPreOverlay2Guest(t *testing.T) {
	...
	if firecracker.GuestContract != "10" {
		t.Fatalf("the overlay2 image change speaks contract %q, want 10",
			firecracker.GuestContract)
	}
	if err := manifest.Usable(firecracker.GuestContract, "x86_64"); err == nil {
...
func TestCurrentBinaryRejectsAPreActionsInterceptionGuest(t *testing.T) {
	...
	if firecracker.GuestContract != "10" {
		t.Fatalf("the Actions interception image change speaks contract %q, want 10",
			firecracker.GuestContract)
	}
	if err := manifest.Usable(firecracker.GuestContract, "x86_64"); err == nil {
...
func TestCurrentBinaryRejectsAGuestWithoutAWritableRunnerWorkDirectory(t *testing.T) {
	...
	if firecracker.GuestContract != "10" {
		t.Fatalf("the writable runner work-directory change speaks contract %q, want 10",
			firecracker.GuestContract)
	}
	if err := manifest.Usable(firecracker.GuestContract, "x86_64"); err == nil {
...
// Contract 9 images carry the interception passthrough but not the plaintext
// loopback cache adapter, so a container-driver BuildKit on such an image meets
// the node's leaf with a trust store billet cannot populate and its `type=gha`
// export dies with x509 -- the failure the adapter exists to remove.
func TestCurrentBinaryRejectsAGuestWithoutTheLoopbackCacheAdapter(t *testing.T) {
	_, manifest := stageDir(t, []byte("rootfs"), []byte("kernel"))
	manifest.GuestContract = "9"

	if firecracker.GuestContract != "10" {
		t.Fatalf("the loopback cache adapter change speaks contract %q, want 10",
			firecracker.GuestContract)
	}
	if err := manifest.Usable(firecracker.GuestContract, "x86_64"); err == nil {
		t.Fatal("a contract-9 image without the loopback cache adapter was accepted")
	}
}
...
func TestInstallKernelSurvivesTheStagingDirectory(t *testing.T) {
	...
	got, err := installKernel(durablefile.Installer{}, m, staging, kernels)
	if err != nil {
...
func TestInstallKernelSeparatesDifferentKernelsWithOneVersion(t *testing.T) {
	...
	pathA, err := installKernel(durablefile.Installer{}, a, first, kernels)
	if err != nil {
		t.Fatalf("install a: %v", err)
	}

	pathB, err := installKernel(durablefile.Installer{}, b, second, kernels)
	if err != nil {
...
func TestInstallKernelIsIdempotent(t *testing.T) {
	...
	first, err := installKernel(durablefile.Installer{}, m, staging, kernels)
	if err != nil {
		t.Fatalf("first install: %v", err)
	}

	second, err := installKernel(durablefile.Installer{}, m, staging, kernels)
	if err != nil {
...
func TestKernelFileNameIdentifiesTheFileOnDisk(t *testing.T) {
	...
	installed, err := installKernel(durablefile.Installer{}, m, staging, kernels)
	if err != nil {
		t.Fatalf("install: %v", err)
	}
...
func TestInstallKernelRefusesAnExistingFileThatIsNotThatKernel(t *testing.T) {
	...
	_, err := installKernel(durablefile.Installer{}, m, staging, kernels)
	if err == nil {
