...
internal/provider/   firecracker | tart | ec2 | docker  (all but tart are built)
...
**The cluster is a precondition billet CHECKS, not one it hopes for.** `billet check` refuses a cluster that would clone the old way, because on clone v1 a protected snapshot with a live clone can be neither unprotected nor removed — so a cache generation any running job holds would be undeletable and #25's eviction would be blocked by ordinary traffic. Refusing now is a judgement about when the cost lands: nothing clones yet, so the fix is one command on an empty cluster.
