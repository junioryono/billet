...
- **Registration is dynamic; the admission CLI is not, and that is a defect.** `Plane.Register` never asks whether a host was declared — it checks the protocol version, a non-empty name, the deployment identity, a non-zero contribution, and that the site is one the deployment declares (`WithSites`, wired from `cfg.SiteNames()`), after which the allocator requires a provider and refuses a provider or site change while leases are outstanding. So `nodes:` is policy about hosts rather than a roster of them. But every operator command that reaches the ledger — `nodes pending|approve|revoke`, `ca token|issue`, `leases quarantined|release` — goes through `controlPlaneAllocator`, which calls `state.Open` and takes the exclusive state-directory lock the running server already holds, so all of them fail with `ErrLocked` against a live control plane. `leases release --force` is the worst case: it exists to reclaim capacity on a running deployment. Tiers are read at startup and each becomes one scale set, and `nodes:` is snapshotted into `alloc.New`, so changing either needs a restart.
