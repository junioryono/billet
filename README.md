...
billet images compatible                 # prove the selected tier image speaks this binary's contract
```
...
`billet images pull --verify` combines import, a real guest boot and promotion, and `--result-file` records the exact promoted generation for an upgrade transaction to withdraw if it later rolls back. `billet images compatible` first reads the contract recorded on the selected verified generation. A generation published before that metadata existed is boot-verified once and backfilled instead of being replaced speculatively; a matching recorded generation needs no image download. `@verified` is contract-relative at launch: during a rolling upgrade, an older binary keeps resolving its newest compatible verified generation while the new binary may select the newer contract it just proved.
