...
| `billet check` | Validates the config, the App private key, and the state database. For an EC2 node it also reports that node's conservative compute-only peak implied by its declared shape prices and resource ceilings; deployment-wide aggregation across several EC2 nodes remains tracked in #44 |
...
**The cloud runner path is proven end to end; multi-provider failover is not.** Real private-repository jobs have launched on EC2, registered through JIT configuration, used Docker and a service container, installed Go at runtime, completed green, and left no live instance behind. EBS/S3 cache behavior still uses fake AWS boundaries, and nobody has stopped the preferred bare-metal host and watched the same `runs-on` label finish in a region, so #32 remains open. See [AWS acceptance](docs/aws-acceptance.md); [#33](https://github.com/junioryono/billet/issues/33) tracks the caching plane.
...
**EC2 cold-start measurement is built into images made by `billet ami build`.** Immediately before each `RunInstances` attempt billet puts its epoch in that attempt's user data. The image records when its runner entry point is ready, then GitHub's supported `ACTIONS_RUNNER_HOOK_JOB_STARTED` hook prints `billet timing: launch_to_job_start_ms=… launch_to_runner_ms=… runner_to_job_start_ms=…` before the first job step. Invalid or missing timing state exits successfully and prints nothing, so the probe cannot fail a workflow. Three cold runs reached the first job step in 47.6, 52.2 and approximately 55.0 seconds; the measured runs with complete phase data spent 37.9–42.2 seconds reaching the runner and 9.7–10.0 seconds from runner readiness to the job. That is acceptable for fallback capacity and does not justify live, billable, unleased instances, so `warm_pool` remains refused. See [AWS acceptance](docs/aws-acceptance.md).
...
| P8 — EC2 provider, cloud-hosted control plane, provider failover | 🚧 The provider, AMI builder and real GitHub job path are proven; exact purchased-shape accounting and spot-warning handling are implemented. Same-label power-loss failover and live Spot interruption remain [#32](https://github.com/junioryono/billet/issues/32) [#41](https://github.com/junioryono/billet/issues/41) |
