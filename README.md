...
**Transparent `actions/cache` interception is opt-in and stays off on unqualified tiers.** GitHub exposes no supported custom cache URL, and `ACTIONS_RESULTS_URL` carries artifact metadata as well as cache traffic, so Billet terminates TLS only for that exact host and handles only the three CacheService v2 methods from the official toolkit client. BuildKit `type=gha`, ArtifactService, unknown clients and unknown methods remain upstream; a supervised guest forwarder falls back to GitHub when it cannot establish the node tunnel; the runner's supported job hook propagates proxy and CA state into job and action containers; and `billet cache disable` is the central organisation/repository kill switch. See [Transparent Actions cache](docs/actions-cache.md) for configuration, failure semantics, and the live conformance gate.
...
| P4 — colocated Actions cache | 🚧 The generic interceptor, fenced archive store, kill switch and pinned/current conformance matrix are implemented; immutable release, transactional host upgrade and live cold/save/warm/restore/fault proof remain [#29](https://github.com/junioryono/billet/issues/29) |
